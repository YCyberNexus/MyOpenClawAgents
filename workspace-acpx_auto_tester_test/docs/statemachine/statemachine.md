> ⚠️ 已废弃 / 历史 v1。本文件描述早期语义（单一 `blocked`/`failed`、`done`+`pr` 共存、`continue` 重入队），已被取代。当前 benchmark-test 分支的权威状态机见 `workspace-acpx_auto_tester_test/skills/gitlab_issue_campaign_dispatcher/references/label_lifecycle.md`（及 statemachine.v2.md §6）。请勿据本文件理解当前行为。
