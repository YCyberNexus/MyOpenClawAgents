    ...
    ifp-result/                                          ← ${RESULT_ROOT}; agent runtime root
        ...
        .worktrees/                                      ← ${WORKTREES_ROOT}; per-issue linked worktrees (one per IID)
            issue-<iid>/                                 ← ${WORKTREE_DIR}; acpx cwd; created on attempt 1 via
                                                            ...
                                                            (every attempt resets from origin/${DEV_BRANCH}; continue disabled)
                .claude/ hulat/ ${DATA_BASENAME}/        (tracked config refreshed from origin/${DEV_BRANCH} every attempt)
                ${RESULT_BASENAME}/issue-<iid>/hulat-spec-issue<iid>/   ← ${OUTPUT_DIR}; Claude Code output (committed; pushed to ${LOCAL_ATTEMPT_BRANCH}; shared across attempts of this IID)
                ${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN-<tier>/  ← ${LOG_DIR}; STILL attempt-scoped inside the shared worktree (the -<tier> suffix is the pinned model MODEL — flash/pro/max — so the run folder shows the model at a glance)
                    prompt.txt                                          ← force-added by stage_and_guard.sh (pushed to ${LOCAL_ATTEMPT_BRANCH})
                    claude_result.txt                                   ← force-added by stage_and_guard.sh (pushed to ${LOCAL_ATTEMPT_BRANCH})
                    acpx_raw.log                                        ← force-added by stage_and_guard.sh (lands on ${LOCAL_ATTEMPT_BRANCH})
                    git_status.txt                                      ← force-added by stage_and_guard.sh (lands on ${LOCAL_ATTEMPT_BRANCH})
                    git_diff.patch                                      ← force-added by stage_and_guard.sh (lands on ${LOCAL_ATTEMPT_BRANCH})
...
1. `git clone -b ${BRANCH}` into `${REPO_PATH}` (uses a tmpfs lock at `/tmp/acpx_auto_tester_test.clone.${PROJECT}.lock` since the in-repo lock dir does not exist yet).
...
| `WORK_BRANCH`             | `issue/${ISSUE_IID}-auto-fix`                                                        | naming prefix only; NOT pushed as a remote branch (`LOCAL_ATTEMPT_BRANCH` is the remote) |
| `ATTEMPT_NUMBER_PADDED`   | e.g. "001"                                                                           | zero-padded for labels, comments, local branches              |
...
| `LOG_DIR`                 | `${WORKTREE_DIR}/${RESULT_BASENAME}/issue-${ISSUE_IID}/log/attempt-${ATTEMPT_NUMBER_PADDED}-${MODEL}` | current-attempt log files INSIDE the shared worktree (still attempt-scoped so successive attempts don't overwrite each other; the trailing `-${MODEL}` is the pinned model tier — flash/pro/max — so the run folder shows the model at a glance); the **whole** `${LOG_DIR}` (`prompt.txt`, `claude_result.txt`, `acpx_raw.log`, `git_status.txt`, `git_diff.patch`, `acpx_command.txt`, `timing.txt`, `metrics.json`, etc.) force-added and pushed to `${LOCAL_ATTEMPT_BRANCH}`; only the post-push `wiki_*` files locally ignored and removed with the worktree |
...
| `LOCAL_ATTEMPT_BRANCH`    | `${WORK_BRANCH}-att${ATTEMPT_NUMBER_PADDED}-${MODEL}`                                | the single remote branch — immutable per-attempt branch, pushed once and never overwritten; the trailing `-${MODEL}` is the pinned model tier so the GitLab branch list shows which model produced the run (e.g. `issue/14-auto-fix-att003-pro`) |
...
1. **Each IID runs in its own SHARED linked git worktree at `${WORKTREE_DIR}=${WORKTREES_ROOT}/issue-${ISSUE_IID}/`** (no `-att-<NNN>` suffix). `prepare_attempt.sh` creates it on attempt 1 via `git worktree add -B ${LOCAL_ATTEMPT_BRANCH} ${WORKTREE_DIR} ${BASE_REF}`; on attempt N>1 it force-switches the checked-out branch via `git -C ${WORKTREE_DIR} checkout -B ${LOCAL_ATTEMPT_BRANCH} ${BASE_REF} --force`. `BASE_REF` is always `origin/${DEV_BRANCH}` for a clean reset (continue / resume is disabled on benchmark-test, so the work-branch resume base is never used). After every base checkout, `prepare_attempt.sh` refreshes tracked shared config paths (`.claude/`, `hulat/`, and `${DATA_BASENAME}/`) from the latest just-fetched `origin/${DEV_BRANCH}`, so every run uses current test-team runner/materials config. Before an in-place branch switch, `prepare_attempt.sh` snapshots `${RESULT_BASENAME}/issue-${ISSUE_IID}/` from the shared worktree. Every (fresh) run archives the snapshot and then quarantines any active same-IID runtime subtree that survived checkout, so the files are not deleted but are not visible to the reset run. The parent checkout at `${REPO_PATH}` is never mutated by an attempt — only `git fetch` runs against it (under `${RESULT_ROOT}/_dispatcher/locks/repo.lock`). Same-IID attempts never run concurrently (single-batch invariant), so the shared per-issue worktree is safe; cross-IID parallelism stays safe because different IIDs use different worktree paths. `max_concurrent_subagents` has a hard lower bound of `1`; values below 1 abort the tick. When `ui_accounts_relpath` is configured (no default — trigger field, carry-forward persisted; the relpath is resolved under `${REPO_PATH}`, NOT under `${REPO_PATH}/${DATA_BASENAME}/`), `max_concurrent_subagents` is ALSO bounded above by the UI account pool size read from `${REPO_PATH}/${UI_ACCOUNTS_RELPATH}`, and values above the pool abort the tick. When `ui_accounts_relpath` is unconfigured, the pool load is skipped and only the lower bound applies.
2. **Agent runtime files live under `${RESULT_ROOT}` (= `${REPO_PATH}/${RESULT_BASENAME}/`).** The dispatcher subtree (`_dispatcher/...`), per-issue cross-attempt subtree (`issue-<iid>/state.json`, `issue-<iid>/attempt_state.json`, `issue-<iid>/summary.md`), and `.worktrees/` are runtime/audit data. They are kept out of normal `git add -A` by `clone_or_pull.sh`'s entry in `.git/info/exclude` (which excludes the entire `${RESULT_BASENAME}/` subtree, including `.worktrees/`), but no script will refuse to push them if they make it into the diff some other way (e.g. tracked on the base branch). Inside the per-issue worktree the same exclude also covers `${LOG_DIR}` (under `${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN-<tier>/`); `stage_and_guard.sh` force-adds the current issue's `${OUTPUT_DIR}` plus the **entire** `${LOG_DIR}` so they survive the exclude and are pushed to `${LOCAL_ATTEMPT_BRANCH}` (eval full archival: `prompt.txt`, `claude_result.txt`, `acpx_raw.log`, `git_status.txt`, `git_diff.patch`, `acpx_command.txt`, `timing.txt`, `metrics.json`, etc.). Only the `wiki_*.md` / `.jsonl` files, created after the push, stay locally ignored and disappear with the worktree.
3. **`hulat/`, `.claude/`, and `${DATA_BASENAME}/` are shared repository content.** They are committed to master + dev by the test team. `prepare_attempt.sh` refreshes their tracked contents from the latest `origin/${DEV_BRANCH}` before every acpx run, so the agent does NOT symlink `hulat/` and does NOT copy `.claude/`. They may be changed when an issue genuinely requires it; avoid unrelated edits.
4. **Per-issue spec output goes to `${OUTPUT_DIR}` only.** `build_prompt.sh` injects this rule into the Claude Code prompt. Per-issue output never collides because each issue writes into a distinct `${RESULT_BASENAME}/issue-<iid>/hulat-spec-issue<iid>/` subdirectory (relative path inside the worktree).
5. There is no `${ISSUE_ROOT}/attempts/` directory. Each attempt recreates only its own `${LOG_DIR}` (`${WORKTREE_DIR}/${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN-<tier>/`), overwrites `${ATTEMPT_STATE_FILE}`, and updates `${SUMMARY_FILE}`. `LOG_DIR` lives INSIDE the per-issue worktree; the whole `${LOG_DIR}` (all artifacts present at staging time) survives on the per-attempt remote branch because `stage_and_guard.sh` force-adds it. Cross-attempt audit is preserved across the per-attempt remote branches; locally, every (fresh) run archives the prior snapshot and active same-IID runtime residue under `.preserved-attempts` before recreating empty current output/log directories. Historical summaries are preserved as GitLab issue notes.
6. Per-attempt immutable branches: each attempt pushes exactly ONE remote branch — the immutable per-attempt branch `${LOCAL_ATTEMPT_BRANCH}` (`issue/<iid>-auto-fix-att<NNN>-<tier>`), pushed once from inside the shared per-issue worktree and never overwritten, so every pinned model run is preserved for benchmarking. The legacy mutable pointer `${WORK_BRANCH}` is NO LONGER pushed; it survives only as the naming prefix `${LOCAL_ATTEMPT_BRANCH}` derives from.
7. Claude Code is invoked only through `scripts/run_acpx_attempt.sh`. That script `cd`s into `${WORKTREE_DIR}` (the shared per-issue worktree for this IID, NOT the parent checkout) and runs `acpx --auth-policy skip claude exec -f "${LOG_DIR}/prompt.txt"`, sets `TASK_OUTPUT_DIR=${OUTPUT_DIR}`, and writes stdout/stderr to `${LOG_DIR}/claude_result.txt` / `${LOG_DIR}/acpx_raw.log`. Current acpx releases expose `claude exec` as a one-shot command with no saved-session flag, so attempts of the same IID do not share Claude-Code session memory at the acpx level. Every attempt runs fresh from `origin/${DEV_BRANCH}` (continue / resume is disabled), so there is no cross-attempt continuity beyond the rendered prompt and the refreshed shared config paths. The `-f` flag points to `${LOG_DIR}/prompt.txt` via absolute path.
8. Two-branch model. `${BRANCH}` (typically `master`) is the **integration / target** branch. `${DEV_BRANCH}` (typically `dev`) is the **clean baseline** — every attempt resets to `origin/${DEV_BRANCH}` so Claude does NOT see past issues' spec output (continue / resume is disabled, so the work-branch resume base is never used). `prepare_attempt.sh` refreshes `.claude/`, `hulat/`, and `${DATA_BASENAME}/` from latest `origin/${DEV_BRANCH}` before running.
9. **No path-based leak rejection.** `stage_and_guard.sh` and `post_push_verify.sh` no longer reject anything by path. They still serve their bookkeeping roles — staging, NO_CHANGES detection, evidence files, post-push fetch — but every file present in the staged or pushed diff goes through. `.git/info/exclude` (set by `clone_or_pull.sh`) keeps untracked `${RESULT_BASENAME}/` runtime files out of `git add -A`, and the current issue's `${OUTPUT_DIR}` is force-added so it survives that exclude.
...
`${LOG_DIR}` lives INSIDE the shared per-issue worktree at `${WORKTREE_DIR}/${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN-<tier>/` (still attempt-scoped so attempts don't overwrite each other). By the end of each Claude run, these MUST exist:

- `prompt.txt`           — the prompt fed to `acpx claude exec -f` (**force-added by `stage_and_guard.sh`; pushed to `${LOCAL_ATTEMPT_BRANCH}`**)
- `claude_result.txt`    — stdout from acpx (**force-added by `stage_and_guard.sh`; pushed to `${LOCAL_ATTEMPT_BRANCH}`**)
- `acpx_raw.log`         — stderr from acpx (**force-added by `stage_and_guard.sh`; pushed to `${LOCAL_ATTEMPT_BRANCH}`**)
- `git_status.txt`       — `git status --porcelain` after the Claude run (**force-added by `stage_and_guard.sh`; pushed to `${LOCAL_ATTEMPT_BRANCH}`**)
- `git_diff.patch`       — `git diff` after the Claude run (**force-added by `stage_and_guard.sh`; pushed to `${LOCAL_ATTEMPT_BRANCH}`**)

After post-push verification succeeds, these are auto-created (all locally ignored; removed with the worktree):
...
The whole `${LOG_DIR}` present at staging time (`prompt.txt`, `claude_result.txt`, `acpx_raw.log`, `git_status.txt`, `git_diff.patch`, `acpx_command.txt`, `timing.txt`, `metrics.json`, etc.) is force-added and ends up on the immutable per-attempt branch `${LOCAL_ATTEMPT_BRANCH}`; only the post-push `wiki_*` files stay local and are discarded with the worktree by housekeeping.
