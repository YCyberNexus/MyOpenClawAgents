# acpx_auto_tester_pts 使用文档

本文档面向调度器配置、运维和项目使用者。它说明如何把 trigger prompt 传给
`acpx_auto_tester_pts`，以及 agent 从收到 trigger 到创建 MR、处理回调的完整流程。
...
`acpx_auto_tester_pts` 是一个非交互式 GitLab issue 自动化 agent，用来周期性扫描一批
...
```text
agent:acpx_auto_tester_pts:main
```
