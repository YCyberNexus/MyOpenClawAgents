...
| 4 Per-IID Prep | for each IID in the batch: `allocate_attempt.sh` → load that IID's UI account slot from `${REPO_PATH}/${UI_ACCOUNTS_RELPATH}` (no default; opt in via trigger `ui_accounts_relpath`, carry-forward persisted; relpath resolved under the project checkout root, NOT under `${REPO_PATH}/${DATA_BASENAME}/`; when unconfigured the entire UI-account flow is skipped and the rendered Claude Code prompt omits its `# UI test accounts` section) → `prepare_attempt.sh` (creates the shared per-issue linked worktree at `${REPO_PATH}/${RESULT_BASENAME}/.worktrees/issue-<iid>/` via `git worktree add -B` on attempt 1, or in-place force-switches the checked-out branch to BASE_REF on attempt N>1 after preserving same-IID `${RESULT_BASENAME}/issue-<iid>/` files; only `continue` restores those files into the active worktree, while every non-continue entry label resets from the clean baseline; tracked shared config paths `hulat/`, `.claude/`, and `ifp-data/` are refreshed from latest `origin/${DEV_BRANCH}` before every run) → **model-settings copy** (only when trigger `model_settings_dir` is configured; runs after `prepare_attempt.sh` and before `build_prompt.sh`: copies `${model_settings_dir}/${MODEL}-settings.json` → `${WORKTREE_DIR}/.claude/settings.json` then `git update-index --skip-worktree .claude/settings.json`, where `MODEL` is the tier resolved by `resolve_model_tier`; a missing/unreadable tier file marks that IID `blocked-dispatcher`; unconfigured → step skipped and the worktree's committed `.claude/settings.json` is used as-is) → reads issue title/url/labels/body via `glab` → `resolve_model_tier` evaluates the per-issue model tier from PRIOR live labels + on-disk `continue_count` (see §Model-tier dimension) and `set_issue_label.sh` transitions the v2 workflow group (`todo` / `retry` / `new` / `continue` / `blocked-cc` / `blocked-dispatcher` / `timeout` / `failed-cc` / `failed-dispatcher` / `done` / `pr` plus matched trigger labels) to `doing`, then stamps the resolved `model:<tier>` (monotonic; replaces any other `model:*`) and consumes `quality:low` if it drove an upgrade — the `model:<tier>` and `quality:low` dimensions are DELIBERATELY excluded from this "进 doing 清除集" → `build_prompt.sh` (writes `${LOG_DIR}/prompt.txt` with UI account + resolved `MODEL` injected) → initializes `${ISSUE_STATE_FILE}` and `${ATTEMPT_STATE_FILE}` (status=in_progress) → writes `pending_subagents[iid]` placeholder → renders [`references/executor_prompt.md`](workspace-acpx_auto_tester/skills/gitlab_issue_campaign_dispatcher/references/executor_prompt.md) with per-IID values |
...
1. SETUP: call `bash ${SCRIPTS_DIR}/run_acpx_attempt.sh` with the standard per-issue env. The script `cd`s into `${WORKTREE_DIR}` (the shared per-issue linked worktree for this IID) and owns the fixed `acpx --auth-policy skip claude exec -f ${LOG_DIR}/prompt.txt` invocation. Current acpx releases expose `claude exec` as a one-shot command with no saved-session flag, so attempts of the same IID do NOT share Claude-Code session memory. Cross-attempt continuity for `continue` comes from four layers: (a) the self-contained rendered prompt (incl. prior attempt summaries auto-posted as GitLab notes and reviewer comments); (b) the committed `origin/${WORK_BRANCH}` contents or latest local prior-attempt branch that continue/resume attempts check out; (c) tracked shared config paths refreshed from latest `origin/${DEV_BRANCH}`; and (d) the shared per-issue worktree's `${RESULT_BASENAME}/issue-<iid>/` subtree, which `prepare_attempt.sh` snapshots and restores around in-place branch switches. Non-continue entry labels (`todo`, `retry`, `new`, `blocked`, trigger require_labels) are reset runs: the prior subtree is archived, not restored into the active worktree. See `workspace-acpx_auto_tester/skills/gitlab_issue_campaign_dispatcher/references/executor_prompt.md` and `references/continue_mode.md` for the externalized continuity contract.
...
On any subagent FAIL path, remove `doing` and add `blocked-cc` (subagent failures are always CC-side; dispatcher-side prep/spawn/eviction failures instead get `blocked-dispatcher` synthesized by the orchestrator) before summarizing and returning the compact JSON. For non-timeout acpx failures, the subagent first tries `stage_and_guard.sh` → `commit_and_push.sh` → `post_push_verify.sh` so committable partial work reaches `${WORK_BRANCH}` even though the issue remains `blocked-cc` and no Wiki/MR/`pr` is produced. Phase 6 re-applies the final label state idempotently when the callback arrives.
...
Key reconciled signals (produced by `reconcile.sh`): `is_closed_on_gitlab` (closed = hard terminal skip, never schedule), `has_pr` / `is_done_on_gitlab` (in v2 `pr` REPLACES `done` — they never coexist; `has_pr` present + not `needs_continue` = completed), the per-side failure signals `has_blocked_cc` / `has_blocked_dispatcher` / `has_failed_cc` / `has_failed_dispatcher` / `has_timeout`, the current `model_tier` (read from the live `model:<tier>` label so the disk `state.json.model_tier` cache can be corrected toward it), `needs_continue` (opened + has `continue` or legacy `contiune` = reviewer wants resume; wins over cached done state), `user_reopened` (opened + missing `pr` + none of the `blocked-*` / `failed-*` / `timeout` / `continue` / `contiune` variants).
...
- `dev_branch` (typically `dev`) — **clean baseline and shared config source**. Fresh-mode attempts reset their per-issue worktree to `origin/${dev_branch}` so Claude Code does not see past issues' spec accumulation on tracked files. Every attempt, including continue, refreshes tracked `hulat/`, `.claude/`, and `${DATA_BASENAME}/` from latest `origin/${dev_branch}` before acpx runs. Untracked scratch left in the shared worktree by a prior attempt is NOT touched by the reset (that is intentional — `acpx claude exec` resumes from it). Set `dev_branch=<same-as-branch>` to disable.
...
- Continue mode resets the per-issue worktree to `origin/${WORK_BRANCH}` (the resumable WIP branch), not `${dev_branch}`, then refreshes `hulat/`, `.claude/`, and `${DATA_BASENAME}/` from latest `origin/${dev_branch}`. If `origin/${WORK_BRANCH}` is missing, `prepare_attempt.sh` downgrades to fresh mode and records `mode_downgraded_from="continue"` — the only documented exception to the no-fallback policy.
...
**Per-side attribution of the failure labels.** The single `blocked` label is split by cause into `blocked-cc` (Claude-Code side: acpx non-timeout failure / `NO_CHANGES` / push rejected / acpx post-step failure — committable partial work is still force-pushed) and `blocked-dispatcher` (dispatcher side: prep / spawn / scope or stuck eviction — no CC output). `failed` is split the same way into `failed-cc` / `failed-dispatcher`, and over-limit promotion stays on the same side (`blocked-cc → failed-cc`, `blocked-dispatcher → failed-dispatcher` when `retry_count > blocked_retry_limit`). `timeout` is its own terminal **parking** label: acpx wall-clock overrun, partial work pushed, no MR/`pr`, NOT auto-retried, it does NOT consume `retry_count`, and it is never auto-promoted to `failed`. `pr` **REPLACES** `done` (they never coexist; the only allowed transient pair is `done` + `blocked-cc` / `done` + `blocked-dispatcher`). The "进 doing 清除集" the dispatcher strips on the `doing` transition is exactly the v2 workflow group `{ todo, new, retry, continue, contiune, done, pr, blocked-cc, blocked-dispatcher, timeout, failed-cc, failed-dispatcher }` (plus matched trigger `require_labels`) — it does NOT contain `model:<tier>` or `quality:low`. The non-workflow `precheck-failed` marker is also stripped on the `doing` transition (it is in the dispatcher's `REMOVE_LBLS` set) even though it is not part of the workflow group.

**`model:<tier>` — an orthogonal, persistent, monotonic dimension.** A second label dimension `model:flash` (TIER_0, default) → `model:pro` (TIER_1) → `model:max` (TIER_2, cap) tracks which model the issue runs on. The tiers come from the ordered trigger field `model_tiers` (default `flash,pro,max`; the label for tier *k* is `model:<element-k>`). It is orthogonal to and coexists with the workflow label, and is internally mutually exclusive (adding one `model:*` removes the others in the same GitLab update, touching no workflow label and no `quality:low`). It is **per-issue monotonic — only ever raised, never downgraded — and follows the issue for life until `CLOSED`**, which is precisely why it is excluded from the "进 doing 清除集". A new issue with no `model:<tier>` is treated as TIER_0; the first Phase 4 PREPARE explicitly stamps `model:flash`. GitLab labels are the source of truth; `state.json.model_tier` is only a cache that `reconcile` corrects toward the live label.

**`resolve_model_tier` (Phase 4 of `dispatch_prepare_tick.sh`).** Each PREPARE re-evaluates the tier from the issue's PRIOR live labels (read from the reconcile evidence, captured before the `doing` transition wipes them) plus the on-disk `continue_count`, and resolves `MODEL` (`flash` / `pro` / `max`). `UPGRADE? = hard ∪ soft`, restricted to the CC side:
- **hard**: prior outcome ∈ `{ blocked-cc, timeout, failed-cc }`.
- **soft** (any hit): `quality:low` present ∨ `continue_count ≥ model_upgrade_continue_threshold` (trigger field N; `0` disables) ∨ auto-score < threshold (black-box — NOT implemented this version, only a commented hook).
- **excluded**: `blocked-dispatcher` / `failed-dispatcher` (infrastructure side — a bigger model would not help), so dispatcher-side failures NEVER upgrade.

Hit and not capped → +1 tier; hit at the cap → hold `max`; no hit → hold the current tier. One-liner: **CC-side `{ blocked-cc, timeout, failed-cc }` re-runs upgrade one tier; dispatcher-side `{ blocked-dispatcher, failed-dispatcher }` do not.**

**`model_settings_dir` — making the tier actually change acpx's model (new on this branch).** Resolving a tier only relabels the issue; to change the *underlying* model, Phase 4 — after `prepare_attempt.sh`, before `build_prompt.sh` — copies (renames) `${model_settings_dir}/${MODEL}-settings.json` to `${WORKTREE_DIR}/.claude/settings.json` and runs `git update-index --skip-worktree .claude/settings.json` (per-worktree index → concurrent attempts and issue MRs unaffected) whenever the trigger configures the absolute `model_settings_dir` (**per-tick, NOT carry-forward** — unlike `model_tiers` / `ui_accounts_relpath`, omitting it on any tick reverts that tick to legacy behavior; the value is not restored from persisted state, only snapshotted into `campaign_state.json` for same-batch callback consistency). `acpx claude exec` then reads that file and runs on the tier's model. When `model_settings_dir` is unconfigured the copy is skipped and the worktree's committed `.claude/settings.json` is used as-is (the tier is then only a prompt-text hint, not a real model switch). A configured directory whose `${MODEL}-settings.json` is missing or unreadable marks **that IID** `blocked-dispatcher` (strict no-fallback — no downgrade to a default tier file). This field **supersedes the removed `claude_settings_path`** single fixed-file override; to reproduce the old behavior, populate every tier file with identical content.

**Tier auto-discovery (full/effective split).** When `model_settings_dir` is configured, `model_tiers` (default `flash,pro,max`) is the ordered *wisdom superset* and the actual per-deployment upgrade ladder is the **effective** subset whose `<tier>-settings.json` exists in that directory — order preserved, re-derived each tick by `derive_effective_model_tiers` in `_dispatch_lib.sh`. So shipping only `pro-settings.json` + `max-settings.json` yields the ladder `pro→max` (start pro, upgrade to max) even though `model_tiers` still lists `flash`; only `flash-settings.json` + `max-settings.json` yields `flash→max`; a single file means a single non-upgrading tier. `reconcile.sh` (integer `model_tier` index) and `resolve_model_tier` (ladder + `MODEL` selection) consume the **effective** subset; `ensure_labels.sh` (creates every `model:<tier>` label) and `set_issue_label.sh` (model:* mutual-exclusion clear-set) keep using the **full** list so labels for tiers outside the current subset can still be created and cleared (migration safety — e.g. clearing a leftover `model:flash` when migrating to a pro+max deployment). The same effective derivation runs on the callback path (`dispatch_followup.sh`) so the narrow reconcile's integer index never drifts from the prepare tick. A configured `model_settings_dir` containing none of the tiers' `<tier>-settings.json` aborts the tick (`no_model_settings_files`). Auto-discovery assumes the tier-file set is fixed at first configuration; adding/removing tier files mid-deployment can desync an existing issue's cached `model:<tier>` (see `statemachine.v2.md`).

**`quality:low` — a one-shot soft signal.** A human adds it in `AWAITING_REVIEW` (the `pr` state) to mark a mediocre round. It is one of the soft upgrade triggers and is **consumed (removed) by `resolve_model_tier`** on any PREPARE where it is present — including when the tier is already capped and no upgrade is possible — so it can never linger as permanent noise.
...
- v2 state-machine semantics (per-side `blocked` / `failed` split, `pr` replaces `done`, `timeout` parking, the `model:<tier>` dimension, `resolve_model_tier`, `model_settings_dir`, `quality:low`): [`statemachine.v2.md`](statemachine.v2.md) and `references/label_lifecycle.md`; the `model_settings_dir` / `model_tiers` / `model_upgrade_continue_threshold` trigger fields are specified in `references/trigger_command.md`.
