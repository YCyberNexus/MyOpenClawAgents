...
1. SETUP: call `bash ${SCRIPTS_DIR}/run_acpx_attempt.sh` with the standard per-issue env. The script owns the fixed `acpx --auth-policy skip claude exec -s issue-<iid> -f ${LOG_DIR}/prompt.txt` invocation from `${REPO_PATH}` (the parent checkout, so all attempts share the same project identity and session store; session-persisted; on retry after interruption, the session resumes).
...
- The Claude Code invocation contract is `bash ${SCRIPTS_DIR}/run_acpx_attempt.sh` with the standard per-issue env. That script is the only place that constructs the `acpx --auth-policy skip claude exec -s issue-<iid> -f "${LOG_DIR}/prompt.txt"` command, and it runs from `cwd=${REPO_PATH}` (the parent checkout, NOT the per-attempt worktree), session-persisted. Invoking from `${REPO_PATH}` ensures all attempts share the same project identity for session storage — per-attempt worktrees have different paths and would isolate sessions from each other. Start the script with PTY on the first attempt, use a timeout that covers the expected run, and poll the same process if the exec tool yields a session. Do not re-run the script after a tool timeout. The `-s issue-<iid>` persists the conversation across attempts — if interrupted, the next attempt resumes the session with a short continue prompt. No direct `acpx` call from the subagent, no `--no-wait`, no streaming mode, no calling `claude` directly without acpx, no other LLM as substitute.
