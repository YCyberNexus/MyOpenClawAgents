...
Full `sessions_spawn` shape (anonymous, `task=<rendered prompt>`, `label=`, `runtime="subagent"`, `mode="run"`, `cleanup="keep"`, `context="isolated"`, serial-only, validation of launch ack, 3-attempt in-tick launch retry, stuck-pending eviction backstop) is documented in [`skills/gitlab_issue_campaign_dispatcher/SKILL.md`](skills/gitlab_issue_campaign_dispatcher/SKILL.md) §The orchestrator loop and §No-Fallback. The hard prohibitions on session-name parameters are §Subagent Concurrency Policy hard rule 5 above. OpenClaw 的全局 subagent 运行上限由部署配置控制；若配置为正值，应至少为 `acpx_timeout_seconds + 120`，不能作为 trigger 字段或单次工具参数传入。
