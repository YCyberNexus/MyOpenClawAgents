...
The repo follows a **two-branch model**: a clean baseline branch (typically `dev`, passed as `dev_branch=`) from which fresh attempts are checked out, and an integration branch (typically `master`, passed as `branch=`) that accumulates spec output via merge requests. Claude Code runs at the main repo root `${REPO_PATH}` (default `/data/${PROJECT}`; trigger `repo_path` overrides the parent, so `repo_path=/data/ifp1` gives `/data/ifp1/${PROJECT}`). Each issue's spec output is required to live under `ifp-result/issue-<iid>/hulat-spec-issue<iid>/`, so MRs into `master` never collide on file paths.
