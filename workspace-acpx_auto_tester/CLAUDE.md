...
1. SETUP: call `bash ${SCRIPTS_DIR}/run_acpx_attempt.sh` with the standard per-issue env. The script `cd`s into `${WORKTREE_DIR}` (the shared per-issue linked worktree for this IID) and owns the fixed `acpx --auth-policy skip claude exec -f ${LOG_DIR}/prompt.txt` invocation. Current acpx releases expose `claude exec` as a one-shot command with no saved-session flag, so attempts of the same IID do NOT share Claude-Code session memory. Cross-attempt continuity for `continue` comes from four layers: (a) the self-contained rendered prompt, including every non-system issue comment and any historical summary notes left by older deployments; (b) the committed `origin/${WORK_BRANCH}` contents or latest local prior-attempt branch that continue/resume attempts check out; (c) tracked shared config paths refreshed from latest `origin/${DEV_BRANCH}`; and (d) the shared per-issue worktree's `${RESULT_BASENAME}/issue-<iid>/` subtree, which `prepare_attempt.sh` snapshots and restores around in-place branch switches. Non-continue entry labels (`todo`, `retry`, `new`, `blocked-cc`, `blocked-dispatcher`, `blocked` [legacy compat], trigger require_labels) are reset runs: the prior subtree is archived, not restored into the active worktree. See `workspace-acpx_auto_tester/skills/gitlab_issue_campaign_dispatcher/references/executor_prompt.md` and `references/continue_mode.md` for the externalized continuity contract.
...
9. `summarize_attempt.sh` writes a local per-attempt summary and never posts it as a GitLab issue note.
