...
5. **Subagent:** Runs Claude Code via the same script-owned session-persisted invocation used in all modes: `scripts/run_acpx_attempt.sh` runs `acpx --auth-policy skip claude exec -s issue-<iid> -f "${LOG_DIR}/prompt.txt"` from `${REPO_PATH}` (the parent checkout, for stable session storage; the prompt tells Claude Code to work in `${WORKTREE_DIR}`). Same No-Fallback Policy applies. Cross-attempt continuity comes from the persistent Claude session (the `-s` flag), supplemented by the prompt's "Past attempt summaries" section for reviewer-continue context.
