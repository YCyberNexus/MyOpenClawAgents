...
5. **Subagent:** Runs Claude Code via the same session-persisted invocation used in all modes: `acpx --auth-policy skip claude exec -s issue-<iid> -f "${LOG_DIR}/prompt.txt"` from `${WORKTREE_DIR}`. Same No-Fallback Policy applies. Cross-attempt continuity comes from the persistent Claude session (the `-s` flag), supplemented by the prompt's "Past attempt summaries" section for reviewer-continue context.
