...
```
   ...
   doing ──► timeout   (acpx exceeded wall-clock cap; partial work force-pushed to ${WORK_BRANCH};
                        NO MR; terminal until a human strips the label)
...
| `timeout`  | `doing`    | dispatcher | a human reviewer stripped the `timeout` label so the regular reconcile flow re-enqueued the IID (or applied `continue` / `retry`) — the dispatcher then treats it like any other unfinished entry | normal `*` → `doing` transition above |
...
9. **`timeout` is never auto-retried.** Unlike `blocked`, a `timeout` IID stays in `timeout_iids` until a human reviewer strips the label. Reviewers strip `timeout` to re-enqueue the IID via the regular `user_reopened` path, or apply `continue` to resume from the existing `${WORK_BRANCH}` (the partial work is already pushed there). The agent does NOT promote `timeout` to `failed`; `retry_count` is NOT consumed.
