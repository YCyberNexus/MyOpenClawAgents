    ...
    ifp-result/                                          ← ${RESULT_ROOT}; agent runtime root
        ...
        .worktrees/                                      ← ${WORKTREES_ROOT}; per-issue linked worktrees (one per IID)
            issue-<iid>/                                 ← ${WORKTREE_DIR}; acpx cwd; created on attempt 1 via
                ...
                ${RESULT_BASENAME}/issue-<iid>/hulat-spec-issue<iid>/   ← ${OUTPUT_DIR}; Claude Code output (committed; force-pushed to ${WORK_BRANCH}; shared across attempts of this IID)
                ${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN/         ← ${LOG_DIR}; STILL attempt-scoped inside the shared worktree
                    prompt.txt                                          ← force-added by stage_and_guard.sh (force-pushed to ${WORK_BRANCH})
                    claude_result.txt                                   ← force-added by stage_and_guard.sh (force-pushed to ${WORK_BRANCH})
...
2. **Agent runtime files live under `${RESULT_ROOT}` (= `${REPO_PATH}/${RESULT_BASENAME}/`).** The dispatcher subtree (`_dispatcher/...`), per-issue cross-attempt subtree (`issue-<iid>/state.json`, `issue-<iid>/attempt_state.json`, `issue-<iid>/summary.md`), and `.worktrees/` are runtime/audit data. They are kept out of normal `git add -A` by `clone_or_pull.sh`'s entry in `.git/info/exclude` (which excludes the entire `${RESULT_BASENAME}/` subtree, including `.worktrees/`), but no script will refuse to push them if they make it into the diff some other way (e.g. tracked on the base branch). Inside the per-issue worktree the same exclude also covers `${LOG_DIR}` (under `${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN/`); `stage_and_guard.sh` force-adds the current issue's `${OUTPUT_DIR}` plus `${LOG_DIR}/prompt.txt` and `${LOG_DIR}/claude_result.txt` so those three paths survive the exclude and are force-pushed to `${WORK_BRANCH}`. The remaining log files (acpx_raw.log, git_status.txt, git_diff.patch, wiki_*.md / .jsonl) stay locally ignored and disappear with the worktree.
...
5. There is no `${ISSUE_ROOT}/attempts/` directory. Each attempt recreates only its own `${LOG_DIR}` (`${WORKTREE_DIR}/${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN/`), overwrites `${ATTEMPT_STATE_FILE}`, and updates `${SUMMARY_FILE}`. `LOG_DIR` lives INSIDE the per-issue worktree; `prompt.txt` and `claude_result.txt` survive on the work branch because `stage_and_guard.sh` force-adds them. Cross-attempt audit is preserved on the work branch; locally, every (fresh) run archives the prior snapshot and active same-IID runtime residue under `.preserved-attempts` before recreating empty current output/log directories. Historical summaries are preserved as GitLab issue notes.
...
- `prompt.txt`           — the prompt fed to `acpx claude exec -f` (**force-added by `stage_and_guard.sh`; force-pushed to `${WORK_BRANCH}`**)
- `claude_result.txt`    — stdout from acpx (**force-added by `stage_and_guard.sh`; force-pushed to `${WORK_BRANCH}`**)
