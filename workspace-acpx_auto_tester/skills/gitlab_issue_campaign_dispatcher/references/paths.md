...
7. Claude Code is invoked with `acpx --auth-policy skip claude exec -s issue-<iid> -f "${LOG_DIR}/prompt.txt"` from inside `${WORKTREE_DIR}` (the per-attempt worktree, NOT the parent checkout). The `-s issue-<iid>` flag persists the Claude Code conversation across attempts — if the run is interrupted (API error, timeout, etc.), the next attempt resumes the same session with a short "continue from where you left off" prompt. The `-f` flag keeps the invocation non-interactive (Claude Code processes the file and exits; the session is saved to disk on exit). Cross-attempt continuity for reviewer-continue mode still additionally comes from the prompt itself (past attempt summaries + reviewer comments).
