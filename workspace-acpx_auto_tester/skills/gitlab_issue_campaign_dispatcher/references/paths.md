...
7. Claude Code is invoked only through `scripts/run_acpx_attempt.sh`. That script runs `acpx --auth-policy skip claude exec -s issue-<iid> -f "${LOG_DIR}/prompt.txt"` from inside `${REPO_PATH}` (the parent checkout, NOT the per-attempt worktree), sets `TASK_OUTPUT_DIR=${OUTPUT_DIR}`, and writes stdout/stderr to `${LOG_DIR}/claude_result.txt` / `${LOG_DIR}/acpx_raw.log`. The `-s issue-<iid>` flag persists the Claude Code conversation across attempts — if the run is interrupted (API error, timeout, etc.), the next attempt resumes the same session with a short "continue from where you left off" prompt. **acpx MUST be invoked from `${REPO_PATH}`** because Claude Code derives its project identity (used for session storage at `~/.claude/projects/<hash>/`) from the invocation directory. Per-attempt worktrees have different paths (`.../issue-<iid>-att-001/` vs `.../issue-<iid>-att-002/`), which would produce different project hashes and isolate sessions from each other. The parent checkout path is stable across all attempts, so all attempts share the same project hash and the same session store. The `-f` flag points to `${LOG_DIR}/prompt.txt` via absolute path; the prompt tells Claude Code to work in `${WORKTREE_DIR}`. Cross-attempt continuity for reviewer-continue mode still additionally comes from the prompt itself (past attempt summaries + reviewer comments).
