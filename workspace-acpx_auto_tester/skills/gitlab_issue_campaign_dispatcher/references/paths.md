    ...
    ifp-result/                                          ← ${RESULT_ROOT}; agent runtime root
        ...
        .worktrees/                                      ← ${WORKTREES_ROOT}; per-issue linked worktrees (one per IID)
            issue-<iid>/                                 ← ${WORKTREE_DIR}; acpx cwd; created on attempt 1 via
                                                            ...
                                                            (tracked files reset to BASE_REF; untracked files survive)
                .claude/ hulat/ ${DATA_BASENAME}/        (from base branch checkout; reset to BASE_REF every attempt)
...
3. **`hulat/`, `.claude/`, and `${DATA_BASENAME}/` are shared repository content.** They are committed to master + dev by the test team. The per-issue worktree's checkout already contains them (because they live on the base branch) — the agent does NOT symlink `hulat/` and does NOT copy `.claude/`. They may be changed when an issue genuinely requires it; avoid unrelated edits.
...
5. There is no `${ISSUE_ROOT}/attempts/` directory. Each attempt recreates only its own `${LOG_DIR}` (`${WORKTREE_DIR}/${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN/`), overwrites `${ATTEMPT_STATE_FILE}`, and updates `${SUMMARY_FILE}`. `LOG_DIR` lives INSIDE the per-issue worktree, so its contents disappear when housekeeping removes the worktree; `prompt.txt` and `claude_result.txt` survive in the work-branch / MR history because `stage_and_guard.sh` force-adds them. Cross-attempt history is also preserved on the work branch itself: a continue-mode attempt resets the per-issue worktree to `origin/${WORK_BRANCH}` so prior attempts' `log/attempt-001/`, `log/attempt-002/`, ... appear at their original paths after the reset. Historical summaries are preserved as GitLab issue notes.
