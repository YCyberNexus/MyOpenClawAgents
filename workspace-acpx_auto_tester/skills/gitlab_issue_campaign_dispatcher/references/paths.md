...
1. **Each IID runs in its own SHARED linked git worktree at `${WORKTREE_DIR}=${WORKTREES_ROOT}/issue-${ISSUE_IID}/`** (no `-att-<NNN>` suffix). `prepare_attempt.sh` creates it on attempt 1 via `git worktree add -B ${LOCAL_ATTEMPT_BRANCH} ${WORKTREE_DIR} ${BASE_REF}`; on attempt N>1 it force-switches the checked-out branch via `git -C ${WORKTREE_DIR} checkout -B ${LOCAL_ATTEMPT_BRANCH} ${BASE_REF} --force`. `BASE_REF` is `origin/${WORK_BRANCH}` or the latest local prior-attempt branch only for live `continue` / `contiune` labels; every non-continue entry path intentionally uses `origin/${DEV_BRANCH}` for a clean reset. Before an in-place branch switch, `prepare_attempt.sh` snapshots `${RESULT_BASENAME}/issue-${ISSUE_IID}/` from the shared worktree. Continue mode restores that snapshot after checkout so prior same-IID output/log files are visible; fresh reset mode archives the snapshot and then quarantines any active same-IID runtime subtree that survived checkout, so the files are not deleted but are not visible to the reset run. The parent checkout at `${REPO_PATH}` is never mutated by an attempt — only `git fetch` runs against it (under `${RESULT_ROOT}/_dispatcher/locks/repo.lock`). Same-IID attempts never run concurrently (single-batch invariant), so the shared per-issue worktree is safe; cross-IID parallelism stays safe because different IIDs use different worktree paths. `max_concurrent_subagents` is bounded above by the UI account pool size read from `${REPO_PATH}/${UI_ACCOUNTS_RELPATH}` (default relpath `ifp-data/ifp-common/ifp_users.json`; override with trigger field `ui_accounts_relpath`, carry-forward persisted; the relpath is resolved under the project checkout root, NOT under `${REPO_PATH}/${DATA_BASENAME}/`); values below 1 or above the pool abort the tick.
