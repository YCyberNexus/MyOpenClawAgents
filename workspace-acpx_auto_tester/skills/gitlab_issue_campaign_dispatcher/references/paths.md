...
9. **Leak surface.** The leak guards (`stage_and_guard.sh`, `post_push_verify.sh`) reject protected runtime paths in the staged/MR diff: `ifp-result/_dispatcher/` and any `ifp-result/issue-*` path except the current `${OUTPUT_DIR}`.
