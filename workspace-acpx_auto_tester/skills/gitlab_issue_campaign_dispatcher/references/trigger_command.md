...
| `accounts_per_issue`       | Integer ≥ 1, defaults to `10`. Number of UI accounts allocated per subagent (equals max robot files per issue). Each robot file execution needs a distinct account. `max_concurrent_subagents * accounts_per_issue` must not exceed the UI account pool size; pool-too-small is a tick-level failure. |
