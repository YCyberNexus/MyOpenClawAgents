...
| `kill_subagent_on_done`     | Boolean, defaults to `true` when omitted. When true, Phase 6 step 9 calls the runtime-side `subagents` tool with `action="kill"` and `target=<child_session_key>` after a `done` callback drains, releasing the completed subagent's runtime session and transcript-store entry so the OpenClaw runtime does not bloat with completed sessions over a long campaign. Only `done` IIDs are killed — `blocked` and `failed` are intentionally left alive so operators can replay them via `sessions_history` for postmortem. Cleanup is best-effort: failure of `subagents kill` does NOT mutate state files or re-classify the IID, only adds `cleanup_status` to the chat summary. Set to `false` during debug windows when you need every completed subagent's transcript preserved on the runtime. Truthy values (case-insensitive): `true`, `1`, `yes`. Falsy: `false`, `0`, `no`. Any other value aborts the tick with `"invalid_kill_subagent_on_done"`. |
...
`kill_subagent_on_done` (when supplied) overrides the persisted value with the same "trigger-wins each tick + persist the post-override value" rule. When omitted, the dispatcher MUST default the field to `true` for the tick AND persist that default into `campaign_state.json.kill_subagent_on_done` so the disk schema stays consistent across versions. The post-override boolean is consulted by Phase 6 step 9; toggling it on a later tick takes effect immediately for the next callback wake-up. The callback path does NOT re-read the trigger override (callbacks carry no scalar inputs) — the persisted value from the most recent scheduled wake-up is authoritative.
...
The callback payload does NOT need to carry: `branch`, `dev_branch`, `issue_min_iid`, `issue_max_iid`, `hourly_issue_quota`, `max_runtime_minutes`, `blocked_retry_limit`, `blocked_cooldown_ticks`, `max_concurrent_subagents`, `stuck_after_minutes`, `kill_subagent_on_done`. Those are loaded from the persisted `${CAMPAIGN_STATE_FILE}`. The callback path does NOT apply trigger overrides.
