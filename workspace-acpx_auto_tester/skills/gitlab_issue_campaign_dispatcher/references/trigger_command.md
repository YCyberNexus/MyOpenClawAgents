...
| `model_tiers`               | Optional comma-separated, ordered model list backing the persistent `model:{tier}` dimension (lowest first; element 0 = TIER_0 = default). The label for tier `k` is `model:<element-k>`. Defaults to `flash,pro,max` on a fresh deployment. Each element must match `[A-Za-z0-9_.-]+`. An empty list or a bad tier name aborts the tick with `"invalid_model_tiers: ..."`. Same carry-forward persistence semantics as `result_basename` / `data_basename` (the model list is a deployment property): when supplied, the dispatcher writes it into `campaign_state.json.model_tiers` and uses it; when omitted, the persisted value (or the default) is retained. The number of tiers caps the model upgrade; `resolve_model_tier` never raises past the last element. |
| `model_upgrade_continue_threshold` | Optional non-negative integer N — the model-upgrade soft trigger threshold. When `continue_count ≥ N` (cumulative continue-mode attempts for the IID), `resolve_model_tier` requests one upgrade. `0` (default) disables the continue soft trigger; the hard trigger (prior outcome ∈ `blocked-cc` / `timeout` / `failed-cc`) and the `quality:low` soft trigger still apply. Non-integer / negative values abort with `"invalid_model_upgrade_continue_threshold: must be >= 0"`. Same carry-forward persistence semantics as `model_tiers`. |
...
- Unparseable `worker_result_json` → orchestrator synthesizes a Phase 6 blocked-dispatcher reply (`block_reason="callback worker_result_json not valid JSON"`; an unparseable payload is a dispatcher-side anomaly, while a parseable reply stays CC-side) and processes it.
