...
| `model_tiers`               | Optional comma-separated ordered list of model identifiers from lowest (TIER_0, the default for a new issue) to the capped highest. Each name `N` maps to the GitLab label `model:N` (so the names must be label-safe — `[A-Za-z0-9_.-]`). Defaults to the 3-tier example `flash,pro,max`. `resolve_model_tier` (Phase 4 PREPARE, before spawn) stamps the resolved `model:{tier}` on each attempt and raises it one tier on a CC-side re-arm (see the §model upgrade rule in `label_lifecycle.md`). Same per-tick reset / persist semantics as `require_labels`. An empty list aborts with `"invalid_model_tiers"`; a name outside `[A-Za-z0-9_.-]` aborts with `"invalid_model_tiers: tier names limited to [A-Za-z0-9_.-]"`. |
| `model_upgrade_continue_threshold` | Optional non-negative integer `N`. The model-upgrade **soft** trigger fires when the issue's accumulated `continue` re-schedule count reaches `N` (`N=0` disables this particular soft trigger; `quality:low` and the CC-side hard trigger still apply). Defaults to `2`. Non-integer / negative values abort with `"invalid_model_upgrade_continue_threshold"`. Same per-tick reset / persist semantics as `model_tiers`. |
...
| `claude_settings_path`      | Optional absolute path to a Claude Code settings JSON file. When provided, the dispatcher copies this file to `${WORKTREE_DIR}/.claude/settings.json` (replacing the committed settings) during Phase 4 per-IID prep, BEFORE the subagent runs `scripts/run_acpx_attempt.sh` (which invokes `acpx claude exec`). Then runs `git update-index --skip-worktree .claude/settings.json` inside the worktree so the replacement is never staged into issue MRs. The flag is per-worktree (linked worktrees have independent indexes), so concurrent attempts are unaffected. Must be an absolute file path; values with `..`, whitespace, or shell-unsafe characters outside `[A-Za-z0-9_./-]` are rejected with `"invalid_claude_settings_path"`. The file must exist and be readable at copy time; a missing/unreadable file marks the IID `blocked-dispatcher` (a per-IID prep failure is dispatcher-side). Omitted or empty → the committed `.claude/settings.json` from the base branch is used as-is. |
...
- Unparseable `worker_result_json` → orchestrator synthesizes a Phase 6 `blocked-dispatcher` reply (an unparseable callback is a dispatcher-side condition; `block_reason="callback worker_result_json not valid JSON"`) and processes it.
...
1. Deliver `RUN_CHILD_COMPLETION_CALLBACK` to the SAME orchestrator session that issued the original `sessions_spawn` (typically `agent:acpx_auto_tester_temporal:main`).
