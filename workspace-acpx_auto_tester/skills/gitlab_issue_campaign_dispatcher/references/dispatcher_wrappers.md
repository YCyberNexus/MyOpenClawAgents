...
14. Call `load_ui_accounts.sh`, which reads
    ...
    `ifp-data/ifp-common/ifp_users.json`; override with trigger
    `ui_accounts_relpath`, carry-forward persisted; resolved under the
    project checkout root, NOT under `${REPO_PATH}/${DATA_BASENAME}/`);
    map exit codes 10–16 to the documented tick-level abort strings
