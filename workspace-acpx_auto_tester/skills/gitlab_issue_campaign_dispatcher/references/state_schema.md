...
- **Missing `model_settings_dir`** — treat as `null` in memory and persist as `null` on next write. There is no hardcoded default. Same Phase 1 override / carry-forward rule as `ui_accounts_relpath` (the per-tier settings directory is a deployment property, not a per-tick decision), with `null` / `""` triggering **settings-copy skip mode**: Phase 4 per-IID prep does not copy any `<tier>-settings.json`, and acpx uses the worktree's committed `.claude/settings.json` as-is. When configured, Phase 4 copies `${model_settings_dir}/${MODEL}-settings.json` → `${WORKTREE_DIR}/.claude/settings.json` (a missing/unreadable tier file marks that IID `blocked-dispatcher`). Replaces the removed `claude_settings_path` field.
