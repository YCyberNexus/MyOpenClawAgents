...
max_concurrent_subagents  = 1
active_issue_iids         = []
active_issue_sessions     = []   # same-index childSessionKey/runId entries; `issue-<project>-<iid>` means pending launch
unfinished_iids           = []
completed_iids            = []
blocked_iids              = []
failed_iids               = []
...
- Same applies to `active_issue_session` → `active_issue_sessions`. In async-callback mode the plural field may contain anonymous child keys such as `agent:acpx_auto_tester:subagent:<uuid>`. A value equal to the logical issue key `issue-<project>-<iid>` is a pending-launch placeholder, not proof that a child exists.
