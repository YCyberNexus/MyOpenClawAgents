...
max_concurrent_subagents     = 1
max_accounts_per_issue       = 14
stuck_after_minutes          = 332   # = ceil((acpx_timeout_seconds + 120) / 60) + 30
acpx_timeout_seconds         = 18000
kill_subagent_on_terminal    = true
kill_subagent_on_done        = true
...
| `acpx_timeout_seconds`  | int             | Post-override snapshot of the trigger's `acpx_timeout_seconds`. Defaults to `18000` and must be integer ≥ 60. Rendered into the executor prompt in Phase 4 step 7 as `{ACPX_TIMEOUT_SECONDS}` and `{ACPX_TIMEOUT_MINUTES} = floor(value / 60)`. Persisted for audit; callback path reads it for diagnostic purposes only. The runtime-wide subagent timeout, if enabled, is deployment configuration and is not persisted here. |
...
- **Missing `stuck_after_minutes`** — default to `ceil((acpx_timeout_seconds + 120) / 60) + 30` (`332` when `acpx_timeout_seconds` is omitted) and persist.
- **Legacy `run_timeout_seconds` present** — delete it during `load_state`; OpenClaw 2026.6.11 does not accept this value as a per-call spawn parameter, and runtime-wide timeout policy belongs to deployment configuration.
- **Missing `acpx_timeout_seconds`** — default to `18000` and persist.
...
10. **Best-effort 测试结果回报 (optional).** When `result_note_enabled=true` AND the final status is terminal `done` / `failed` / `timeout` (NOT `blocked` — retryable), `dispatch_followup.sh` runs `scripts/post_result_note.sh` after the drain: it reads the issue's `<!-- req_origin v1 {...} -->` marker note (written upstream by `git_issuer`, via G1b) and — **only if present** — posts a `<!-- req_result v1 {...} -->` note (via G9) carrying `{iid,status,attempt,mr_url,wiki_url,reason,ts,origin}` for an external 114-side relay to deliver to the original requester. Isolated: child stdout → `/dev/null` (never pollutes the Phase 6 envelope), wrapped in `set +e` (a failure is logged to `wrapper.log` and NEVER aborts Phase 6), and a no-op when the issue carries no `req_origin`. Touches only issue **notes** — never labels / MR / state files. Full cross-region contract: the req_dispatcher workspace's `docs/integration/result_notify_loop.md`; glab forms in `references/glab_commands.md` §G14.
