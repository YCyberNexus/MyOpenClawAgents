...
| `ui_accounts_relpath`   | string          | Post-override snapshot of the trigger's `ui_accounts_relpath`. Defaults to `"ifp-data/ifp-common/ifp_users.json"`. Forwarded as `UI_ACCOUNTS_RELPATH=...` to `scripts/load_ui_accounts.sh`, which derives the absolute pool file path `${REPO_PATH}/${ui_accounts_relpath}` (the relpath is resolved under the project checkout root, NOT under `${REPO_PATH}/${DATA_BASENAME}/`). Tick aborts with `"invalid_ui_accounts_relpath"` when the value is empty, absolute, contains `.` / `..` segments, whitespace, or characters outside `[A-Za-z0-9_./-]`. Carry-forward semantics: omitted-in-trigger keeps the persisted value (or the default on a fresh deployment). **Schema migration:** before SKILL_VERSION 2026-05-27.1 this field was resolved under `${REPO_PATH}/${DATA_BASENAME}/`; a persisted carry-forward value from before the upgrade (e.g. `"ifp-common/ifp_users.json"`) MUST be re-sent on the next trigger with the data basename prepended (e.g. `ui_accounts_relpath=ifp-data/ifp-common/ifp_users.json`) so the resolved path matches reality. The loader does NOT auto-migrate — instead, `load_ui_accounts.sh` exit 10 carries an explicit hint when the legacy resolved path still exists on disk. |
