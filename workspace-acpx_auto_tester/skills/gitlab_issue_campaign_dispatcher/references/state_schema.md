...
```json
{
  ...
  "last_reconcile_evidence": "/data/openclaw_work/.../openclaw_log/dispatcher/reconcile-20260508T100501Z.json",
  "updated_at": "2026-05-08T10:05:30Z"
}
```
...
- **Missing `issue_iids_whitelist` / `require_labels` / `require_labels_match`** — default per "Fresh-init values" above (`[]` / `[]` / `"or"`) and persist on next write. These fields are NOT carried forward across ticks beyond the trigger's say-so: each scheduled wake-up's Phase 1 OVERRIDES them with the trigger's current values (or with defaults when the trigger omits them). The on-disk copy is for audit and crash-recovery only.
