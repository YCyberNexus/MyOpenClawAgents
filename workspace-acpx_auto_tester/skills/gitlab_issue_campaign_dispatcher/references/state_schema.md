...
| `model_tiers`           | array of string | Post-override snapshot of the trigger's `model_tiers` field (ordered low→high). Defaults to `["flash","pro","max"]`. Drives `resolve_model_tier`; each name `N` maps to the `model:N` label. |
