...
| `dispatch_record_spawn.sh` | `campaign_state.json` (post-launch ack writeback of `run_id` / `child_session_key` / `spawned_at`, or — on launch failure — Phase 6 `blocked-dispatcher` synth + drain). |
...
```json
{
  ...
  "precheck_relpath": "hulat/precheck.json",
  "model_settings_dir": "/data/models/ifp-models",
  ...
  "pending_subagents": {
    "14": {
      ...
      "child_session_key": "agent:acpx_auto_tester_temporal:subagent:b6719233-bcc8-4418-b401-c5f5f752609a",
...
| `child_session_key`  | string \| null | The anonymous `childSessionKey` returned by `sessions_spawn` (e.g. `agent:acpx_auto_tester_temporal:subagent:<uuid>`). For runtime-side audit only; not used for matching callbacks. Same nullability rule as `run_id`. |
...
A `pending_subagents` entry with `placeholder: true` is a transient state during Phase 4 step 5 / Phase 5; it MUST NOT survive the end of the scheduled wake-up. If a crash leaves a placeholder behind, the next scheduled wake-up's stuck-pending eviction (which inspects `spawned_at`) treats it as stuck and synthesizes a `blocked-dispatcher` Phase 6 reply (no callback arrived — a dispatcher-side condition; `block_reason="placeholder pending entry survived: spawn was never observed to land"`).
...
precheck_relpath          = null   # unconfigured by default; trigger field opts in
model_settings_dir        = null   # unconfigured by default; trigger field opts in
repo_path                 = "/data"
active_issue_iids         = []
...
| `issue_iids_whitelist`  | array of int    | Post-override snapshot of the trigger's `issue_iids` field. Empty `[]` = no whitelist (full `[issue_min_iid, issue_max_iid]` range). When non-empty, the effective IID universe = range ∩ this list (IIDs outside range are silently dropped at Phase 1). Pending entries outside the effective universe are scope-evicted at the top of the scheduled tick, marked `blocked-dispatcher` (scope eviction is a dispatcher-side condition), and returned with a best-effort runtime kill action when a `child_session_key` is known. |
...
| `model_tiers`           | array of string | Post-override snapshot of the trigger's `model_tiers` field (ordered low→high). Defaults to `["flash","pro","max"]`. Drives `resolve_model_tier`; each name `N` maps to the `model:N` label. **When `model_settings_dir` is configured this list is the wisdom-order superset; the effective per-deployment upgrade ladder is its subset whose `<tier>-settings.json` exists on disk (auto-discovered each tick via `derive_effective_model_tiers`). `reconcile.sh` / `resolve_model_tier` consume the effective subset; `ensure_labels.sh` / `set_issue_label.sh` keep using the full list. A configured `model_settings_dir` with none of the tiers' `<tier>-settings.json` present aborts the tick with `"no_model_settings_files"`.** |
| `model_upgrade_continue_threshold` | int | Post-override snapshot of the trigger's `model_upgrade_continue_threshold`. Defaults to `2`. The model-upgrade soft trigger fires when an issue's `continue_count` reaches this value. |
| `model_settings_dir`    | string \| null  | Absolute path to the directory holding the per-tier Claude Code settings files (`<tier>-settings.json`, one per `model_tiers` element). **No default** — `null` / `""` / absent means unconfigured: the per-tier settings copy is skipped and acpx uses the worktree's committed `.claude/settings.json` as-is. When set, Phase 4 per-IID prep copies `${model_settings_dir}/${MODEL}-settings.json` to `${WORKTREE_DIR}/.claude/settings.json` (renamed on copy) and marks it `skip-worktree` — this is what actually switches acpx's underlying model for the resolved `model:{tier}`. Tick aborts with `"invalid_model_settings_dir"` when a non-null trigger or persisted value is `/`, or contains `.` / `..` path segments (`/.`, `/./`, `/..`, `/../`), whitespace, or characters outside `[A-Za-z0-9_./-]`. A configured dir whose `${MODEL}-settings.json` is missing/unreadable marks **that IID** `blocked-dispatcher` (strict no-fallback — no downgrade to a default tier file). **Per-tick, NOT carry-forward** (unlike `ui_accounts_relpath`): omitting the field on a trigger means unconfigured for that tick. The dispatcher still snapshots the current value (or empty) here so the same-batch callback (`dispatch_followup.sh`) derives the identical effective tier list, but the snapshot is never restored on a later omitting trigger. Omission alone reverts that tick to legacy behavior; no manual reset is needed. Replaces the removed `claude_settings_path` field. |
...
| `precheck_relpath`      | string \| null  | Post-override snapshot of the trigger's `precheck_relpath`. **No default** — `null` / `""` / absent means unconfigured: the dispatcher skips the §16b environment precheck entirely. When non-null and non-empty, after batch formation the dispatcher runs `scripts/precheck.sh` with `PRECHECK_RELPATH=...`, which reads the manifest at `${REPO_PATH}/${precheck_relpath}` and probes its `urls` (pure-bash `/dev/tcp`) / `commands` / `env_vars` / `files`. A `required` failure (exit 1) or malformed manifest (exit 2) tags the tick's batch IIDs `precheck-failed` and aborts the tick; an absent manifest file is skipped (exit 0). Tick aborts with `"invalid_precheck_relpath"` when a non-null trigger or persisted value is absolute, contains `.` / `..` segments, whitespace, or characters outside `[A-Za-z0-9_./-]`. **Carry-forward** semantics (same as `ui_accounts_relpath`): omitted-in-trigger keeps the persisted value. To disable a previously-configured deployment, manually `jq` `campaign_state.json` to set `.precheck_relpath = null`. The `precheck-failed` marker does not consume retry or upgrade the model tier and is cleared when the issue next enters `doing`. See [`precheck_manifest.md`](precheck_manifest.md). |
...
- **Missing `model_settings_dir`** — treat as `null` in memory; the dispatcher persists the current tick's value (the trigger's, or `null` / `""` when omitted) on next write. There is no hardcoded default. **Per-tick, NOT carry-forward** (unlike `ui_accounts_relpath`): omitting the trigger field means unconfigured for that tick — the persisted snapshot is NOT restored (it exists only for same-batch callback consistency) — with `null` / `""` triggering **settings-copy skip mode**: Phase 4 per-IID prep does not copy any `<tier>-settings.json`, and acpx uses the worktree's committed `.claude/settings.json` as-is. When configured, Phase 4 copies `${model_settings_dir}/${MODEL}-settings.json` → `${WORKTREE_DIR}/.claude/settings.json` (a missing/unreadable tier file marks that IID `blocked-dispatcher`). Replaces the removed `claude_settings_path` field.
- **Missing `precheck_relpath`** — treat as `null` in memory and persist as `null` on next write. No hardcoded default. Same Phase 1 override / **carry-forward** rule as `ui_accounts_relpath`: omitted-in-trigger keeps the persisted value, with `null` / `""` triggering **precheck-skip mode** (the §16b environment precheck does not run). When configured, after batch formation the dispatcher runs `precheck.sh`; a `required` failure (or malformed manifest) tags the batch IIDs `precheck-failed` and aborts the tick, while an absent manifest file is skipped (not an error). The field is new, so there is no schema-migration caveat.
...
Initialized by `scripts/allocate_attempt.sh` (which the dispatcher runs before each spawn). The dispatcher's Phase 4 prep refreshes `status="in_progress"` / `mode` / `attempts_total` / `latest_attempt_*` before spawn. The dispatcher's Phase 6 follow-up writes the terminal `status` / `commit_sha` / `merge_request_url` / `block_reason` from the subagent's compact JSON reply or from an inline-synthesized `blocked-dispatcher` reply (launch / scope / stuck-pending eviction). Launch-side `sessions_spawn` failures preserve `retry_count`; other blocked/failed outcomes consume that budget. The subagent does NOT write this file.
...
| `block_reason`          | string \| null  | Required when `status` is any `blocked-*` or `failed-*`.               |
...
| `continue_count`        | int             | v2 model upgrade (§6): how many times this IID has been re-scheduled in continue mode. Incremented in PREPARE when the attempt runs in continue mode; read by `resolve_model_tier` as a soft upgrade trigger (`>= model_upgrade_continue_threshold`). |
| `model_tier`            | int             | v2 cache of the resolved model tier (0-based index into `model_tiers`). Source of truth is the live `model:{tier}` label; this mirror only feeds the next tick's read. |
| `model_name`            | string          | v2 cache of the resolved model identifier (`model_tiers[model_tier]`); injected into build_prompt as `MODEL_NAME`. |
...
| `blocked-cc`  | CC-side retryable failure (acpx non-timeout failure, `NO_CHANGES`, push rejected, a post-acpx step failed). For acpx failures after worktree prep, the subagent first tries to stage, commit, and force-push any committable partial work to `${WORK_BRANCH}`; it still opens no MR and consumes retry budget. | no |
| `blocked-dispatcher` | Dispatcher-side retryable failure (per-IID prep failed, `sessions_spawn` launch failed, scope eviction, stuck-pending / no callback). No CC output; consumes retry budget. | no |
| `failed-cc`   | CC-side non-recoverable, or `blocked-cc` with `retry_count > blocked_retry_limit`.   | yes |
| `failed-dispatcher` | Dispatcher-side `blocked-dispatcher` with `retry_count > blocked_retry_limit`. | yes |
| `done`        | After post-push verification, Wiki evidence publication, `doing → done`, MR creation / rotation, and `pr` label addition succeeded. `pr` REPLACES `done` on the live issue. | yes |
...
| `no_changes`  | Legacy compact-reply value for `stage_and_guard.sh` `NO_CHANGES`; new prompts normalize this to `blocked-cc` because no MR / `pr` label can be produced. | no |
| `blocked` / `failed` | Legacy v1 statuses, normalized by the dispatcher (`blocked` → `blocked-cc`, `failed` → `failed-cc`); never written by current code. | n/a |
...
| `prior_attempt_count`     | continue mode only — number of past `acpx_auto_tester_temporal:attempt-summary` notes (plus legacy pre-rename attempt-summary notes) the prompt included |
...
| `status`             | string (enum)   | v2 per-side: `done` / `no_changes` (legacy) / `blocked-cc` / `failed-cc` / `timeout`. The subagent IS the CC side, so it only ever emits CC-side failures; the dispatcher synthesizes `blocked-dispatcher` / `failed-dispatcher` for its own prep/spawn/stuck failures. New subagent prompts convert no-diff outcomes to `blocked-cc`; legacy `no_changes` / `blocked` / `failed` are normalized by the dispatcher (`no_changes`/`blocked` → `blocked-cc`, `failed` → `failed-cc`). The subagent prefers `blocked-cc` — the dispatcher promotes `blocked-cc → failed-cc` (and `blocked-dispatcher → failed-dispatcher`) in Phase 6 when retry budget exhausted. The subagent emits `timeout` only from the dedicated timeout flow (see `executor_prompt.md` §timeout_flow); the dispatcher does NOT promote `timeout → failed-*`. |
...
| `commit_sha`         | string          | Empty `""` if commit/push did not run or failed. May be non-empty for `done`, `timeout`, or `blocked-cc` replies when partial work was successfully force-pushed. |
...
| `labels_added`       | array of string | The labels the subagent ADDED in Steps 6 / 8 or fail-flow label sync (e.g. `["pr"]` for done — pr replaces done; `["blocked-cc"]` for a blocked failure before done; `["done","blocked-cc"]` for a failure after done but before pr). |
...
| `block_reason`       | string          | Required non-empty when `status` is any `blocked-*`, `failed-*`, or `timeout`; empty `""` otherwise. For `timeout`, the value typically reads `acpx exec exceeded {ACPX_TIMEOUT_SECONDS}s wall-clock cap`. |
...
- **Inline-synthesized path** — Phase 5 launch failure after in-tick retry exhaustion / scope or stuck-pending eviction at the top of a scheduled wake-up; the orchestrator constructs a minimal `blocked-dispatcher` reply on the spot (all of these are dispatcher-side conditions). Phase 5 launch failures and pending evictions are tracked as launch-side internally for retry accounting; this is not a compact-reply field.

The validation pipeline is the same in both paths:

1. Parse `worker_result_json` (callback path) or use the synthesized object directly. On parse failure, treat as a synthetic dispatcher-side blocked reply: `{"iid":<callback.iid>,"attempt_number":<callback.attempt_number>,"status":"blocked-dispatcher","block_reason":"callback worker_result_json not valid JSON: <first 200 chars>"}` and continue with that (a missing/unparseable callback is a dispatcher-side condition).
...
3. Normalize legacy statuses: `reply.status="no_changes"`/`"blocked"` → `"blocked-cc"` (set `block_reason="subagent produced no staged changes"` for `no_changes` when the reply gave no reason); `reply.status="failed"` → `"failed-cc"` (the subagent IS the CC side).
4. If `reply.status` is any `blocked-*` / `failed-*` / `timeout`, require non-empty `reply.block_reason`. Empty → set `block_reason="subagent reply status=<status> with empty block_reason"`.
5. Synchronize live GitLab work labels from the final status with `scripts/set_issue_label.sh`: `done` ends as `pr` (pr replaces done); `blocked-cc` / `blocked-dispatcher` end as no `doing` + that label; `failed-cc` / `failed-dispatcher` end as no `doing` + that label; and `timeout` ends as no `doing` + `timeout`. The work-label exclusivity in `set_issue_label.sh` clears the other work labels on the single `add`, and never touches `model:{tier}` / `quality:low`. A required live-label sync failure on a `done` result demotes it to `blocked-cc` (with `block_reason` appended); `blocked-*` / `failed-*` / `timeout` keep their status and best-effort re-sync.
6. If `status` is `blocked-cc`/`blocked-dispatcher` AND `retry_count > blocked_retry_limit` (after incrementing), promote per side (`blocked-cc → failed-cc`, `blocked-dispatcher → failed-dispatcher`), add to `failed_iids`, and run the failed-label sync. For Phase 5 launch-side synthesized blocked replies only (always `blocked-dispatcher`), do not increment `retry_count` and do not promote on this tick. `timeout` is NEVER promoted regardless of `retry_count` — it stays parked in `timeout_iids` until a human strips `timeout`, adds `retry`, or applies `continue`.
...
- `retry_count` ← prior + 1 if final status is any `blocked-*` / `failed-*` and the reply is NOT a Phase 5 launch-side synthesized `blocked-dispatcher` reply; else prior unchanged. `timeout` does NOT consume retry budget.
- `block_reason` ← final block_reason after validation / label-sync errors (empty → null)
