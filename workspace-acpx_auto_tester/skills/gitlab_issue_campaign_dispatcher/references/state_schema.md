...
```json
{
  ...
  "pending_subagents": {
    "14": {
      ...
      "child_session_key": "agent:acpx_auto_tester_ifp:subagent:b6719233-bcc8-4418-b401-c5f5f752609a",
...
| `child_session_key`  | string \| null | The anonymous `childSessionKey` returned by `sessions_spawn` (e.g. `agent:acpx_auto_tester_ifp:subagent:<uuid>`). For runtime-side audit only; not used for matching callbacks. Same nullability rule as `run_id`. |
...
| `prior_attempt_count`     | continue mode only — number of past `acpx_auto_tester_ifp:attempt-summary` notes (plus legacy pre-rename attempt-summary notes) the prompt included |
