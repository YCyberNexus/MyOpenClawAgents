...
- Subagent session name SHOULD be the deterministic `issue-${PROJECT}-${ISSUE_IID}` when the channel supports thread-bound named sessions. On channels that do not (e.g., webchat returns `errorCode=thread_required`), anonymous spawns (`mode="run"` or runtime equivalent) are acceptable as long as the synchronous compact JSON reply contract holds. The orchestrator matches replies to dispatched IIDs by the `iid` field of the compact JSON (Phase 6 validation), not by the runtime session-key label — so the rendered prompt's `iid` field MUST be correct. See SKILL.md §Concurrency Policy "Subagent identity" for the full rule.
