...
# JSON. The caller performs the actual label mutations (add model:{name}, and
# remove quality:low when consume_quality_low is set — i.e. an upgrade landed
# or the tier is already capped) and injects the resolved model name into
# build_prompt.
#
# Inputs (positional):
...
#     "hard_trigger":   <bool>,
#     "soft_trigger":   <bool>,
#     "consume_quality_low": <bool>   # remove quality:low (upgrade landed or tier capped)
#   }
#
# UPGRADE? = hard ∪ soft (§6):
