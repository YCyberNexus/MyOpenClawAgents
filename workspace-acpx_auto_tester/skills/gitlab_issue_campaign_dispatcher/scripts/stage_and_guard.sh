...
# another issue's `ifp-result/` subtree (or the campaign-level
# `ifp-result/_dispatcher/` subtree).
#
# Why this guard:
...
#     concern.
#   - The agent's runtime state lives at `${REPO_PATH}/ifp-result/...`
#     OUTSIDE the worktree's path. Inside the worktree, `ifp-result/` is
#     the project's own gitignored placeholder (since master/dev
#     gitignore ifp-result content). Anything staged under
#     `ifp-result/_dispatcher/` or another issue's
#     `ifp-result/issue-<other-iid>/` is a structural leak — almost
...
#   3   another-issue / dispatcher ifp-result subtree leaked into the
