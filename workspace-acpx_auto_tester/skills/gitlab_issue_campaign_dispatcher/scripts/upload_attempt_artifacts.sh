...
{
  echo "<!-- acpx_auto_tester_ifp:attempt-wiki-artifacts v1 attempt=${ATTEMPT_NUMBER_PADDED} -->"
  echo "## acpx_auto_tester_ifp attempt ${ATTEMPT_NUMBER_PADDED} OpenClaw logs"
  echo
  ...
  echo
  echo "<!-- /acpx_auto_tester_ifp:attempt-wiki-artifacts -->"
} > "${NOTE_FILE}"
