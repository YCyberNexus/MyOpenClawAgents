...
`scripts/` is FLAT for the wrappers you type. The three wrapper scripts the
orchestrator ever invokes — `scripts/dispatch_prepare_tick.sh`,
`scripts/dispatch_record_spawn.sh`, `scripts/dispatch_followup.sh` — all live
directly in `scripts/`, never in a per-step or named subfolder. There is no
`scripts/vsc/`, no `scripts/<phase>/`; any such nested path is a
hallucination, not a real location. Type exactly the `scripts/<name>.sh` path
written in this SKILL — never invent, guess, or "locate" a deeper path, and
never `find` / `ls` for the script. If a wrapper you expect is genuinely
missing, that is a deployment fault to report and stop on (per §No-Fallback),
NOT a cue to try an alternative directory. (`scripts/` also holds other files
— subagent-side scripts and a `scripts/safety_bin/` helper dir — that the
orchestrator never types a path into; ignore them.)
