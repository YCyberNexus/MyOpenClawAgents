...
> `issue/<iid>-auto-fix-att<NNN>`; subagent Step 1.5 (`collect_metrics.sh`)
...
> ledger for `aggregate_benchmark.sh` to render an issue × model matrix. Full
...
| Tells it to | run the steps: `bash run_acpx_attempt.sh` → collect metrics → stage → push → verify → wiki → labels → summarize → emit compact JSON (on benchmark-test the MR/`pr` steps 7/8 are removed and `done` is terminal — see the callout above) | implement the GitLab issue using `hulat/agents/*.md` and write spec output under `${OUTPUT_DIR}` |
...
| v2 label model (per-side blocked-cc / blocked-dispatcher / failed-cc / failed-dispatcher, pr replaces done, model:{tier} dimension, quality:low). **benchmark-test diverges: `done` is terminal (no `pr`), and `model:{tier}` is pinned per tick (not escalated) — see the callout above.** | `references/label_lifecycle.md`; `_dispatch_lib.sh::phase6_sync_labels`; `set_issue_label.sh` |
