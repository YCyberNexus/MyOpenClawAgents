...
Universal contract (serial repo-root execution, same-IID never parallel, async-callback delivery): [`SOUL.md`](../../SOUL.md) §Subagent Concurrency Policy. Dispatcher-specific operational details:

- **Serial-only batch shape (scheduled wake-up only).** Because all attempts share the main repo checkout, `max_concurrent_subagents` MUST be exactly `1`; any trigger value greater than `1` is a tick-level configuration error. Pick at most one IID (`min(1, launch_budget_remaining, eligible_iids_remaining)`) in the standard backlog-first order. Allocate one attempt number, one UI account, run prep for that IID, and Phase 5 issues exactly one anonymous `sessions_spawn`.
...
- **Completion contract.** Each subagent's terminal compact JSON is delivered via `RUN_CHILD_COMPLETION_CALLBACK`. The orchestrator wakes on the callback, runs Phase 6 for the matched IID, drains the pending entry. If a callback never arrives within `pending_subagents[iid].stuck_after_minutes` (default: 210 min) after `spawned_at`, the next scheduled wake-up performs **stuck-pending eviction** — synthesize a blocked reply (`block_reason="no callback received within stuck_after_minutes"`) and process it as Phase 6.
...
2. In Phase 4, before per-IID prep, the dispatcher runs `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> BATCH_SIZE=<n> bash scripts/load_ui_accounts.sh` (where `n` is the batch size). The script:
...
3. **Pool-too-small is a tick-level failure.** If `load_ui_accounts.sh` exits 13, the dispatcher MUST abort the tick with a one-line summary (`"ui_account_pool_too_small: pool=<size> batch=<n>"`). It MUST NOT share an account between IIDs. In this repo-root execution model the batch size is always 1, so this means the deployment has no usable account in `<workspace>/config/ui_accounts.env`.
...
- The optional clone target `repo_path` is a bootstrap path input. When omitted, scripts default to `/data/${project}`. When supplied, validate it is an absolute concrete repo directory (no broad parent like `/data`, no `..`, no whitespace, no shell-unsafe characters outside `[A-Za-z0-9_./-]`), then forward it as `REPO_PATH=...` on every script exec and render it into the executor prompt. Non-default deployments must keep passing it on scheduled triggers and callbacks because the dispatcher needs it before locating `campaign_state.json`.
...
- Always: `PROJECT`, `GROUP`, `GITLAB_TOKEN`. Also always when non-default is in use: `REPO_PATH`, `RESULT_BASENAME`, `DATA_BASENAME`. `REPO_PATH` defaults inside `env_paths.sh` to `/data/${PROJECT}` if unset; the basenames default to `ifp-result` / `ifp-data`. The orchestrator MUST forward the post-override values so projects that ship custom paths or basenames get them on every exec.
...
1. **Bootstrap.**
   ...
   - Source `scripts/env_paths.sh` with `PROJECT`, `GROUP`, `GITLAB_TOKEN`, and `REPO_PATH=<repo_path>` if the trigger supplied a non-default clone target. This must happen before acquiring the flock because `LOCK_FILE` is derived from the selected repo path.
...
2. **Load + override campaign state.**
   ...
   - Apply trigger-input override: overwrite `issue_min_iid`, `issue_max_iid`, `hourly_issue_quota`, `max_runtime_minutes`, `blocked_retry_limit`, `blocked_cooldown_ticks`, `max_concurrent_subagents`, and (optionally) `stuck_after_minutes` with the trigger values. When the trigger omits `max_concurrent_subagents`, default it to `1` for the tick AND persist that default. If the post-override value is anything other than `1`, abort the tick with `"invalid_max_concurrent_subagents_for_repo_root: expected 1"`. When the trigger omits `stuck_after_minutes`, default to `210` and persist.
   - **Repo path override.** When the trigger supplies `repo_path`, validate it is an absolute concrete repo directory (reject `/`, broad shared parents such as `/data` / `/tmp`, dot segments, whitespace, and shell-unsafe characters outside `[A-Za-z0-9_./-]`; abort with `"invalid_repo_path"`) and use it for this tick. When omitted, default to `/data/${project}`. Forward the selected value as `REPO_PATH=...` on every subsequent script exec this tick and render it into the executor prompt as `{WORKTREE_DIR}`. Persist `campaign_state.json.repo_path` for audit after the state file has been loaded, but do not rely on the persisted value to bootstrap a non-default deployment; the trigger/callback must keep carrying it.
...
1. **Reconcile against the `effective_iid_universe` from Phase 1 step 4.** Two invocation shapes are supported:
   - **Range mode** (whitelist empty — current default behavior): `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> MIN_IID=<min> MAX_IID=<max> bash scripts/reconcile.sh`.
   - **List mode** (whitelist non-empty): `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> IID_LIST="<comma-separated effective_iid_universe>" bash scripts/reconcile.sh`. When `IID_LIST` is non-empty, the script ignores `MIN_IID` / `MAX_IID` and queries exactly the listed IIDs. If `effective_iid_universe == []`, pass `IID_LIST=""`; the script writes an evidence file containing an empty JSON array.
...
2. **Tick-level prep — once per tick, only if pending is empty.**
   - `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> bash scripts/ensure_labels.sh` — idempotent; creates the workflow labels if missing. Failure → tick-level `"ensure_labels_failed"` summary and stop.
   - `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> BRANCH=<branch> bash scripts/clone_or_pull.sh` — keeps the main repo's refs current. Failure → tick-level `"clone_or_pull_failed"` summary and stop.
...
3. **Allocate attempt numbers SEQUENTIALLY.** For each IID in the batch, run `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> IID=<iid> bash scripts/allocate_attempt.sh` in its own Bash exec, capturing the printed number `N_iid`.
4. **Allocate UI accounts for the batch.** Run `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> BATCH_SIZE=<batch_size> bash scripts/load_ui_accounts.sh` in a single Bash exec, capturing exactly `batch_size` lines of `user:pass` form. Bind `account[k]` to the `k`-th IID of the batch (k=0..batch_size-1). Record `ui_account_index=k` for the IID — this is what goes into `pending_subagents[iid].ui_account_index`. On any non-zero exit code (10/11/12/13), abort the tick.
...
6. **Per-IID prep.** For the single IID in the batch:
...
**Spawn the surviving serial batch.** Issue exactly one `sessions_spawn` if Phase 4 prep succeeded. The entire payload is the rendered subagent prompt.
...
1. Validate the ack contains both `runId` and `childSessionKey`. Missing → synthesize a Phase 6 blocked reply for THIS IID immediately (`block_reason="sessions_spawn returned no runId/childSessionKey: <raw response>"`), process it on the spot (write terminal state files, classify), and DO NOT add to `pending_subagents`.
...
**Spawn-time failure note.** The dispatcher MUST NOT retry a failed launch inside this tick. The IID is blocked/cooldown'd and the next scheduled wake-up may reschedule it.
...
1. **Bootstrap.** `cd ${SKILL_DIR}`, source `env_paths.sh` with the callback's `project` / `group` / `gitlab_token` (plus `REPO_PATH` when the scheduled trigger used non-default `repo_path`, and plus `RESULT_BASENAME` / `DATA_BASENAME` if the callback payload carries them — otherwise env_paths.sh's hardcoded `/data/${PROJECT}`, `ifp-result`, and `ifp-data` defaults bootstrap the path layout for the initial state-file read), acquire flock. If the lock is held, return `"lock_held"` and exit 0 — the callback is idempotent at the runtime level (it will be retried, OR the holder of the lock is a still-running scheduled tick that will replay state on its next wake-up). Do NOT spin.
