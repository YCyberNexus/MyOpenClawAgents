...
| Sent how | `sessions_spawn(prompt=<rendered executor_prompt.md>, label=..., timeoutSeconds=30, runTimeoutSeconds=18000, cleanup="keep")` — no session name | NEVER sent over `sessions_spawn`; only read by `acpx` from disk via its `-f` flag inside `run_acpx_attempt.sh` |
