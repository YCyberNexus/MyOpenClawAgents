...
6. **Per-IID prep.** For each IID in the batch (run all sub-steps for one IID before moving to the next; `prepare_attempt.sh` serializes its `git fetch` + `git worktree add` segment under `${RESULT_ROOT}/_dispatcher/locks/repo.lock`, but that segment is short and other prep work runs without the lock):
   ...
   1.5. **Detect resume.** Read the previous `attempt_state.json` (attempt number = `attempts_total`). If the file is missing or unreadable, set `ACPX_RESUME=false` — the previous run left no usable evidence. Otherwise, if `attempts_total > 0` (not the first attempt) AND `status=blocked` AND `block_reason` contains a marker of acpx interruption (`"acpx run failed"`, `"acpx exec timed out"`, or `"acpx exec timed out and no pollable process session was available"`), set `ACPX_RESUME=true`. Otherwise `ACPX_RESUME=false`. When `ACPX_RESUME=true`, force `ISSUE_MODE=continue` (so the worktree branches from `origin/${WORK_BRANCH}` if it exists) and skip the `needs_continue` / per-issue state `mode` check for this tick — resume takes priority over reviewer-continue. **CROSS-REF:** The `block_reason` markers above MUST match the FAIL messages in `references/executor_prompt.md` Step 1 (acpx exit != 0 and acpx exec timeout). If you edit those error messages, update this list.
