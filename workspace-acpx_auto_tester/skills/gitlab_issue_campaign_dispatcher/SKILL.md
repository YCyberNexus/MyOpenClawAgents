...
The dispatcher MUST therefore allocate **distinct UI accounts per IID** (one per robot file, up to `accounts_per_issue`) for every concurrent batch:
...
2. In Phase 4, before per-IID prep, the dispatcher runs `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> BATCH_SIZE=<n> ACCOUNTS_PER_ISSUE=<k> bash scripts/load_ui_accounts.sh` (where `n` is the batch size and `k` is `accounts_per_issue`). The script:
   - prints `n * k` accounts in pool-file order, one `user:pass` per line, and exits 0; OR
   ...
   - exits 13 if the pool is smaller than `BATCH_SIZE * ACCOUNTS_PER_ISSUE`;
   - exits 14 if `ACCOUNTS_PER_ISSUE` is set but not a positive integer;
   - exits 15 if `BATCH_SIZE` is set but not a positive integer.
3. **Pool-too-small is a tick-level failure.** If `load_ui_accounts.sh` exits 13, the dispatcher MUST abort the tick with a one-line summary (`"ui_account_pool_too_small: pool=<size> needed=<n*k>"`). It MUST NOT share an account between IIDs or between robot executions within an IID. The pool size is therefore the hard upper bound on `max_concurrent_subagents * accounts_per_issue` for this deployment — Phase 1 step 2 rejects a trigger combination whose product exceeds the pool size with the same `"ui_account_pool_too_small"` summary.
4. **Allocation is per-batch and persisted into `pending_subagents` until callback drains.** The dispatcher binds a block of `accounts_per_issue` accounts to each IID in the batch, in iteration order (`accounts[k*N .. k*N+N-1]` → `k`-th IID, where `N = accounts_per_issue`). The accounts are serialized as a JSON array and passed to `scripts/build_prompt.sh` via the `UI_ACCOUNTS` env var; the script renders all accounts into the Claude Code prompt's `# Working environment` section. The `ui_account_index_start` (= `k * N`) is recorded in `pending_subagents[iid].ui_account_index_start`. The account block is considered "in use" until the matching `RUN_CHILD_COMPLETION_CALLBACK` arrives and Phase 6 drains the pending entry, OR until stuck-pending eviction releases it. Because §Concurrency Policy's single-batch-in-flight invariant + stuck-pending eviction at the top of every scheduled wake-up together guarantee `pending_subagents` is empty when a new batch forms, the next batch's accounts are always drawn fresh from the pool head — no persistent allocation table needed across ticks.
...
- All scalar trigger inputs (`issue_min_iid`, `issue_max_iid`, `hourly_issue_quota`, `max_runtime_minutes`, `blocked_retry_limit`, `blocked_cooldown_ticks`, `max_concurrent_subagents`, `accounts_per_issue`) are authoritative for this tick. Overwrite the disk copy in `campaign_state.json` before running the algorithm. `max_concurrent_subagents` and `accounts_per_issue` must satisfy `1 ≤ max_concurrent_subagents`, `1 ≤ accounts_per_issue`, and `max_concurrent_subagents * accounts_per_issue ≤ ui_account_pool_size`; values below 1 abort the tick with `"invalid_max_concurrent_subagents: must be >= 1"` or `"invalid_accounts_per_issue: must be >= 1"`, product exceeding the pool size aborts with `"ui_account_pool_too_small: pool=<size> needed=<product>"`.
...
2. **Load + override campaign state.**
   ...
   - Apply schema migration if the on-disk file uses the legacy scalar `active_issue_iid` / `active_issue_session` — see `references/state_schema.md` "Legacy on-disk shapes the loader must tolerate" for the rule. Default `max_concurrent_subagents` to `1` if missing; default `accounts_per_issue` to `14` if missing. If `pending_subagents` is missing (legacy file), initialize to `{}`. If `issue_iids_whitelist` / `require_labels` / `require_labels_match` are missing (older file), initialize to `[]` / `[]` / `"or"`.
   - Apply trigger-input override: overwrite `issue_min_iid`, `issue_max_iid`, `hourly_issue_quota`, `max_runtime_minutes`, `blocked_retry_limit`, `blocked_cooldown_ticks`, `max_concurrent_subagents`, `accounts_per_issue`, and (optionally) `stuck_after_minutes` / `acpx_resume` / `kill_subagent_on_terminal` with the trigger values. When the trigger omits `max_concurrent_subagents`, default it to `1` for the tick AND persist that default. When the trigger omits `accounts_per_issue`, default it to `14` for the tick AND persist that default. The post-override values must satisfy `1 ≤ max_concurrent_subagents`, `1 ≤ accounts_per_issue`, and `max_concurrent_subagents * accounts_per_issue ≤ ui_account_pool_size` (call `BATCH_SIZE=<max_concurrent_subagents> ACCOUNTS_PER_ISSUE=<accounts_per_issue> bash scripts/load_ui_accounts.sh` and treat exit code 13 as `"ui_account_pool_too_small: pool=<size> needed=<product>"`). Values below 1 abort the tick with `"invalid_max_concurrent_subagents: must be >= 1"` or `"invalid_accounts_per_issue: must be >= 1"`. When the trigger omits `stuck_after_minutes`, default to `330` and persist. When the trigger omits `acpx_resume`, default to `false` and persist. When the trigger omits `kill_subagent_on_terminal`, default to `true` and persist, except that legacy `kill_subagent_on_done=false` with no new field disables terminal cleanup for compatibility.
...
4. **Allocate UI accounts for the batch.** Run `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> BATCH_SIZE=<batch_size> ACCOUNTS_PER_ISSUE=<accounts_per_issue> bash scripts/load_ui_accounts.sh` in a single Bash exec, capturing exactly `batch_size * accounts_per_issue` lines of `user:pass` form. Let `N = accounts_per_issue`. Bind the block `accounts[k*N .. k*N+N-1]` to the `k`-th IID of the batch (k=0..batch_size-1). Record `ui_account_index_start = k * N` for the IID — this is what goes into `pending_subagents[iid].ui_account_index_start`. On any non-zero exit code (10/11/12/13/14/15), abort the tick.
5. **Pre-spawn persist.** For every IID in the batch, write a placeholder pending entry: `pending_subagents[iid] = {attempt_number: N_iid, run_id: null, child_session_key: null, ui_account_index_start: k*N, spawned_at: null, placeholder: true}` (where `N = accounts_per_issue`) and append `iid` to `active_issue_iids` (and a human-readable label `issue-<project>-<iid>` to `active_issue_sessions` for logging). Persist `campaign_state.json` BEFORE any glab mutation. **This persist is the structural guarantee that the orchestrator does not double-spawn the same IID across crashes / concurrent ticks** — see §Per-Issue Subagent Rules. Phase 5 replaces the placeholder with the real `run_id` / `child_session_key` / `spawned_at` after `sessions_spawn` returns its launch ack.
