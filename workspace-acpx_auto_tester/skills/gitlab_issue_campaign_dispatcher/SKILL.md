...
| Shape | starts with sentinel `# ACPX_AUTO_TESTER_EXECUTOR_PROMPT_V1`, contains `<config>` / `<issue>` / `<env_contract>` / `<instructions>` XML-style blocks | starts with "You are working on GitLab issue #<iid>. Implement the change ...", contains `# Issue` / `# Working environment` / `# UI test accounts` / `# Rules` markdown headers |
| Sent how | `sessions_spawn(prompt=<rendered executor_prompt.md>, label=..., timeoutSeconds=30, runTimeoutSeconds=<run_timeout_seconds> (default 18000), cleanup="keep")` — no session name | NEVER sent over `sessions_spawn`; only read by `acpx` from disk via its `-f` flag inside `run_acpx_attempt.sh` |
| File on disk | NOT persisted to disk — rendered in-memory by the orchestrator and passed to `sessions_spawn` | persisted at `${LOG_DIR}/prompt.txt`, force-added into the MR diff by `stage_and_guard.sh` |

**HARD RULE: `${LOG_DIR}/prompt.txt` is NEVER the spawn payload.** If the orchestrator is about to call `sessions_spawn`, the prompt string it passes MUST come from rendering `references/executor_prompt.md` — not from reading `${LOG_DIR}/prompt.txt`, not from the stdout of `build_prompt.sh`, not from any other source. Phase 5 step 0 (see §Phase 5 — Async Spawn) enforces this with a sentinel grep before the spawn call.
...
| 5     | Async Spawn       | issue one **anonymous** `sessions_spawn` per IID (NO session name passed — runtime returns `runId`/`childSessionKey`). Persist `(iid, attempt_number, run_id, child_session_key, ui_account_index_start, spawned_at)` into `campaign_state.json.pending_subagents`. Return a `waiting_for_callbacks` summary and exit. **Does NOT wait for subagent completion.** |
...
The system under test is a UI / web app. When the same UI account logs in twice, the older session is logged out. Two concurrent subagents — or two concurrent robot executions within a subagent — that share an account therefore continuously kick each other out; the work product is unreliable, and the issue cannot complete.

The dispatcher MUST therefore allocate **distinct UI accounts per IID** for every concurrent batch. The per-IID account count is computed automatically from the pool size, `max_concurrent_subagents` (the configured concurrency cap, NOT the actual batch size — slot sizes stay stable across ticks even when fewer IIDs are eligible than the cap allows), and `max_accounts_per_issue` (trigger field, default 14). The pool is divided into exactly `max_concurrent_subagents` raw slots; the integer remainder is front-loaded onto the first slots, then each slot size is capped at `max_accounts_per_issue`. Accounts left beyond the capped slot sizes are unused for that tick. Examples:

- `pool=3,  max_concurrent_subagents=2` → slot sizes `2,1` (first IID gets 2 accounts, second gets 1)
- `pool=50, max_concurrent_subagents=4, max_accounts_per_issue=14` → slot sizes `13,13,12,12`
- `pool=40, max_concurrent_subagents=1, max_accounts_per_issue=14` → slot sizes `14`
- `pool=40, max_concurrent_subagents=1, max_accounts_per_issue=10` → slot sizes `10`
...
2. In Phase 4, before per-IID prep, the dispatcher runs `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> MAX_CONCURRENT_SUBAGENTS=<n> MAX_ACCOUNTS_PER_ISSUE=<m> bash scripts/load_ui_accounts.sh` (where `n` is the post-override `max_concurrent_subagents` and `m` is the post-override `max_accounts_per_issue`, default 14). The script:
   - prints all `pool_size` accounts in pool-file order, one `user:pass` per line, on stdout;
   - prints `POOL_SIZE=<n>` and capped `SLOT_SIZES=<size_0>,<size_1>,...,<size_{n-1}>` on stderr (info only — captured by the orchestrator to slice stdout into per-IID blocks); the csv length equals `MAX_CONCURRENT_SUBAGENTS`, and every size is `≤ MAX_ACCOUNTS_PER_ISSUE`;
   - exits 0 on success; OR
   ...
   - exits 13 if `MAX_CONCURRENT_SUBAGENTS > pool_size` (each in-flight subagent must hold at least one distinct account);
   - exits 14 if `MAX_CONCURRENT_SUBAGENTS` is set but not a positive integer;
   - exits 15 if `MAX_ACCOUNTS_PER_ISSUE` is set but not a positive integer.
3. **Pool-too-small is a tick-level failure.** If `load_ui_accounts.sh` exits 13, the dispatcher MUST abort the tick with a one-line summary (`"ui_account_pool_too_small: pool=<size> max_concurrent_subagents=<value>"`). It MUST NOT share an account between IIDs or between robot executions within an IID. The pool size is therefore the hard upper bound on `max_concurrent_subagents` for this deployment — Phase 1 step 2 rejects a trigger value above the pool size with the same `"ui_account_pool_too_small"` summary.
4. **Allocation is per-batch and persisted into `pending_subagents` until callback drains.** Let `SLOT_SIZES = [s_0, s_1, …, s_{max_concurrent_subagents-1}]` from stderr after `max_accounts_per_issue` capping. The `k`-th IID of the batch (k=0..batch_size-1) is bound to the slot `accounts[offset_k .. offset_k + s_k - 1]`, where `offset_k = sum(SLOT_SIZES[0..k-1])` (offset_0 = 0). When `batch_size < max_concurrent_subagents`, only the first `batch_size` slots are consumed; the remaining accounts stay in the pool unused for this tick. If capping leaves surplus accounts beyond the consumed slot sizes, those accounts also stay unused for this tick. The accounts are serialized as a JSON array and passed to `scripts/build_prompt.sh` via the `UI_ACCOUNTS` env var; the script renders all accounts into the Claude Code prompt's `# Working environment` section. The values `ui_account_index_start = offset_k` and `ui_account_count = s_k` are recorded in `pending_subagents[iid]`. The account block is considered "in use" until the matching `RUN_CHILD_COMPLETION_CALLBACK` arrives and Phase 6 drains the pending entry, OR until stuck-pending eviction releases it. Because §Concurrency Policy's single-batch-in-flight invariant + stuck-pending eviction at the top of every scheduled wake-up together guarantee `pending_subagents` is empty when a new batch forms, the next batch's accounts are always drawn fresh from the pool head — no persistent allocation table needed across ticks.
5. **Forbidden workarounds.** The dispatcher MUST NOT:
   ...
   - skip the `load_ui_accounts.sh` call even when the batch size is 1 (the script is cheap; the single-IID batch still gets a deterministic account block from the pool head)
   - persist account-to-IID assignments across ticks (the next tick re-allocates from the head of the pool file)
   - inject the accounts into the rendered subagent prompt (the subagent does not need the credentials — they live in the Claude Code prompt only, where Claude Code reads them)
   - divide the pool by the actual `batch_size` (which can be smaller than `max_concurrent_subagents` when fewer IIDs are eligible) — raw slot sizes MUST be derived from `max_concurrent_subagents`, then capped by `max_accounts_per_issue`, so a single IID in a sparse tick does not get an over-sized account block while a future full batch starves
...
- All scalar trigger inputs (`issue_min_iid`, `issue_max_iid`, `hourly_issue_quota`, `max_runtime_minutes`, `blocked_retry_limit`, `blocked_cooldown_ticks`, `max_concurrent_subagents`, `max_accounts_per_issue`) are authoritative for this tick. Overwrite the disk copy in `campaign_state.json` before running the algorithm. `max_concurrent_subagents` must satisfy `1 ≤ max_concurrent_subagents ≤ ui_account_pool_size`; values below 1 abort the tick with `"invalid_max_concurrent_subagents: must be >= 1"`, values exceeding the pool size abort with `"ui_account_pool_too_small: pool=<size> max_concurrent_subagents=<value>"`. `max_accounts_per_issue` defaults to `14` when omitted and must be a positive integer; values below 1 or non-integers abort with `"invalid_max_accounts_per_issue: must be >= 1"`. Per-IID account counts are derived automatically from the pool size, `max_concurrent_subagents`, and `max_accounts_per_issue` (see §UI Account Allocation Policy); the trigger does NOT specify an `accounts_per_issue` field.
...
- `load_ui_accounts.sh`: optional `MAX_CONCURRENT_SUBAGENTS` and `MAX_ACCOUNTS_PER_ISSUE`.
- `clone_or_pull.sh`: also `BRANCH`.
- `prepare_attempt.sh`, `build_prompt.sh`: also `ISSUE_IID`, `ATTEMPT_NUMBER`, `BRANCH`, `DEV_BRANCH`, `ISSUE_MODE` (and for `build_prompt.sh`: `UI_ACCOUNTS`). `HULAT_DIR` is derived inside `env_paths.sh` and does not need to be passed.
...
2. **Load + override campaign state.**
   ...
   - Apply schema migration if the on-disk file uses the legacy scalar `active_issue_iid` / `active_issue_session` — see `references/state_schema.md` "Legacy on-disk shapes the loader must tolerate" for the rule. Default `max_concurrent_subagents` to `1` if missing. Default `max_accounts_per_issue` to `14` if missing. A legacy `accounts_per_issue` field on disk is silently ignored (per-IID account counts are now derived from `max_concurrent_subagents`, `max_accounts_per_issue`, and the pool size — see §UI Account Allocation Policy); drop it on the next persist. If `pending_subagents` is missing (legacy file), initialize to `{}`. If `issue_iids_whitelist` / `require_labels` / `require_labels_match` are missing (older file), initialize to `[]` / `[]` / `"or"`.
   - Apply trigger-input override: overwrite `issue_min_iid`, `issue_max_iid`, `hourly_issue_quota`, `max_runtime_minutes`, `blocked_retry_limit`, `blocked_cooldown_ticks`, `max_concurrent_subagents`, `max_accounts_per_issue`, and (optionally) `stuck_after_minutes` / `run_timeout_seconds` / `acpx_timeout_seconds` / `kill_subagent_on_terminal` with the trigger values. When the trigger omits `max_concurrent_subagents`, default it to `1` for the tick AND persist that default. When the trigger omits `max_accounts_per_issue`, default it to `14` for the tick AND persist that default. The trigger does NOT carry `accounts_per_issue`; if a legacy trigger still includes the field, ignore it. The post-override `max_concurrent_subagents` must satisfy `1 ≤ max_concurrent_subagents ≤ ui_account_pool_size` (call `MAX_CONCURRENT_SUBAGENTS=<max_concurrent_subagents> MAX_ACCOUNTS_PER_ISSUE=<max_accounts_per_issue> bash scripts/load_ui_accounts.sh` and treat exit code 13 as `"ui_account_pool_too_small: pool=<size> max_concurrent_subagents=<value>"`). Values below 1 abort the tick with `"invalid_max_concurrent_subagents: must be >= 1"`. The post-override `max_accounts_per_issue` must be a positive integer; values below 1 or non-integers abort the tick with `"invalid_max_accounts_per_issue: must be >= 1"`. When the trigger omits `stuck_after_minutes`, default to `330` and persist. When the trigger omits `kill_subagent_on_terminal`, default to `true` and persist, except that legacy `kill_subagent_on_done=false` with no new field disables terminal cleanup for compatibility.
   - **Timeout overrides.** When the trigger omits `run_timeout_seconds`, default to `18000` for the tick AND persist that default. When the trigger omits `acpx_timeout_seconds`, default to `18000` for the tick AND persist. Both post-override values MUST be integers ≥ 60 (values below 60 or non-integers abort with `"invalid_run_timeout_seconds: must be >= 60"` / `"invalid_acpx_timeout_seconds: must be >= 60"`) AND satisfy `acpx_timeout_seconds ≤ run_timeout_seconds` (abort with `"run_timeout_seconds_below_acpx_timeout_seconds"` when violated). `run_timeout_seconds` is consulted by Phase 5 when constructing the `sessions_spawn` call's `runTimeoutSeconds` parameter; `acpx_timeout_seconds` is rendered into the executor prompt in Phase 4 step 7 (see §Phase 4 — Per-IID Prep step 7 for the placeholder substitution). Persist both into `campaign_state.json` so the callback path can read the authoritative values off disk without re-reading the trigger.
   - **Repo path override.** When the trigger supplies `repo_path`, validate it is an absolute parent directory (reject `/`, dot segments, whitespace, and shell-unsafe characters outside `[A-Za-z0-9_./-]`; abort with `"invalid_repo_path"`) and use it for this tick. When omitted, default the parent to `/data`. Forward the selected parent as `REPO_PARENT_PATH=...` on every subsequent dispatcher script exec this tick, derive final `REPO_PATH=${repo_path}/${project}`, and render that final path into the executor prompt as `{REPO_PATH}`. The dispatcher also renders the per-attempt linked worktree path as `{WORKTREE_DIR}` (acpx's cwd). Persist `campaign_state.json.repo_path` as the parent path for audit after the state file has been loaded, but do not rely on the persisted value to bootstrap a non-default deployment; the trigger/callback must keep carrying it.
...
4. **Allocate UI accounts for the batch.** Run `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> MAX_CONCURRENT_SUBAGENTS=<max_concurrent_subagents> MAX_ACCOUNTS_PER_ISSUE=<max_accounts_per_issue> bash scripts/load_ui_accounts.sh` in a single Bash exec. Capture stdout (`pool_size` lines of `user:pass`) AND stderr (`POOL_SIZE=<n>` and capped `SLOT_SIZES=<csv>`); the csv length equals `max_concurrent_subagents`, and every slot size is `≤ max_accounts_per_issue`. Let `SLOT_SIZES = [s_0, s_1, …, s_{max_concurrent_subagents-1}]`. For the `k`-th IID of the batch (k=0..batch_size-1), set `offset_k = sum(SLOT_SIZES[0..k-1])` (offset_0 = 0) and bind the block `accounts[offset_k .. offset_k + s_k - 1]` to that IID. Record `ui_account_index_start = offset_k` and `ui_account_count = s_k` for the IID — these are what go into `pending_subagents[iid]`. When `batch_size < max_concurrent_subagents`, only the first `batch_size` slots are consumed; the remaining accounts stay unused for this tick. If capping leaves surplus accounts beyond the consumed slot sizes, those accounts also stay unused for this tick. On any non-zero exit code (10/11/12/13/14/15), abort the tick.
5. **Pre-spawn persist.** For every IID in the batch, write a placeholder pending entry: `pending_subagents[iid] = {attempt_number: N_iid, run_id: null, child_session_key: null, ui_account_index_start: offset_k, ui_account_count: s_k, spawned_at: null, placeholder: true}` and append `iid` to `active_issue_iids` (and a human-readable label `issue-<project>-<iid>` to `active_issue_sessions` for logging). Persist `campaign_state.json` BEFORE any glab mutation. **This persist is the structural guarantee that the orchestrator does not double-spawn the same IID across crashes / concurrent ticks** — see §Per-Issue Subagent Rules. Phase 5 replaces the placeholder with the real `run_id` / `child_session_key` / `spawned_at` after `sessions_spawn` returns its launch ack.
6. **Per-IID prep.** For each IID in the batch (run all sub-steps for one IID before moving to the next; `prepare_attempt.sh` serializes its `git fetch` + `git worktree add` segment under `${RESULT_ROOT}/_dispatcher/locks/repo.lock`, but that segment is short and other prep work runs without the lock):
   ...
   4. **Transition to `doing`.** Use `scripts/set_issue_label.sh` with single-label calls only:
      ...
      - `set_issue_label.sh add doing` is also an idempotent safety net: it removes conflicting workflow labels in the same GitLab issue update, preserving only `doing` and unrelated non-workflow labels.
   ...
   5. `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> ISSUE_IID=<iid> ATTEMPT_NUMBER=<N_iid> BRANCH=<branch> DEV_BRANCH=<dev_branch> ISSUE_MODE=<mode_actual> UI_ACCOUNTS='<json-array>' bash scripts/build_prompt.sh` where `<json-array>` is a JSON array of `{"u":"<user>","p":"<pass>"}` objects for this IID's allocated slot (`s_k` accounts — see Phase 4 step 4). Writes `${LOG_DIR}/prompt.txt` with the issue body, working environment, and UI-accounts list. Capture stderr (`CONTINUE_MODE_NO_REVIEWER_COMMENTS`, `CONTINUE_MODE_PRIOR_ATTEMPT_COUNT`) for the attempt state.
   ...
   7. **Render the executor prompt.** Substitute every `{...}` placeholder in `references/executor_prompt.md` with the per-IID values; verify no unsubstituted placeholders remain. If render fails (missing variable, unsubstituted token), mark the IID `blocked` with `block_reason="prompt template render incomplete: <name>"` and skip this IID for the batch. The full placeholder list lives in [`references/executor_prompt.md`](references/executor_prompt.md) §Template Variables; the timeout-specific placeholders `{ACPX_TIMEOUT_SECONDS}` and `{ACPX_TIMEOUT_MINUTES}` are derived from `campaign_state.json.acpx_timeout_seconds` (post-override value from Phase 1 step 2) — substitute `{ACPX_TIMEOUT_SECONDS}` with the integer value and `{ACPX_TIMEOUT_MINUTES}` with `floor(acpx_timeout_seconds / 60)` so the executor's Step 1 bash command timeout and the executor's hard soft-cap message stay in lockstep.

      **Do NOT use `${LOG_DIR}/prompt.txt` as the spawn payload.** That file is the *inner* Claude Code prompt for `acpx -f` only — see §Two prompts you MUST NOT confuse. The *outer* spawn payload is the in-memory string produced by rendering `references/executor_prompt.md`. The two are not interchangeable; sending the inner prompt to `sessions_spawn` makes the subagent skip `run_acpx_attempt.sh` entirely and execute `hulat/agents/*` directly, which then breaks `stage_and_guard.sh` (no `${LOG_DIR}/git_status.txt`/`acpx_raw.log`/`claude_result.txt` were ever written) and corrupts the per-IID state. The rendered executor prompt MUST start with the sentinel line `# ACPX_AUTO_TESTER_EXECUTOR_PROMPT_V1`; Phase 5 step 0 grep-checks this before issuing `sessions_spawn`.
...
- Set `timeoutSeconds=30` (launch-ack wait — without this the harness/gateway defaults to ~10s and the spawn was observed to return only a `childSessionKey` placeholder with no `runId`, no real subagent ever started). Set `runTimeoutSeconds=<run_timeout_seconds>` (subagent runtime cap; post-override value from `campaign_state.json.run_timeout_seconds`, default `18000`), `cleanup="keep"`. If the trigger supplied `--model` (reserved), forward it.
...
**Step 0 — payload sentinel pre-flight check (run BEFORE each `sessions_spawn` call).** Before issuing `sessions_spawn` for a given IID, the orchestrator MUST verify the rendered prompt string it is about to send begins with the literal sentinel line `# ACPX_AUTO_TESTER_EXECUTOR_PROMPT_V1` (exact bytes, no leading whitespace, no translation). Implementation: take the rendered payload string and check that its first line (everything up to the first `\n`) equals that sentinel exactly — a `head -n 1` + `grep -Fx` would do the same. This is a fixed-string check, not regex, not substring; the sentinel only validates as a match when it is the very first line of the payload. The sentinel is shipped as the first line of the rendered block in [`references/executor_prompt.md`](references/executor_prompt.md) and survives placeholder substitution unchanged. If the check fails for an IID, do **NOT** call `sessions_spawn` for it — instead synthesize a Phase 6 blocked reply on the spot with `block_reason="spawn payload missing executor sentinel — refused to ship inner Claude Code prompt (${LOG_DIR}/prompt.txt) as the outer spawn payload"`, run best-effort live-label sync (`remove doing`; `add blocked`), drain the placeholder pending entry written in Phase 4 step 5, return the IID's allocated UI account slot to the pool, and continue with the next IID in the batch. Do NOT mutate the payload to "add" the sentinel inline — a missing sentinel means the orchestrator built the wrong string and shipping it would skip `run_acpx_attempt.sh` entirely (see §Two prompts you MUST NOT confuse for the exact failure mode this guards against). Surface a `sentinel_missing_iids: [<iid>, …]` list in the chat summary so operators can spot the misroute.
...
4. **Synchronize live workflow labels as the final source-of-truth safety net.** Use `scripts/set_issue_label.sh` single-label calls:
   ...
   `set_issue_label.sh add <workflow-label>` removes conflicting workflow labels in the same update, so these explicit removes are intentionally redundant and the final state still converges if a prior remove was missed.
...
```json
{
  ...
  "pending_subagents": {
    "14": {"attempt_number": 3, "run_id": "9710b359-...", "child_session_key": "agent:acpx_auto_tester:subagent:b6719233-...", "ui_account_index_start": 0, "ui_account_count": 14, "spawned_at": "2026-05-07T13:42:01Z"}
  },
...
```json
{
  ...
  "active_issue_sessions": ["issue-px_ifp_hulat-14"],
  "pending_subagents": {
    "14": {"attempt_number": 1, "run_id": "...", "child_session_key": "...", "ui_account_index_start": 0, "ui_account_count": 14, "spawned_at": "2026-05-08T09:10:00Z"}
  },
  "max_concurrent_subagents": 1,
  "max_accounts_per_issue": 14,
  "ui_account_pool_size": 40,
