...
**Spawn the surviving batch.** Issue one `sessions_spawn` per IID whose Phase 4 prep succeeded (sequentially in the orchestrator — anonymous spawns return launch acks within seconds). The entire payload of each spawn is that IID's rendered subagent prompt.
