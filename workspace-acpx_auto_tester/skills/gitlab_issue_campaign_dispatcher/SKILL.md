...
The dispatcher MUST therefore allocate **distinct UI accounts per IID** for every concurrent batch. The per-IID account count is computed automatically from the pool size and `max_concurrent_subagents` (the configured concurrency cap, NOT the actual batch size — slot sizes stay stable across ticks even when fewer IIDs are eligible than the cap allows). The pool is divided into exactly `max_concurrent_subagents` slots; the integer remainder is front-loaded onto the first slots so every account is reachable. Examples:

- `pool=3,  max_concurrent_subagents=2` → slot sizes `2,1` (first IID gets 2 accounts, second gets 1)
- `pool=50, max_concurrent_subagents=4` → slot sizes `13,13,12,12`
- `pool=14, max_concurrent_subagents=1` → slot sizes `14`
...
5. **Forbidden workarounds.** The dispatcher MUST NOT:
   ...
   - divide the pool by the actual `batch_size` (which can be smaller than `max_concurrent_subagents` when fewer IIDs are eligible) — slot sizes MUST be derived from `max_concurrent_subagents` so a single IID in a sparse tick does not get an over-sized account block while a future full batch starves
...
- `load_ui_accounts.sh`: optional `MAX_CONCURRENT_SUBAGENTS`.
...
4. **Allocate UI accounts for the batch.** Run `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> MAX_CONCURRENT_SUBAGENTS=<max_concurrent_subagents> bash scripts/load_ui_accounts.sh` in a single Bash exec. Capture stdout (`pool_size` lines of `user:pass`) AND stderr (`POOL_SIZE=<n>` and `SLOT_SIZES=<csv>`); the csv length equals `max_concurrent_subagents`. Let `SLOT_SIZES = [s_0, s_1, …, s_{max_concurrent_subagents-1}]`. For the `k`-th IID of the batch (k=0..batch_size-1), set `offset_k = sum(SLOT_SIZES[0..k-1])` (offset_0 = 0) and bind the block `accounts[offset_k .. offset_k + s_k - 1]` to that IID. Record `ui_account_index_start = offset_k` and `ui_account_count = s_k` for the IID — these are what go into `pending_subagents[iid]`. When `batch_size < max_concurrent_subagents`, only the first `batch_size` slots are consumed; the remaining accounts stay unused for this tick. On any non-zero exit code (10/11/12/13/14), abort the tick.
...
6. **Per-IID prep.** For each IID in the batch (run all sub-steps for one IID before moving to the next; `prepare_attempt.sh` serializes its `git fetch` + `git worktree add` segment under `${RESULT_ROOT}/_dispatcher/locks/repo.lock`, but that segment is short and other prep work runs without the lock):
   ...
   1.5. **Detect deprecated resume flag.** Read `acpx_resume` from `campaign_state.json` (persisted from trigger override, default `false`). If `acpx_resume == true` AND `attempts_total > 0` (not the first attempt), set `ACPX_RESUME=true`; otherwise `ACPX_RESUME=false`. Does NOT force `ISSUE_MODE=continue` — worktree setup follows the normal reconciliation result (Step 1). Current acpx has no saved-session flag, so `build_prompt.sh` treats this as compatibility input and still writes a full prompt.
   ...
   2.5. **Copy Claude Code settings (if configured).** If `claude_settings_path` is non-empty:
        - Copy the file to the parent checkout, replacing the committed `settings.json`: `cp "<claude_settings_path value>" "${REPO_PATH}/.claude/settings.json"`. If the source file does not exist or is not readable, mark the IID `blocked` with `block_reason="claude_settings_path file not found or not readable: <path>"` and skip this IID for the batch. The parent checkout's `.claude/` directory already exists (committed in the base branch), so the target directory is guaranteed to exist.
        - Prevent the replacement from being staged into issue MRs: `git -C "${REPO_PATH}" update-index --skip-worktree .claude/settings.json`. This tells git to ignore local changes to the file in the parent checkout's index. Because `run_acpx_attempt.sh` invokes acpx from `${REPO_PATH}`, Claude Code reads `.claude/settings.json` from there — NOT from the per-attempt worktree.
   ...
   5. `PROJECT=<project> GROUP=<group> GITLAB_TOKEN=<token> REPO_PARENT_PATH=<repo_path> RESULT_BASENAME=<result_basename> DATA_BASENAME=<data_basename> ISSUE_IID=<iid> ATTEMPT_NUMBER=<N_iid> BRANCH=<branch> DEV_BRANCH=<dev_branch> ISSUE_MODE=<mode_actual> UI_ACCOUNTS='<json-array>' ACPX_RESUME=<true|false> bash scripts/build_prompt.sh` where `<json-array>` is a JSON array of `{"u":"<user>","p":"<pass>"}` objects for this IID's allocated slot (`s_k` accounts — see Phase 4 step 4). Writes `${LOG_DIR}/prompt.txt` with the issue body, working environment, and UI-accounts list. Current acpx has no saved-session flag, so even when `ACPX_RESUME=true`, the script writes the full self-contained prompt and emits `ACPX_RESUME_IGNORED=true`. Capture stderr (`ACPX_RESUME_IGNORED`, `CONTINUE_MODE_NO_REVIEWER_COMMENTS`, `CONTINUE_MODE_PRIOR_ATTEMPT_COUNT`) for the attempt state.
   6. **Initialize state files.** Write/refresh `${ATTEMPT_STATE_FILE}` with `{iid, attempt_number, attempt_started_at, mode_requested, mode_actual, mode_downgraded_from, acpx_resume_ignored, no_reviewer_comments, prior_attempt_count, local_branch, log_dir, status:"in_progress"}`. Write/refresh `${ISSUE_STATE_FILE}` with `{iid, session, status:"in_progress", mode:<mode_actual>, attempts_total:<N_iid>, latest_attempt_number:<N_iid>, latest_attempt_dir, retry_count:<from prior>, updated_at}`.
...
```json
{
  ...
  "pending_subagents": {
    "14": {"attempt_number": 3, "run_id": "9710b359-...", "child_session_key": "agent:acpx_auto_tester:subagent:b6719233-...", "ui_account_index_start": 0, "spawned_at": "2026-05-07T13:42:01Z"}
  },
...
```json
{
  ...
  "active_issue_sessions": ["issue-px_ifp_hulat-14"],
  "pending_subagents": {
    "14": {"attempt_number": 1, "run_id": "...", "child_session_key": "...", "ui_account_index_start": 0, "spawned_at": "2026-05-08T09:10:00Z"}
  },
  "max_concurrent_subagents": 1,
  "ui_account_pool_size": 40,
