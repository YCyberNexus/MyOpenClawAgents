...
2. If `sessions_spawn` for an issue subagent run fails or times out, the dispatcher MUST NOT run the subagent's logic inline, spawn a substitute run, or retry by adding a session name. Mark the IID `blocked` with an accurate `block_reason`, run best-effort live-label sync (`remove doing`; `add blocked`), and continue per Blocked Skip-and-Retry. (`label="#<iid>-att-<NNN>"` parameter rejection is handled separately — see §Concurrency Policy "Session label for runtime UI".)
