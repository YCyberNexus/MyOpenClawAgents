...
4. After every child in the launched batch is drained by runtime completion callback or GitLab reconciliation, the accounts implicitly return to the pool — the dispatcher does not persist allocations; the next batch re-allocates from the head of the file. The dispatcher must not launch a new batch while `active_issue_iids` is non-empty. A launch acknowledgement (`accepted` / `runId` / `childSessionKey`) does not release accounts.
