# acpx_auto_tester_temporal 使用文档

本文档面向调度器配置、运维和项目使用者。它说明如何把 trigger prompt 传给
`acpx_auto_tester_temporal`，以及 agent 从收到 trigger 到创建 MR、处理回调的完整流程。
...
`acpx_auto_tester_temporal` 是一个非交互式 GitLab issue 自动化 agent，用来周期性扫描一批
...
```text
agent:acpx_auto_tester_temporal:main
```
...
flowchart TD
    ...
    U -- "non-timeout failure" --> W["push partial work if possible, label blocked, summarize locally"]
...
- 非 timeout 的 acpx 失败：先尽力 stage、commit、push 和 post-push verify partial work；即使 push 成功也保持 `blocked`，不发 Wiki、不建 MR、不加 `pr`。
...
| `blocked` | 可重试失败；冷却后可能自动重试。非 timeout acpx 失败可能已经把 partial work 推到工作分支。 |
...
stateDiagram-v2
    ...
    Doing --> Blocked: retryable failure / push partial if acpx failed, add blocked
