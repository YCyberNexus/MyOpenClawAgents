...
依据 `CLAUDE.md` + `SKILL.md` + `references/` 中描述的 `acpx_auto_tester_temporal` 标签生命周期与 dispatcher 算法绘制。
...
stateDiagram-v2
    ...
    state PENDING {
        [*] --> Queued
        Queued : entry/ label in todo, new, continue, retry, blocked
    ...
    state DOING {
        ...
        EXECUTING --> [*]        : acpx_fail[non-timeout] / blocked_push_flow, push partial if possible
        EXECUTING --> [*]        : acpx_timeout / timeout_flow, push partial if possible
        EXECUTING --> [*]        : push_rejected / FAIL blocked
    ...
    DOING --> TIMEOUT : callback[timeout] / label(timeout), park
    ...
    BLOCKED : acpx failure may have pushed partial work to WORK_BRANCH

    BLOCKED --> PENDING : add_label[retry or continue]
    BLOCKED --> FAILED  : tick[retry_count gt limit] / promote
    BLOCKED --> CLOSED  : user_closes

    TIMEOUT : label = timeout, partial work may be pushed, no MR
    TIMEOUT : no auto retry, retry_count not consumed

    TIMEOUT --> PENDING : human_strips_timeout or add_label[retry or continue]
    TIMEOUT --> CLOSED  : user_closes
...
- `stuck` = `pending_subagents[iid]` 等待超过 `stuck_after_minutes`（默认 332 分钟，即 `ceil(run_timeout_seconds / 60) + 30`）。
- `callback[*]` 共享的副作用：drain `pending_subagents[iid]` + 尽力 `subagents kill` 子会话；图里只写了状态特有的标签转移。
- `blocked_push_flow` = 非 timeout acpx 失败后，尽力 `stage_and_guard.sh` → `commit_and_push.sh` → `post_push_verify.sh` 推送 partial work，然后仍返回 `blocked`，不发 Wiki、不建 MR、不加 `pr`。
- `timeout_flow` = acpx 达到 wall-clock cap 后，尽力推送 partial work，返回 `timeout`，不发 Wiki、不建 MR、不加 `pr`，也不消耗 blocked retry budget。
