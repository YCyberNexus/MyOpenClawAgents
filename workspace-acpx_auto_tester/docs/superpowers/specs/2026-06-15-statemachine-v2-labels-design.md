...
每个非平凡脚本改动后 `/opt/homebrew/bin/bash -n <script>` 语法检查（本机 /bin/bash 是 3.2.57 会误判，统一用 homebrew bash）。所有 `workspace-acpx_auto_tester/` 改动走 **code-review 子代理循环**（最多 3 轮），并在收尾时把 diff 指纹写入 `.claude/.review-done-sha` 解除 Stop hook。
