#!/usr/bin/env bash
# Stop hook: block end-of-turn if workspace-req_dispatcher/ has unreviewed changes
...
# Both sentinels live next to this script (workspace-req_dispatcher/.claude/) and are
# resolved as ABSOLUTE paths, so the hook works whether Claude Code is launched from the
# repo root or from inside workspace-req_dispatcher/ (the diff scope stays git-root-relative).
set -euo pipefail
...
# Fingerprint = hash of (HEAD diff + contents of untracked files) under workspace-req_dispatcher/.
fingerprint=$(
  {
    git diff HEAD -- workspace-req_dispatcher/ 2>/dev/null || true
    while IFS= read -r f; do
      [ -n "$f" ] && [ -f "$f" ] && printf '\n--- %s ---\n' "$f" && cat -- "$f"
    done < <(git ls-files --others --exclude-standard -- workspace-req_dispatcher/ 2>/dev/null)
...
reason="workspace-req_dispatcher/ 有未 code-review 的改动。按 CLAUDE.md 的 review 循环：调用 Agent(subagent_type=\"code-reviewer\")，在 prompt 里点明 review 当前 workspace-req_dispatcher/ 下未提交的 diff，吸收反馈，最多 3 轮直到零问题（或你判断改动是 trivial）。完成后执行下面这一行解除阻断：
