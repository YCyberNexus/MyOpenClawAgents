...
- **executor 队列恢复路径（C）**（trigger 名 `RUN_EXECUTOR_QUEUE_DRAIN`）：先调用 `evict_stuck.sh`，再幂等调用 `drain_executor_queue.sh`。过期 executor pending 会清匹配 active 槽；active 槽已满时返回 busy；active stuck 在 launching、launch_failed 到期、或 queue 非空且有空闲槽时继续推进。
