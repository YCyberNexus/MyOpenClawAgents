...
**两段匹配各以自己的 `run_id` 为主**，故即便对齐前也能推进；不要求下游 agent 回显 token 作主匹配（零侵入），executor 段额外用 `correlation_id` 作二次校验。用户出站推送通道**已对齐**：`notify_user.sh` 反向网关推 114 智伴（`openclaw agent run`，连接 pin `ZHIBAN_GATEWAY_URL` / `ZHIBAN_GATEWAY_TOKEN` / `ZHIBAN_AGENT`；任一空则留痕；`ZHIBAN_NOTIFY_TIMEOUT_SECONDS` 控制超时）。
