...
- 修订 / 澄清需求（issue 还没被 acpx 捞起，或正在跑，或已出 MR）；
- 撤销整个需求；
- 变更太大、近乎换了个需求。

acpx 已有的"按 label 重跑 + 每次 attempt 重读 issue body + continue 注入 reviewer 评论 + MR 轮换 supersede"机制足以承载重跑，**无需改 acpx**。本流程要做的，是让 git_issuer 把变更**正确落到 issue #N 上**。
...
| opened，`doing`（**正在跑**） | 编辑 description = 新需求 + 留一条 reviewer note 写变更要点；并打 **`retry`**（清零 fresh 重跑）或 **`continue`**（在工作分支上续跑）。**不要试图打断当前 attempt**（见 §5）。 | 当前 attempt 先跑完；**下一次** attempt 用新内容；旧 MR 被 acpx 轮换关闭、建新 MR（`Supersedes !旧`）。 |
| opened，`pr`（**已完成**，MR 已建） | 编辑 description（可选）+ 留 reviewer note 写变更要点 + 打 **`continue`**。 | continue 模式续跑，acpx 把 reviewer note 注入 prompt。 |
| opened，`blocked-*`/`failed-*`/`timeout` | 编辑 description = 新需求 + 留 note + 打 **`retry`**。 | 清零重跑。 |
| **撤销整个需求** | `glab issue close`（note 写撤销原因）。 | acpx 把 `closed` 当**硬终态跳过**，永不再排。 |
| 变更**太大 ≈ 新需求** | 关旧 #N（note 写"被 #N' 取代"）+ **新建 #N'**（带入口标签，走创建流程）。 | acpx 处理 #N'。 |

**最重要的一条规则**：**权威的需求内容永远写进 issue description**（acpx 每次 attempt、所有模式都重读 description）。reviewer **评论**只有 **continue 模式**才会被注入 prompt——所以 `retry`/fresh 重跑必须靠改 description，不能只靠评论。

## 4. acpx label 语义摘要（git_issuer 必须严格遵守）

git_issuer 在变更场景里**扮演 human reviewer**，所以可以打 acpx 的人工 review 标签。但只能用下面这套，且只能碰指定的几个：

- **工作态标签（互斥）**：`todo` `new` `retry` `continue` `doing` `done`(瞬态) `pr` `blocked-cc` `blocked-dispatcher` `failed-cc` `failed-dispatcher` `timeout`。
- git_issuer **只允许打** `retry`（fresh 重置重跑）或 `continue`（续跑）——这俩是 acpx 约定的 human review 重跑信号（acpx 自己从不打它们）。
- git_issuer **绝不能打** `doing`/`done`/`pr`/`blocked-*`/`failed-*`/`timeout`——那些是 acpx 自己的状态机，外部插手会破坏一致性。
- **撤销**用 `glab issue close`（不是打标签）。
- **不要动** acpx 的正交标签 `model:*` / `quality:low`，也不要动用户的 priority/severity 等业务标签。
- 打标签务必用**定向 add/remove**（保留其它非工作态标签），不要整组覆盖 `labels=`。

## 5. 安全边界（务必遵守）

1. **"正在跑（`doing`）"改不了当前这次 attempt**：acpx 在 attempt 开始时一次性渲染 prompt、`acpx claude exec` 是 one-shot。变更只在**下一次** attempt 生效。git_issuer **不要**尝试 kill 正在跑的子代理——acpx 没有对外的"中途取消"流程（只有 stuck 驱逐和终态 kill）。若确需"立刻掐断正在跑的"，那是 **acpx 侧的新能力**，不在本流程。
2. **git_issuer 只动 issue**（description / note / `retry`\|`continue` / `close` / 新建），**绝不 merge MR、绝不 close MR、绝不删历史**——MR 由 acpx 轮换、由人合并。
...
| acpx_auto_tester | **不变**：靠既有 `retry`/`continue`/`closed` + 重读 description + MR 轮换自动重跑收尾 |
