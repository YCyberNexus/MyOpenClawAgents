...
> **更新（落地后）**：本计划提到的 `USER_NOTIFY_CHANNEL`（用户出站推送通道占位）已在 `SKILL_VERSION=2026-06-30.5` 落地为**反向网关推 114 接收 agent**——`notify_user.sh` 用 `openclaw agent run` 把状态信封投给 114 agent，连接 pin 改为 `REPLY_GATEWAY_URL`/`REPLY_GATEWAY_TOKEN`，目标 agent 优先取 `origin.reply_agent`、否则取默认 `DEFAULT_REPLY_AGENT`，超时 pin 为 `REPLY_NOTIFY_TIMEOUT_SECONDS`（详见 `config/dispatcher.env` + SKILL）。下文 Step 中的 `USER_NOTIFY_CHANNEL` 字样按此理解。

## Global Constraints

- 本机 bash 一律用 `/opt/homebrew/bin/bash`（系统 /bin/bash 3.2.57 会误判）。
- 跨 agent spawn/callback 原语属**待对齐**：脚本里以"显式 gated 占位 + 记 ledger 留痕"实现，**不臆造**工具名/字段名（沿用现有 git_issuer spawn 在 SKILL prose 描述、references/trigger_command.md 标 ⚠️待对齐 的做法）。用户推送原语已在 `2026-06-30.5` 落地为反向网关推 114 接收 agent。
...
| `notify_user.sh`（新） | `EVENT`(`result`/`failure`) | `REPLY_GATEWAY_URL`/`REPLY_GATEWAY_TOKEN`/`DEFAULT_REPLY_AGENT`(默认兜底目标；优先使用 `ORIGIN_JSON.reply_agent`),`REPLY_NOTIFY_TIMEOUT_SECONDS`,`ORIGIN_JSON`,`IID`,`STATUS`,`MR_URL`,`REASON` |
...
- [ ] Step1 `routing.env`：注释 + 示例 `claw_gitlab/px_ifp_hulat_test=req_executor_ifp`；`dispatcher.env` 增 `ROUTING_FILE`/`REPLY_GATEWAY_URL`/`REPLY_GATEWAY_TOKEN`/`DEFAULT_REPLY_AGENT`/`DISPATCHER_CALLBACK_TARGET`（结果回调目标仍待对齐占位）。
...
- [ ] Step1 写脚本：`EVENT∈result/failure` 校验 → 缺少网关 pin 或目标 agent 则 `exit 0`（记 ledger 留痕）→ 按 STATUS 拼用户文案（done/failed/timeout，文案同设计稿 §4.3）→ 经反向网关 `openclaw agent run` 把状态信封投给 114 接收 agent（优先 `origin.reply_agent`、否则默认 `DEFAULT_REPLY_AGENT`）；投递失败仅记 ledger/log，exit0。
- [ ] Step2 验证：`bash -n`。
- [ ] Step3 冒烟：channel 空 no-op、非空落行、EVENT 非法非零。
- [ ] Step4 README 记 `REPLY_GATEWAY_URL` / `REPLY_GATEWAY_TOKEN` / `DEFAULT_REPLY_AGENT`。
- [ ] Gate：code-review。
