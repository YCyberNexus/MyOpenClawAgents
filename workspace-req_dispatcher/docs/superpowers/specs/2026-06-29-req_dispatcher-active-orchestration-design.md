...
req_dispatcher 首次需要**主动给用户推实质结论**（受理 ack 之外）。机制已落地为 `notify_user.sh` 经反向网关把结果信封投给 114 接收 agent：连接 pin 为 `REPLY_GATEWAY_URL` / `REPLY_GATEWAY_TOKEN`，目标 agent 优先取 `origin.reply_agent`，否则使用部署期默认 `DEFAULT_REPLY_AGENT`。脚本仿 `ops_notify.sh` 使用 best-effort 语义，但目标是 origin 而非运维 channel；通道未配置、目标 agent 缺失或投递失败时记 ledger 留痕、不静默丢。
...
| 任一段 pending 超 `STUCK_AFTER_MINUTES` 无回调 | `evict_stuck.sh` 合成 stuck、记 ledger（含 stage）、drain；executor 段且带 origin 时额外 best-effort 推 timeout 给用户。 |

> **零回调超时通知（已闭合）**：执行器的*正常*超时——runtime 投来空/无 status 的 `worker_result_json` 合成回调——走 `dispatch_followup.sh` → I2 → req_dispatcher executor 回调 → `notify_user.sh`，用户会收到"处理超时未完成"。"子代理彻底死亡、一个回调都不来"这一子情形落到 `evict_stuck.sh`：它现在把 executor 段 pending 合成 `stuck_evicted` 写 ledger 留痕并删除 pending 后，对 `stage=executor` 且带 `origin` 的驱逐条目 best-effort 调 `notify_user.sh`(`STATUS=timeout`)。`git_issuer` 段 stuck 仍只做 stuck ledger/ops，不伪装成执行超时。
...
2. **用户推送部署 pin**（§4.5）：`REPLY_GATEWAY_URL` / `REPLY_GATEWAY_TOKEN` / `DEFAULT_REPLY_AGENT` 的蓝区部署值需按 114 接收 agent 实际地址填写。
3. **执行器 GitLab token 注入方式**（§3.5）：pin 进 config vs 部署 env 注入；轮换策略。
4. **origin 元数据格式**：114 在需求文本里放 origin 的确切编码约定（字段集合含 `channel`/`user`/`conversation`/`reply_agent`，沿用 result_notify_loop §3 设想）。
