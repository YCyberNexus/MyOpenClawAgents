...
- `TARGET_SESSION_ID` 默认由脚本生成，并统一通过 `--session-id` 传给 OpenClaw CLI：普通调用为 `agent:${TARGET_AGENT}:main`；`RUN_SINGLE_ISSUE` 为 `agent:${TARGET_AGENT}:issue-<sanitized-project>-<iid>`。历史环境变量 `TARGET_SESSION_KEY` 仍可作为兼容输入，但也会转为 `--session-id`。普通下游调用不要手写这两个变量；脚本会拒绝包含省略号或尖括号的占位符 session selector。
...
`run_agent_turn.sh` 对未显式传 `TARGET_SESSION_ID`/`TARGET_SESSION_KEY` 的 I1 调用，会从 payload 的 `project` 与 `iid` 自动生成 session id：`agent:<executor>:issue-<sanitized-project>-<iid>`，例如 `agent:req_executor:issue-ai-infra-veqp-server-v3-11`。不要把多个 I1 固定投到 `agent:req_executor:main`。
