...
- `TARGET_SESSION_ID` 默认由脚本生成，值为 `agent:${TARGET_AGENT}:main`，并统一通过 `--session-id` 传给 OpenClaw CLI。历史环境变量 `TARGET_SESSION_KEY` 仍可作为兼容输入，但也会转为 `--session-id`。普通下游调用不要手写这两个变量；脚本会拒绝包含省略号或尖括号的占位符 session selector。
