...
AUTH_DIGEST='aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
...
record_batch() {
  ...
  CALLBACK_NONCE_SHA256="${AUTH_DIGEST}" \
  ORIGIN_JSON="${ORIGIN}" \
  MATCHED_COUNT="250" \
  REQUEST_DIGEST="$1" \
...
if ! jq -e --arg auth_digest "${AUTH_DIGEST}" '
  type == "object"
  ...
  and .batches["batch-250"].project == "group/subgroup/project"
  and .batches["batch-250"].callback_auth_mode == "nonce_v1"
  and .batches["batch-250"].callback_nonce_sha256 == $auth_digest
...
set +e
STATE_ROOT="${STATE_ROOT_PATH}" \
BATCH_ID="batch-unknown-origin" \
PROJECT="group/subgroup/project" \
EXECUTOR_AGENT="req_executor" \
CALLBACK_AUTH_MODE=nonce_v1 \
CALLBACK_NONCE_SHA256="${AUTH_DIGEST}" \
ORIGIN_JSON='{"channel":"wecom","unexpected_field":"unsupported"}' \
MATCHED_COUNT="1" \
REQUEST_DIGEST="digest-unknown-origin" \
  "${BASH}" "${SKILL_DIR}/scripts/record_executor_batch.sh" \
  >"${TEST_ROOT}/unknown-origin.out" 2>"${TEST_ROOT}/unknown-origin.err"
unknown_origin_rc=$?
set -e

if [ "${unknown_origin_rc}" -eq 0 ]; then
