...
derive_default_session_selector() {
  ...
  batch_id="$(printf '%s\n' "${message}" | extract_trigger_field \
    "RUN_DRIVEN_ISSUE_BATCH" "batch_id")"
  if [ -n "${batch_id}" ]; then
  ...
  project="$(printf '%s\n' "${message}" | extract_trigger_field \
    "RUN_SINGLE_ISSUE" "project")"
  iid="$(printf '%s\n' "${message}" | extract_trigger_field \
    "RUN_SINGLE_ISSUE" "iid")"
  case "${iid}" in
    *[!0-9]*|""|0)
      printf 'agent:%s:main\n' "${target_agent}"
      return 0
...
MESSAGE_SOURCE="${MESSAGE_SOURCE:-auto}"
...
case "${MESSAGE_SOURCE}" in
  auto|stdin) ;;
  *) echo "run_agent_turn: MESSAGE_SOURCE must be auto or stdin" >&2; exit 2 ;;
esac

if [ "${MESSAGE_SOURCE}" = stdin ]; then
  if [ -n "${MESSAGE}" ] || [ -n "${MESSAGE_FILE}" ]; then
    echo "run_agent_turn: stdin mode refuses inherited MESSAGE/MESSAGE_FILE" >&2
    exit 2
  fi
  MESSAGE="$(cat)"
...
if [ -n "${MESSAGE_FOR_SESSION}" ]; then
  ...
  if [ "${derived_selector}" != "agent:${TARGET_AGENT}:main" ]; then
    AUTO_SCOPED_SESSION_SELECTOR="${derived_selector}"
  fi
  unset derived_selector
fi

TARGET_SESSION_KEY_EFFECTIVE=""
TARGET_SESSION_ID_EFFECTIVE=""
if [ -n "${TARGET_SESSION_KEY:-}" ]; then
  TARGET_SESSION_KEY_EFFECTIVE="${TARGET_SESSION_KEY}"
  TARGET_SESSION_SELECTOR_SOURCE="TARGET_SESSION_KEY"
elif [ -n "${TARGET_SESSION_ID:-}" ]; then
  case "${TARGET_SESSION_ID}" in
    agent:${TARGET_AGENT}:*)
      TARGET_SESSION_KEY_EFFECTIVE="${TARGET_SESSION_ID}"
      TARGET_SESSION_SELECTOR_SOURCE="TARGET_SESSION_ID(legacy-key-alias)"
      ;;
    *)
      TARGET_SESSION_ID_EFFECTIVE="${TARGET_SESSION_ID}"
      TARGET_SESSION_SELECTOR_SOURCE="TARGET_SESSION_ID"
      ;;
  esac
else
  if [ -n "${AUTO_SCOPED_SESSION_SELECTOR}" ]; then
    TARGET_SESSION_KEY_EFFECTIVE="${AUTO_SCOPED_SESSION_SELECTOR}"
  else
    TARGET_SESSION_KEY_EFFECTIVE="agent:${TARGET_AGENT}:main"
  fi
  TARGET_SESSION_SELECTOR_SOURCE="auto"
fi
if [ -n "${AUTO_SCOPED_SESSION_SELECTOR}" ] \
    && [ -z "${TARGET_SESSION_ID_EFFECTIVE}" ] \
    && [ "${TARGET_SESSION_KEY_EFFECTIVE}" = "agent:${TARGET_AGENT}:main" ]; then
  TARGET_SESSION_KEY_EFFECTIVE="${AUTO_SCOPED_SESSION_SELECTOR}"
  TARGET_SESSION_SELECTOR_SOURCE="auto"
fi
TARGET_SESSION_SELECTOR="${TARGET_SESSION_KEY_EFFECTIVE:-${TARGET_SESSION_ID_EFFECTIVE}}"
...
unset AUTO_SCOPED_SESSION_SELECTOR
...
(
  ...
  printf '%s' "${MESSAGE}" \
    | env \
        ...
        OPENCLAW_TARGET_SESSION_KEY="${TARGET_SESSION_KEY_EFFECTIVE}" \
...
jq -nc \
  ...
  --arg child_session_key "${TARGET_SESSION_KEY_EFFECTIVE}" \
  --arg target_session_id "${TARGET_SESSION_ID_EFFECTIVE}" \
  --arg run_id "${RUN_ID}" \
  ...
  '{
    ...
    target_session_id: ($target_session_id | select(. != "") // null),
    run_id: $run_id,
    exit_code: $exit_code,
