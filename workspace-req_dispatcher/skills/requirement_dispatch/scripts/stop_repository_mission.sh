...
normalize_project() {
  local target="$1" prefix path decoded segment
  local -a segments=()
  target="${target%%\#*}"
  target="${target%%\?*}"
  ...
  case "${target}" in
    http://*|https://*)
      prefix="${GITLAB_API_PROTOCOL}://${GITLAB_HOST}/"
...
CANONICAL_COMMAND="/mission-stop ${PROJECT_FULL}"
TURN_RESULT="$(TARGET_AGENT="${EXECUTOR_AGENT}" MESSAGE="${CANONICAL_COMMAND}" \
  bash "${RUN_AGENT_TURN_CMD}")" || mission_stop_failure "executor mission stop transport failed"
...
WORKER_RESULT="$(jq -ce --arg project "${PROJECT_FULL}" '
  if (keys | sort) == [
    ...
    and (.stop_id | type == "string" and test("^mission-stop-[A-Za-z0-9._-]+$"))
...
' <<<"${WORKER_RESULT}" 2>/dev/null)" \
  || mission_stop_failure "executor returned an invalid mission stop result"
