...
state 与磁盘布局：[`references/state_schema.md`](references/state_schema.md)。跨 agent 原语与回调的确切形态（待对齐）：[`references/trigger_command.md`](references/trigger_command.md)。git_issuer I/O 契约（待对齐）：[`references/gitissuer_contract.md`](references/gitissuer_contract.md)。
...
1. **解析回调**里 git_issuer 的终态输出：成功（带 issue IID/URL）或失败（带原因）。字段位置见 [`references/gitissuer_contract.md`](references/gitissuer_contract.md)。
...
- git_issuer I/O 契约（待对齐）：[`references/gitissuer_contract.md`](references/gitissuer_contract.md)。
