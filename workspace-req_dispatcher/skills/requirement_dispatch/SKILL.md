...
description: "[SKILL_VERSION=2026-07-08.6] Orchestrate the ZhiBan/OpenClaw requirement → GitLab issue → durable req_executor FIFO queue from the 104 side. Intake supports either a blue-zone GitLab wiki URL or free-text requirements with a deterministic project locator (`group/project`, GitLab repository/Wiki URL, or `glab api projects/<encoded-group%2Fproject>/...` snippet): wiki intake uses `scripts/prepare_wiki_downstream_payloads.sh` to parse the wiki URL, fetch wiki Markdown with read-only `WIKI_GITLAB_*` pins when needed, split the document into requirement items, and compose one git_issuer payload per item; wiki and free-text intake may also extract an explicit target branch from natural-language branch directives. req_dispatcher then calls blue-zone `git_issuer` through `scripts/run_agent_turn.sh`, routes every returned `group/project` to `DEFAULT_EXECUTOR_AGENT` unless `routing.env` has a project-specific override, appends the issue and optional target_branch to durable `executor_queue.json`, drains at most one queued executor issue through `RUN_SINGLE_ISSUE`, receives `RUN_EXECUTOR_RESULT_CALLBACK`, clears active queue state, and pushes conclusions back to the originating user without propagating executor Wiki evidence links. req_dispatcher may read GitLab wiki pages only; it must not create issues, write labels/notes, or run issue work directly. Main helpers: source_dispatcher_env.sh, capture_origin.sh, prepare_wiki_downstream_payloads.sh, prepare_downstream_payloads.sh, run_agent_turn.sh, build_executor_payload.sh, enqueue_executor_issue.sh, drain_executor_queue.sh, finish_executor_queue_active.sh, next_correlation_id.sh, find_pending.sh, record_pending.sh, drain_pending.sh, evict_stuck.sh, route_project.sh, notify_user.sh, ops_notify.sh."
allowed-tools: Bash, Read
---

# Requirement Dispatch Skill

这是一个 **端到端编排契约**。`req_dispatcher` 把 114 转发来的需求主动驱动整条「需求 → 自动处理」链路：入口现在优先支持智伴给出的蓝区 GitLab wiki URL，使用 `scripts/prepare_wiki_downstream_payloads.sh` 解析 wiki 对应的 `group/project`、只读拉取 wiki Markdown、拆分需求并生成一组适合 `git_issuer` 的建单消息；若消息不含 wiki URL，则沿用 `scripts/prepare_downstream_payloads.sh` 处理自由文本需求，从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段中确定性提取 project。两种入口都可从入口自然语言中的明确分支指令提取 `target_branch`，作为 executor 的 `branch=` 透传。随后通过 `scripts/run_agent_turn.sh` 调用蓝区 `git_issuer` 建 GitLab issue；拿到 `worker_result_json` 后按 project 选择执行器（所有合法 `group/project` 默认走 `DEFAULT_EXECUTOR_AGENT`，`routing.env` 只做专属覆盖）；再用 `scripts/enqueue_executor_issue.sh` 把 issue 和可选目标分支写入 durable executor FIFO queue；最后由 `scripts/drain_executor_queue.sh` 生成并启动队首 `RUN_SINGLE_ISSUE`、收执行器结果回调、清 active、继续 drain 下一条，并把结论推回发起需求的企微用户。所有确定性的 state 写入（pending 记录、queue 入队/drain、超时驱逐）、消息准备与本地决策（wiki 拆分、路由查表、推送留痕）都在 `scripts/` 下 flock 保护或 best-effort 的 shell 脚本里；LLM 只做脚本干不了的事：按脚本契约组织多条建单/入队/drain、读执行器回调、可选 ops 通知。

**职责边界（HARD）**：身份从「薄派发器」升级为「编排器」，但**仍不写 GitLab**——不得用任何 token 去建 issue、打标签、写 note、跑 issue；建 issue 是 `git_issuer` 的事，跑 issue 是 `req_executor` 的事。新 wiki 入口只允许用 `WIKI_GITLAB_*` 只读配置拉取 wiki 页面内容。编排器可以做受控入口分析：解析 wiki URL、拆分 wiki 文档、剥离 114/origin 包装、从入口自然语言中提取安全 `target_branch`、从自由文本中的 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段确定性提取 project、生成下游消息；但不能语义猜测项目，不能替代 `git_issuer` 的建单/校验职责，issue 事实仍以 `git_issuer` 返回 JSON 为准。编排器多做三步：入口分析并编辑下游消息；git_issuer 成功 → 路由 + 入 executor durable FIFO queue；executor 回调 → 推用户结论 + 清 active + 继续 drain。不去重；git_issuer / executor 返回失败均不自动重试业务（重试由用户重发需求）。详见 [`../../SOUL.md`](../../SOUL.md) §Global Rules 与下方 §No-Fallback。
...
`scripts/drain_executor_queue.sh` 尝试启动队首。队列文件由
...
2. **准备下游消息（wiki 优先，自由文本回退）**：

   - 若需求文本包含 GitLab wiki URL（`http(s)://<host>/<group>/<project>/-/wikis/<slug>`），调用 `prepare_wiki_downstream_payloads.sh`。它会解析 wiki URL 对应的 project、从入口消息里的 `branch=...`、`target_branch=...`、`目标分支：...`、`合到 ...` 等明确指令提取 `target_branch`、用只读 `WIKI_GITLAB_*` 配置拉取 wiki Markdown（本地测试可用 `WIKI_CONTENT` 注入跳过网络）、按 Markdown 标题/编号块拆分需求，并输出一组 `git_issuer_payloads`。`status=failed` 表示 wiki URL、读取、拆分或分支指令失败，**不要调用 git_issuer**；应推用户失败说明并结束本路径。
     ...
     ```json
     {"status":"success","project":"claw_gitlab/px_ifp_hulat_test","target_branch":"release/2026.07","wiki_url":"http://.../-/wikis/product/requirements","wiki_slug":"product/requirements","requirements":[{"ordinal":1,"title":"Login flow","body":"...","wiki_url":"...","wiki_section":"Login flow"}],"git_issuer_payloads":["CREATE_GITLAB_ISSUE\nrepo=claw_gitlab/px_ifp_hulat_test\nsource=req_dispatcher_wiki\n..."],"reason":null}
     ```

   - 若消息不含 wiki URL，调用 `prepare_downstream_payloads.sh` 对自由文本做受控入口分析：剥离 114 包装和 `[origin]`/`origin={...}` 行，从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段中确定性提取 project；可从 `branch=...`、`target_branch=...`、`目标分支：...`、`合到 ...` 等明确指令提取 `target_branch` 并从需求正文中剥离该路由指令；最后生成单条给 `git_issuer` 的标准化消息。`status=failed` 表示信息不足或形态不合格，**不要调用 git_issuer**；应推用户失败说明并结束本路径。
   ...
   后续步骤统一按 payload 列表处理：wiki 成功时遍历 `.git_issuer_payloads[]`；自由文本成功时把 `.git_issuer_payload` 当成长度为 1 的列表。每个 payload 独立建 issue、路由并入 executor queue。入口准备输出的 `.target_branch` 非空时，入队必须作为 `TARGET_BRANCH` 传给 `enqueue_executor_issue.sh`。wiki 拆出的多个条目顺序处理，不并发建单。
...
4. **调用蓝区 `git_issuer`**：用 `scripts/run_agent_turn.sh` 包装 `openclaw agent`。payload 是第 2 步生成的当前建单消息：wiki 入口为 `.git_issuer_payloads[n]`，自由文本入口为 `.git_issuer_payload`；不要把 114 原文直接发给 git_issuer。该脚本返回结构化 envelope：`{status,run_id,child_session_key,exit_code,worker_result_json,raw_output}`；`worker_result_json` 优先来自 git_issuer 输出中的最后一行紧凑 JSON，也兼容 Markdown 代码块里的 pretty JSON object（成功时含 `project`/`issue_iid`/`issue_url`）。
...
8. **enqueue executor issue（不直接启动）**：把 git_issuer 成功创建的 issue 追加到 durable executor FIFO 队尾。`EXECUTOR_AGENT` 使用第 7 步 `route_project.sh` 的 stdout。`ORIGIN_JSON` 必须沿用接入路径捕获到的 origin；手动入口为空即可。
...
10. **尝试启动队首 executor issue**：每个接入路径在 enqueue 后调用一次 drain；如果已有 active executor，则返回 `busy`，新 issue 保持排队。若没有 active，则启动队首。这个脚本内部生成 `correlation_id`、调用 `build_executor_payload.sh`、通过 `run_agent_turn.sh` 调 executor、并记录 executor pending。
...
11. **回最小受理 ack** 给 114（文案见 [`../../USER.md`](../../USER.md)，如"需求已受理，正在创建 issue 并自动处理，结果稍后通知"），返回 `queued_for_executor` 或 `waiting_for_executor_callback`。issue 创建和入队已经完成；若 drain 返回 `launched`，处理结果会经 executor 回调异步返回；若返回 `busy`，后续回调或周期性 `RUN_EXECUTOR_QUEUE_DRAIN` 会继续启动。
...
| `prepare_wiki_downstream_payloads.sh` | — | `MESSAGE` 或 `MESSAGE_FILE` 或 stdin；`FETCH_WIKI=1` 时读取 `WIKI_GITLAB_HOST`/`WIKI_GITLAB_API_PROTOCOL`/`WIKI_GITLAB_TOKEN`/`WIKI_GLAB_BIN`（也兼容同名 `GITLAB_*`/`GLAB_BIN` 测试覆盖）；`WIKI_CONTENT` 可用于本地测试跳过 glab。stdout：`{status,project,target_branch,wiki_url,wiki_slug,requirements,git_issuer_payloads,reason}`；`status=success` 时 `target_branch` 是从入口消息明确分支指令中提取出的安全 Git ref 名或 `null`，`git_issuer_payloads` 是一组顺序处理的建单消息；`status=failed` 时不调用 git_issuer。入参文件缺失 exit 2。 |
| `prepare_downstream_payloads.sh` | — | `MESSAGE` 或 `MESSAGE_FILE` 或 stdin。stdout：`{status,project,target_branch,requirement_text,git_issuer_payload,reason}`；`status=success` 时 `project` 是从文本里的 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段确定性提取的 `group/project`；`target_branch` 是从明确分支指令中提取出的安全 Git ref 名，缺省为 `null`；`git_issuer_payload` 是发给 git_issuer 的标准化消息；`status=failed` 时不调用 git_issuer，推用户失败说明后结束。入参文件缺失 exit 2。 |
...
| `build_executor_payload.sh` | `PROJECT`, `IID`, `CORRELATION_ID` | `DISPATCHER_CALLBACK_TARGET`, `TARGET_BRANCH`/`BRANCH`。stdout：完整 `RUN_SINGLE_ISSUE` 多行触发文本；`TARGET_BRANCH` 非空时追加 `branch=<target>`；project 形态错、IID 非正整数、分支名不安全或缺必填项 exit 2。 |
| `enqueue_executor_issue.sh` | `STATE_ROOT`, `PROJECT`, `IID`, `EXECUTOR_AGENT` | `ISSUE_URL`, `ORIGIN_JSON`, `REQ_DIGEST`, `TARGET_BRANCH`/`BRANCH`。stdout：`{status:"queued",queue_id,queued_count}`；追加到 `${EXECUTOR_QUEUE_FILE}` FIFO 队尾，安全分支名持久化为 `target_branch`。 |
...
- **只做受控入口分析**：wiki 入口必须先用 `prepare_wiki_downstream_payloads.sh` 生成 `git_issuer_payloads`；自由文本入口必须先用 `prepare_downstream_payloads.sh` 从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段确定性提取 project 并生成 `git_issuer_payload`。若 wiki URL/读取/拆分失败，或自由文本无法确定 project，推用户失败说明并停止，不调用 git_issuer。不语义猜 project、不补写 project、不把 114/origin 包装原样发给 git_issuer。最终 issue 事实仍以 git_issuer 返回的 `project`/`issue_iid`/`issue_url` 为准。
...
orchestrator 每轮只回一条紧凑状态摘要：接入路径 → `{path:"intake", git_issuer_run_id, outcome, project?, iid?, queue_id?, queue_drain_status?}`；executor 回调路径 → `{path:"executor_cb", run_id, status, iid?, notified, queue_finish_status?, queue_drain_status?}`；队列 drain 路径 → `{path:"executor_queue_drain", status, active?, queued_count?}`。详细证据只落 disk（`ledger.jsonl` / `executor_queue.json`），不进 chat。
