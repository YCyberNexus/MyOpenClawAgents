...
3. **跨 agent 异步 spawn `git_issuer`**：用跨 agent spawn 原语（形态类 `sessions_spawn`、指定目标 `${GIT_ISSUER_AGENT}`、异步回调；确切工具名/参数见 trigger_command.md），payload 极简：`{requirement_text: <需求原文>}`。默认**不**附 `correlation_id`（匹配走 `run_id`）；回显模式是仅当对齐后确认回调不带 `run_id` 才启用的退化路径，其 `correlation_id` 生成机制尚未实现、列为待对齐（见 trigger_command.md §匹配），**未对齐前不要自行生成 token**。
   - **失败重试**（no-fallback）：同 payload 最多 3 次、2s 固定退避。三次仍失败 → 视为 `launch_failed`：调 `drain_pending.sh`（`OUTCOME=launch_failed`、`REASON=<最后一次原始错误>`、`RUN_ID="launch-fail-$(date -u +%s)"`——此处用时间戳作审计键是允许的，因为它**不进 pending、无匹配语义**）写 ledger，**不写 pending**；随后**可选 ops 通知**（见 §可选 ops 通知，`EVENT=launch_failed RUN_ID=<同上 launch-fail key> REASON=<最后错误>`）。本路径结束。
...
5. **失败** → 上面 drain 已写 `failed` ledger；随后**可选 ops 通知**（见 §可选 ops 通知，`EVENT=git_issuer_failed RUN_ID=<run_id> REASON=<失败原因>`）。**不回企微用户、不自动重试**（重试由用户重发需求）。
6. 返回单条紧凑状态。
...
- `launch_failed`（接入路径 spawn 三次仍败）：`EVENT=launch_failed RUN_ID=<launch-fail key> REASON=<最后错误>`。
- `git_issuer_failed`（回调报失败）：`EVENT=git_issuer_failed RUN_ID=<run_id> REASON=<reason>`。
- `stuck_evicted`（接入路径开头 `evict_stuck` 驱逐到 `>0` 条）：`EVENT=stuck_evicted COUNT=<驱逐数>`。
...
`STATE_ROOT` / `STUCK_AFTER_MINUTES` / `OPS_NOTIFY_CHANNEL` 来自 `config/dispatcher.env`（`source` 即得）。`ops_notify.sh` 不碰 state（不读写 pending/ledger/锁），只发 best-effort 告警。
