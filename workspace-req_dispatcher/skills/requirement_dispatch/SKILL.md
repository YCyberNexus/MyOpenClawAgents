...
`/mission-stop group/project`（附带仅供跨 agent 持久回执寻址的随机 nonce）。它优先核对
本轮 exec 工具产生的严格 JSON，并可从同 nonce 对应的 executor durable archive 恢复已经
提交但最终回复丢失的回执。只有 executor 返回或持久化了严格 success 后，wrapper 才在 dispatcher
