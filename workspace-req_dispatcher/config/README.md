...
| `REPLY_NOTIFY_WATCHDOG_GRACE_SECONDS` | 否 | Gateway 握手、终态响应和进程收尾宽限秒数，默认 `35`，必须是不小于 `31` 的整数，以覆盖协议适配器内部 `30` 秒终态等待宽限。外层 watchdog 总预算为 `REPLY_NOTIFY_TIMEOUT_SECONDS + REPLY_NOTIFY_WATCHDOG_GRACE_SECONDS`，避免在114接收 agent 到达业务超时前提前杀死协议适配器。 |
...
| 跨 agent 调用契约 | 已定 | `scripts/run_agent_turn.sh` 对需要结构化结果的调用固定走 Gateway helper：在目标 session 锁内记录调用前 transcript 游标，只从本轮新增的成功 `exec` toolResult 中提取唯一 JSON object，找不到或有歧义即失败关闭，因此模型最终散文不会覆盖 wrapper 回执。正文始终从 stdin 进入，绝不进入 argv。普通调用默认 `agent:<target>:main`，`RUN_SINGLE_ISSUE` 默认 `agent:<target>:issue-<sanitized-project>-<iid>`；真正的 `TARGET_SESSION_ID` 在目标 registry 中唯一精确解析，历史完整 `agent:*` 值仅作为 key alias；`RUN_SINGLE_ISSUE` 显式 main 仍会改投 issue 级 session。Gateway 地址/token 由 runner 的本地 OpenClaw 配置管理。 |
...
- 本次 I1 schema 滚动升级必须先暂停新的执行请求并排空或停止旧 req_executor，再部署并验证新版 req_executor，最后才升级 req_dispatcher。旧 executor 使用严格 I1 字段白名单，不认识 `auto_merge` 与 `merge_target_branch`；新版 dispatcher 对普通请求也固定发送 `auto_merge=false`，因此任何新版 I1 都不得发往旧 executor，自动合并请求更不得在旧 executor 上尝试。回滚时先停新入口和双方 tick，先回滚 dispatcher 或保持新版 executor，确认不会再发送新字段后才能回滚 executor；未完成自动合并 intent 必须保留等待兼容版本恢复，不得降级投递。
