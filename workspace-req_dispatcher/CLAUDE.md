...
`req_dispatcher` 是"企微需求 → 自动处理"链路在 104 侧的**统一接入点 + 端到端编排器**：接收 114 转发的需求消息。新主入口是智伴给出的蓝区 GitLab wiki URL，先用 `prepare_wiki_downstream_payloads.sh` 解析 wiki 对应 project、只读拉取 wiki Markdown、拆分需求并生成多条给 `git_issuer` 的建单消息；自由文本入口仍用 `prepare_downstream_payloads.sh` 剥离 114/origin 包装，从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段中确定性提取 project、生成单条建单消息。随后通过 `scripts/run_agent_turn.sh` 调用蓝区 `git_issuer` 建 issue → 按 project 选择目标 `req_executor` 部署（所有合法 `group/project` 默认路由到 `DEFAULT_EXECUTOR_AGENT`，`routing.env` 只做覆盖）→ 把 issue 追加到 durable executor FIFO queue → 由 `drain_executor_queue.sh` 生成 `RUN_SINGLE_ISSUE` 并启动队首 executor issue → 收执行器结果回调 → 清 active 并继续 drain 下一条 → 把结论推回发起需求的企微用户。身份从"薄派发器"升级为"编排器"。

**仍明确不做**：不建 issue、不打标签、不写 GitLab note、不自己跑 issue、不去重、git_issuer/executor 业务失败不自动重试。**新增会做**：用 `WIKI_GITLAB_*` 只读拉取 wiki 内容、受控入口分析和下游消息准备、两段下游 agent 调用（git_issuer + 按队列启动的 executor）、按 project 路由、维护 durable executor FIFO queue、终态把处理结论推回企微用户（仅一次）。project 不能靠语义猜测，issue 事实仍以 git_issuer 返回 JSON 为准。
...
唯一 SKILL：`skills/requirement_dispatch/`，编排器固定 session `agent:req_dispatcher:main`。一条需求经历两段下游 agent 调用：git_issuer 段同轮 record/drain 作审计，executor issue 先进入 durable FIFO queue；队首启动成功后记录 executor pending 等待后续结果回调。

- **接入路径（A）**（114 投来需求消息）：`capture_origin.sh` 捕获 origin（优先 OpenClaw 网关/运行时来源元数据，其次正文 `[origin]` 行；含回推目标 `reply_agent`）→ wiki URL 消息走 `prepare_wiki_downstream_payloads.sh` 生成 `git_issuer_payloads[]`；自由文本走 `prepare_downstream_payloads.sh` 从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段中确定性提取 project 并生成单条 `git_issuer_payload`（无法确定 project 则推用户失败并停）→ `evict_stuck.sh` 兜底 → 对每个 payload 顺序 `run_agent_turn.sh` 调用蓝区 `git_issuer`（同 payload 失败 3 次 2s 退避）→ `record_pending.sh` 记 git_issuer 审计 stage → 解析 `{status,project,iid,url}` → 成功则 `route_project.sh` 选 executor（默认执行器覆盖所有合法 project）→ `enqueue_executor_issue.sh` 把 issue 追加到 durable executor FIFO → drain git_issuer 段 → `drain_executor_queue.sh` 尝试启动队首 → 回最小 ack。executor 启动成功由 queue drain 记录 `stage=executor` pending；已有 active 时新 issue 保留排队。
