...
`req_dispatcher` 是"企微需求 → 自动处理"链路在 104 侧的**统一接入点 + 端到端编排器**：接收 114 转发来的需求消息。新主入口是智伴给出的蓝区 GitLab wiki URL，先用 `prepare_wiki_downstream_payloads.sh` 解析 wiki 对应 project、只读拉取 wiki Markdown、拆分需求并生成多条给 `git_issuer` 的建单消息；旧自由文本入口仍用 `prepare_downstream_payloads.sh` 剥离 114/origin 包装、要求文本里明确写出 GitLab `group/project`、生成单条建单消息。随后通过 `scripts/run_agent_turn.sh` 调用蓝区 `git_issuer` 建 issue → 按 project 选择目标 `req_executor` 部署（所有合法 `group/project` 默认路由到 `DEFAULT_EXECUTOR_AGENT`，`routing.env` 只做覆盖）→ 把 issue 追加到 durable executor queue → 由 `drain_executor_queue.sh` 生成 `RUN_SINGLE_ISSUE` 并在可配置并发槽内尽量启动 eligible executor issue → 收执行器结果回调 → 清匹配 active 槽并继续 drain 后续批次 → 把结论推回发起需求的企微用户。身份从"薄派发器"升级为"编排器"。
...
- **接入路径（A）**（114 投来需求消息）：`capture_origin.sh` 捕获 origin（优先 OpenClaw 网关/运行时来源元数据，其次正文 `[origin]` 行；含回推目标 `reply_agent`）→ wiki URL 消息走 `prepare_wiki_downstream_payloads.sh` 生成 `git_issuer_payloads[]`；旧自由文本走 `prepare_downstream_payloads.sh` 生成单条 `git_issuer_payload`（缺明确 project 则推用户失败并停）→ `evict_stuck.sh` 兜底 → 对每个 payload 顺序 `run_agent_turn.sh` 调用蓝区 `git_issuer`（同 payload 失败 3 次 2s 退避）→ `record_pending.sh` 记 git_issuer 审计 stage → 解析 `{status,project,iid,url}` → 成功则 `route_project.sh` 选 executor（默认执行器覆盖所有合法 project）→ `enqueue_executor_issue.sh` 把 issue 追加到 durable queue → drain git_issuer 段 → `drain_executor_queue.sh` 按 batch 上限填充可用 active 槽 → 回最小 ack。executor 启动成功由 queue drain 记录 `stage=executor` pending；active 槽满或 origin 到达上限时新 issue 保留排队。
