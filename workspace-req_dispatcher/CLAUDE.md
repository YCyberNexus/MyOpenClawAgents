...
`req_dispatcher` 是"企微需求 → 自动处理"链路在 104 侧的**统一接入点 + 端到端编排器**：接收 114 转发的需求消息。新主入口是智伴给出的蓝区 GitLab wiki URL，先用 `prepare_wiki_downstream_payloads.sh` 解析 wiki 对应 project、只读拉取 wiki Markdown、拆分需求并生成多条给 `git_issuer` 的建单消息；自由文本入口仍用 `prepare_downstream_payloads.sh` 剥离 114/origin 包装，从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段中确定性提取 project、生成单条建单消息；两种入口都可从明确分支指令提取可选 `target_branch`。随后通过 `scripts/run_agent_turn.sh` 调用蓝区 `git_issuer` 建 issue → 按 project 选择目标 `req_executor` 部署（所有合法 `group/project` 默认路由到 `DEFAULT_EXECUTOR_AGENT`，`routing.env` 只做覆盖）→ 把 issue 和可选目标分支追加到 durable executor queue → 由 `drain_executor_queue.sh` 生成 `RUN_SINGLE_ISSUE` 并在可配置并发槽内尽量启动 eligible executor issue → 收执行器结果回调 → 清匹配 active 槽并继续 drain 后续批次 → 把结论推回发起需求的企微用户。身份从"薄派发器"升级为"编排器"。

**仍明确不做**：不建 issue、不打标签、不写 GitLab note、不自己跑 issue、不去重、git_issuer/executor 业务失败不自动重试。**新增会做**：用 `WIKI_GITLAB_*` 只读拉取 wiki 内容、受控入口分析和下游消息准备、两段下游 agent 调用（git_issuer + 按队列启动的 executor）、按 project 路由、维护 durable executor queue、终态把处理结论推回企微用户（仅一次）。project 不能靠语义猜测，issue 事实仍以 git_issuer 返回 JSON 为准。
...
唯一 SKILL：`skills/requirement_dispatch/`，编排器结果回调和恢复 drain 固定 session `agent:req_dispatcher:main`；入口流量可由 114 按用户或会话投到同一 agent 的稳定 intake session，不能为一千名员工复制一千个 agent 部署。一条需求经历两段下游 agent 调用：git_issuer 段同轮 record/drain 作审计，executor issue 先进入 durable queue；`drain_executor_queue.sh` 在 `EXECUTOR_QUEUE_MAX_ACTIVE` 与 `EXECUTOR_QUEUE_MAX_ACTIVE_PER_ORIGIN` 控制下按 batch 上限启动 eligible issue，并记录 executor pending 等待后续结果回调。

- **接入路径（A）**（114 投来需求消息）：`capture_origin.sh` 捕获 origin（优先 OpenClaw 网关/运行时来源元数据，其次正文 `[origin]` 行；含回推目标 `reply_agent`）→ wiki URL 消息走 `prepare_wiki_downstream_payloads.sh` 生成 `git_issuer_payloads[]` 并可提取 `target_branch`；自由文本走 `prepare_downstream_payloads.sh` 从 `group/project`、GitLab 仓库/Wiki URL，或 `glab api projects/<encoded-group%2Fproject>/...` 片段中确定性提取 project、可选 `target_branch` 并生成单条 `git_issuer_payload`（无法确定 project 则推用户失败并停）→ `evict_stuck.sh` 兜底 → 对每个 payload 顺序 `run_agent_turn.sh` 调用蓝区 `git_issuer`（同 payload 失败 3 次 2s 退避）→ `record_pending.sh` 记 git_issuer 审计 stage → 解析 `{status,project,iid,url}` → 成功则 `route_project.sh` 选 executor（默认执行器覆盖所有合法 project）→ `enqueue_executor_issue.sh` 把 issue 和可选目标分支追加到 durable queue → drain git_issuer 段 → `drain_executor_queue.sh` 按 batch 上限填充可用 active 槽 → 回最小 ack。executor 启动成功由 queue drain 记录 `stage=executor` pending；active 槽满或 origin 到达上限时新 issue 保留排队。
- **executor 回调路径（B）**：解析结果信封(I2) → 按 executor `run_id` 匹配 pending，回调缺 `run_id` 时按 `correlation_id` 反查（`correlation_id` 二次校验）→ `notify_user.sh` 把结论推回 origin → drain executor 段 → `finish_executor_queue_active.sh` 清匹配 active 槽 → `drain_executor_queue.sh` 继续填充可用槽。
- **executor 队列恢复路径（C）**：周期性 `RUN_EXECUTOR_QUEUE_DRAIN` 先调用 `evict_stuck.sh`，再调用 `drain_executor_queue.sh`，用于清理超时 pending 对应的 active 槽、恢复启动中断、重试到期的 launch_failed active、或在队列非空且还有空闲槽时继续推进。

完整算法见 [`skills/requirement_dispatch/SKILL.md`](skills/requirement_dispatch/SKILL.md)。

## State 布局

由 `scripts/env_paths.sh` 从 `STATE_ROOT` 派生：`${STATE_ROOT}/_dispatcher/` 下 `pending.json`（run_id 主键，flock 保护）/ `executor_queue.json`（executor durable queue，`active[]` + `queue`，复用同一锁）/ `ledger.jsonl`（append-only 审计）/ `pending.lock` / `seq` / `log/`。schema：[`skills/requirement_dispatch/references/state_schema.md`](skills/requirement_dispatch/references/state_schema.md)。
...
- executor 启动失败只由 `drain_executor_queue.sh` 对同一 `RUN_SINGLE_ISSUE` payload 做 3 次 2s 退避；耗尽后保留对应 active 槽为 `launch_failed`，等待后续 `RUN_EXECUTOR_QUEUE_DRAIN` 继续重试，不清 active、不丢 issue。
