...
`req_dispatcher` 是"企微需求 → 自动处理"链路在 104 侧的**统一接入点 + 端到端编排器**：接收 114 转发的需求消息。新主入口是智伴给出的蓝区 GitLab wiki URL，先用 `prepare_wiki_downstream_payloads.sh` 解析 wiki 对应 project、只读拉取 wiki Markdown、拆分需求并生成多条给 `git_issuer` 的建单消息；旧自由文本入口仍用 `prepare_downstream_payloads.sh` 剥离 114/origin 包装、要求文本里明确写出 GitLab `group/project`、生成单条建单消息。随后通过 `scripts/run_agent_turn.sh` 调用蓝区 `git_issuer` 建 issue → 按 project 选择目标 `req_executor` 部署（所有合法 `group/project` 默认路由到 `DEFAULT_EXECUTOR_AGENT`，`routing.env` 只做覆盖）→ 把 issue 追加到 durable executor FIFO queue → 由 `drain_executor_queue.sh` 生成 `RUN_SINGLE_ISSUE` 并启动队首 executor issue → 收执行器结果回调 → 清 active 并继续 drain 下一条 → 把结论推回发起需求的企微用户。身份从"薄派发器"升级为"编排器"。
