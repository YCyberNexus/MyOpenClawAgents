...
该命令修改默认 `req_executor` 的共享并行仓库数上限，默认是 10。所有 batch session 共同
生效，不是每个 session 各自设置。不同仓库可并行。
在线调低时不会取消已有任务；如果当前活跃仓库数高于新上限，会停止发放新仓库槽位并等待
自然回落。

每个仓库内部的 Issue 并发数也可独立调整，默认是 1（串行）：

```text
/repo-slot 2
```

该命令对默认 `req_executor` 下的所有仓库和 batch session 共同生效。在线调低不会取消已启动
任务；后续 tick 会把超出新上限、尚未启动的 reservation 安全退回 pending，并等待已启动任务
自然回落。
...
基准分支而未写合并目标，则合并到该处理基准分支；若两者都未指定，则 executor 先读取每个
Issue 的基准分支声明，没有声明时再使用仓库默认分支，MR 目标跟随该最终处理基准。只有 GitLab
端对精确 MR、源分支、目标分支和提交 SHA 的合并结果验证通过后，Issue
