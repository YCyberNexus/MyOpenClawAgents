...
- **接入路径（A）**（114 投来需求消息）：`capture_origin.sh` 捕获 origin 元数据（优先 OpenClaw 网关/运行时来源元数据，其次正文 `[origin]` 行；含回推目标 `reply_agent`）→ wiki URL 消息走 `prepare_wiki_downstream_payloads.sh` 生成 `git_issuer_payloads[]`；旧自由文本走 `prepare_downstream_payloads.sh` 生成单条 `git_issuer_payload`（缺明确 project 则推用户失败并停）→ `evict_stuck.sh` 回收泄漏 pending → 对每个 payload 顺序 `run_agent_turn.sh` 调用蓝区 `git_issuer`（payload 为准备后的建单消息）→ `record_pending.sh` 记一条 git_issuer 审计 stage → 解析 `{status,project,iid,url}` → 成功则 `route_project.sh` 选 executor（默认 `DEFAULT_EXECUTOR_AGENT` 覆盖所有合法 project）→ `enqueue_executor_issue.sh` 追加 durable FIFO → drain git_issuer stage → `drain_executor_queue.sh` 尝试启动队首 → 回最小受理 ack；git_issuer 失败则推用户"建 issue 失败" + drain。
...
4. **队列必须可恢复且不能丢待执行 issue**：`executor_queue.json` 是待执行 issue 的 durable FIFO。新 issue 只追加队尾；active 未完成时不得启动后续 issue；回调清 active 后必须再次 drain。OpenClaw 会话被用户或运行时中断时，下一次 `RUN_EXECUTOR_QUEUE_DRAIN` 必须能从 disk 恢复。
