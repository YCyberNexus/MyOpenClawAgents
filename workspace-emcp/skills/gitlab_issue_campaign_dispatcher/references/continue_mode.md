...
1. **Read the issue comments.** Every attempt reads all non-system comments and passes them to Claude Code. Attempt summaries stay local under `${ISSUE_ROOT}/summary.md` and are not posted as comments. Continue mode still recognizes historical `<!-- acpx_auto_tester:attempt-summary ... -->` and legacy pre-rename markers left by older deployments, while excluding agent-posted Wiki artifact notes from task guidance.
...
3. **Flip the issue's label to `continue`.** The issue currently carries `pr` (the stable completion label). Replace it with `continue`. The agent triggers continue mode on the `continue` label. It also tolerates the legacy misspelling `contiune`, but new human action should use `continue`. You may optionally add `quality:low` at the same time to signal that the prior run's output quality was low and a stronger model should be used on the next attempt.
...
3. **Dispatcher prep:** Runs `scripts/build_prompt.sh` which reads the issue notes (`G1b`) and **partitions them in two buckets**:
   - **Historical attempt summaries** — old notes whose body contains `<!-- acpx_auto_tester:attempt-summary ` or the legacy pre-rename summary marker. Current deployments no longer create these notes.
   - **Issue comments** — every other non-system note except auto-posted Wiki artifact notes (`<!-- acpx_auto_tester:attempt-wiki-artifacts ... -->`) and legacy pre-rename Wiki artifact notes. This block is injected in both fresh and continue modes.
...
5. **Subagent:** Runs Claude Code via the same script-owned one-shot invocation used in all modes: `scripts/run_acpx_attempt.sh` `cd`s into `${WORKTREE_DIR}` (the shared per-issue worktree) and runs `acpx --auth-policy skip claude exec -f "${LOG_DIR}/prompt.txt"`. Same No-Fallback Policy applies. Continue-mode continuity comes from four sources: the checked-out `origin/${WORK_BRANCH}` contents, refreshed shared config paths from `origin/${DEV_BRANCH}`, the prompt's historical-summary and issue-comments sections, and the restored same-IID runtime subtree. Fresh-mode runs deliberately quarantine same-IID runtime residue before the new acpx invocation.
6. **Subagent:** After the attempt finishes (terminal status, any of done / blocked-cc / blocked-dispatcher / failed-cc / failed-dispatcher / timeout; legacy no_changes is normalized to blocked-cc), `scripts/summarize_attempt.sh` always writes `${SUMMARY_FILE}` locally and reports `summary_posted=false`; it never posts an issue comment. Failure paths also do not publish Wiki evidence. Cumulative continue-mode runs increment `state.json.continue_count`, which is evaluated by `resolve_model_tier` on the next scheduling tick as a soft trigger for model upgrade when `continue_count >= continue_upgrade_threshold`.
...
same-IID work branch or local prior-attempt branch, refreshes shared config
paths from the latest dev branch, and restores prior files under
<RESULT_BASENAME>/issue-<iid>/ so you can inspect them and
continue. Read what's already there, then continue or correct it according
to the historical summaries and current issue comments below.
...
- Source baseline:            same-IID work branch/local prior-attempt branch, with shared config paths refreshed from latest <dev-branch>
...
In fresh mode the prompt omits only the historical-summary section, retains the issue-comments section, and uses a fresh-mode preamble.

## What if there are no issue comments?

If `G1b` returns zero task-relevant non-system notes after filtering historical summaries and artifact links, the dispatcher's `build_prompt.sh`:

- Builds the prompt with the issue-comments section saying `(no issue comments)`.
- Continues normally; the dispatcher records `no_reviewer_comments=true` in the current-attempt state for operator awareness.

The agent MUST NOT block, abort, or refuse to run just because issue comments are missing.

## What if there are no prior attempt summaries either?

This is expected for attempts produced after summaries became local-only. It means the issue has no historical `acpx_auto_tester:attempt-summary` or legacy pre-rename summary notes from older deployments. Other possible causes include:

- The label was flipped to `continue` before any prior attempt actually ran.
- Historical summary comments were removed manually.

In this case the prompt's "Historical attempt summaries" section says `(no historical agent-posted summaries; inspect the issue branch's existing commits and diff for prior work)`. The agent still runs.
