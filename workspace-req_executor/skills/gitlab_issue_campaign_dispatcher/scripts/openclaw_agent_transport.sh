...
validate_session_key() {
  case "$1" in
    ...
    *) echo "openclaw_agent_transport: session key does not belong to target agent" >&2; exit 65 ;;
  esac
}
...
resolve_existing_session_key() {
  ...
  matches="$(jq -c --arg id "${session_id}" '[to_entries[] | select(.value.sessionId == $id) | .key | select(startswith("agent:"))]' "${registry}")" || {
...
if [ -n "${OPENCLAW_TARGET_SESSION_KEY}" ]; then validate_session_key "${OPENCLAW_TARGET_SESSION_KEY}"; fi
...
has_option() { printf '%s\n' "${AGENT_HELP}" | grep -Eq "^[[:space:]]+$1([[:space:]]+<[^>]+>)?([[:space:]]|$)"; }
HAS_MESSAGE_FILE=0; HAS_SESSION_KEY=0; HAS_SESSION_ID=0
...
if [[ "${OPENCLAW_BIN}" == */* ]]; then OPENCLAW_BIN_PATH="${OPENCLAW_BIN}"; else OPENCLAW_BIN_PATH="$(command -v "${OPENCLAW_BIN}" 2>/dev/null || true)"; fi
