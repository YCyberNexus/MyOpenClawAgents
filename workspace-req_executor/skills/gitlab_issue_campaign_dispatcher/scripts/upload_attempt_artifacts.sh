...
{
  echo "<!-- req_executor:attempt-wiki-artifacts v1 attempt=${ATTEMPT_NUMBER_PADDED} -->"
  echo "## req_executor attempt ${ATTEMPT_NUMBER_PADDED} OpenClaw logs"
  echo
  ...
  echo
  echo "<!-- /req_executor:attempt-wiki-artifacts -->"
} > "${NOTE_FILE}"
