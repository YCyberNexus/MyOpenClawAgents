...
### Path C — `RUN_SINGLE_ISSUE_TEST` (req_dispatcher-driven single-issue entry, I1)

This is the **driven** entry point: `req_dispatcher` orchestrates one issue
end-to-end and wants this executor to test exactly that one IID, then report
...
| ----- | -------- | ------- |
| `project` | yes | GitLab project slug to test (透传 from the git_issuer callback by req_dispatcher). |
| `iid` | yes | The single issue IID to test (positive integer). |
...
| `dispatcher_callback_target` | yes (I2) | The callback target req_dispatcher reports to. **Exact form 待对齐** (§9) — carried opaquely into `dispatch_origin.json`. |
| `group` | no | Falls back to the `GROUP` pin in `config/campaign_defaults.env` / `gitlab.env` when omitted. |
...
1. cd "${SKILL_DIR}" && bash scripts/dispatch_single_issue.sh <<'TRIGGER_EOF'  → envelope
   ...
   dispatcher_callback_target=<dispatcher_callback_target>   # 待对齐 form
   ...
   #   • sources config/gitlab.env + config/campaign_defaults.env, requires
   #     GITLAB_TOKEN from the environment (token injection 待对齐, §9; never sent
...
**Cross-agent send 原语 = 待对齐** (§9). Until the team aligns the actual send
tool / envelope field names, `notify_dispatcher.sh` records the I2 JSON to
`${WORK_ROOT}/log/dispatcher_callbacks.jsonl` and exits 0 (留痕, never silently
dropped); only a malformed send shape (illegal `STATUS`) exits non-zero. When
`dispatcher_callback_target` is empty it is a no-op (`exit 0`). See
[`references/trigger_command.md`](references/trigger_command.md) §RUN_SINGLE_ISSUE_TEST
