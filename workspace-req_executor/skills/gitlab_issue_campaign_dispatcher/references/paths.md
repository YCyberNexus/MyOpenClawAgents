    ...
    ifp-result/                                          ← ${RESULT_ROOT}; agent runtime root
        ...
        .worktrees/                                      ← ${WORKTREES_ROOT}; per-issue linked worktrees (one per IID)
            issue-<iid>/                                 ← ${WORKTREE_DIR}; acpx cwd; created on attempt 1 via
                ...
                ${RESULT_BASENAME}/issue-<iid>/output/   ← ${OUTPUT_DIR}; Claude Code's issue deliverables (committed; lands in MR; shared across attempts of this IID)
...
1. `git clone -b ${BRANCH}` into `${REPO_PATH}` (uses a tmpfs lock at `/tmp/req_executor.clone.${PROJECT}.lock` since the in-repo lock dir does not exist yet).
...
| `OUTPUT_DIR`              | `${WORKTREE_DIR}/${RESULT_BASENAME}/issue-${ISSUE_IID}/output` | primary committable output directory; lives INSIDE the shared worktree (shared across attempts of this IID) |
...
4. **Per-issue committable deliverables that need a dedicated home go to `${OUTPUT_DIR}`.** `build_prompt.sh` injects this guidance into the Claude Code prompt. Multiple MRs into master never collide because each issue writes into a distinct `${RESULT_BASENAME}/issue-<iid>/output/` subdirectory (relative path inside the worktree).
...
8. Two-branch model. `${BRANCH}` (typically `master`) is the **integration / target** branch — MRs are opened against it; each issue's deliverables accumulate here. `${DEV_BRANCH}` (typically `dev`) is the **clean baseline** — fresh-mode checkouts reset to `origin/${DEV_BRANCH}` so Claude does NOT see past issues' output. Continue mode bases on `origin/${WORK_BRANCH}` (the resumable WIP branch), not `${DEV_BRANCH}`, but still refreshes `.claude/`, `hulat/`, and `${DATA_BASENAME}/` from latest `origin/${DEV_BRANCH}` before running.
