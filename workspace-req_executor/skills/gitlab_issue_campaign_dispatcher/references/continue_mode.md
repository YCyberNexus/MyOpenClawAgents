...
1. **Read the latest auto-posted attempt summary on the issue.** The agent posts a short marked comment for successful attempts with `<!-- req_executor:attempt-summary v2 ... -->`; failure summaries stay local under `${ISSUE_ROOT}/summary.md`. Posted summaries tell you the attempt status, commit SHA when available, MR URL when available, changed-file count / preview, and the runner evidence path for full logs and diffs. For successful push-ready attempts, the agent also posts `<!-- req_executor:attempt-wiki-artifacts v1 ... -->` before MR creation with links to the Wiki pages for prompt/result logs and optional report. Continue mode also recognizes legacy pre-rename markers from earlier runs.
...
6. **Subagent:** After the attempt finishes (terminal status, any of done / blocked-cc / blocked-dispatcher / failed-cc / failed-dispatcher / timeout; legacy no_changes is normalized to blocked-cc), `scripts/summarize_attempt.sh` always writes `${SUMMARY_FILE}` locally. Successful `done` attempts (which end with `pr` replacing `done`) also post that summary as a marked GitLab issue comment (`<!-- req_executor:attempt-summary v2 attempt=NNN -->`) for future continue-mode context. Failure paths keep evidence local only and do not publish Wiki evidence. Cumulative continue-mode runs increment `state.json.continue_count`, which is evaluated by `resolve_model_tier` on the next scheduling tick as a soft trigger for model upgrade when `continue_count >= continue_upgrade_threshold`.
...
This is unusual — it means continue mode triggered on an issue that has no `req_executor:attempt-summary` or legacy pre-rename summary notes from past attempts. Typical causes:
