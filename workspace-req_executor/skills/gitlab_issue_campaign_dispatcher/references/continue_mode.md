...
Fresh mode resets the same local issue branch to the selected clean baseline, while still including the current issue comments in the Claude Code prompt. The issue-local runtime and log paths remain fixed; later runs overwrite current-result evidence rather than moving it into attempt-specific directories. Attempt summaries remain local and are never posted back to the issue.
