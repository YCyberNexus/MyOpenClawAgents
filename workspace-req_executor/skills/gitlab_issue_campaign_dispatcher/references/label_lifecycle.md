...
| `done`     | `pr`       | fixed wrapper + Phase 6 | immediately after ordinary MR creation/rotation, or after a shared member's private current-attempt marker proves the one exact MR is opened at the pushed SHA (Step 7) — `done` is removed and `pr` is added in its place | `set_issue_label.sh add pr` (which also removes `done`); result: `pr` only, `done` absent |
