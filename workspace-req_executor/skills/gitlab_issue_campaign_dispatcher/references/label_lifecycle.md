...
10. **Shared MR recovery is label-safe.** Late binding first writes A's `branch_migration.status:"pending"` checkpoint, then creates `issue/A+C`, closes A's ordinary MR, creates one intent-owned replacement, deletes `issue/A`, and rewrites A's binding; replay resumes those exact identities without rerunning A or duplicating the replacement. C's later shared push writes `mr_finalization.status:"pending"` with the same 64-hex `intent_id`. Missing C evidence leaves `done` in place and retains the exact scheduler claim; identity-only evidence selects the replacement for fresh exact-IID and source-branch uniqueness reads but never authorizes success by itself. Heartbeat recovery only finalizes the MR for the already-pushed SHA. `pr` is added only after those live reads prove exactly one open replacement MR with the role-specific `created|reused` identity, exact branch/target/SHA, current token author, intent marker, and both closing lines. C repeats that live check and also waits for A's current campaign pending drain; an earlier-campaign A is proved through its private completed state and live ref/MR identity. Closed, moved, or foreign replacement identities never authorize another replacement; duplicate, truncated, repeating, or otherwise ambiguous shared MR history becomes an immediate terminal `failed-dispatcher` result and emits the scheduler handoff. A transient GitLab read or final `pr` label failure retains the claim and retries only the checkpoint/live-read/label transition.
