...
- `pr` — **stable completion label.** Applied by the subagent in Step 7 immediately after `create_mr.sh` creates, rotates, or validly reuses the MR; replaces `done` (which is removed in the same operation). An issue carrying `pr` is considered complete by the dispatcher.
...
| `done`     | `pr`       | fixed wrapper + Phase 6 | immediately after ordinary MR creation/rotation, or after a shared member's private current-execution marker proves the one exact MR is opened at the pushed SHA (Step 7) — `done` is removed and `pr` is added in its place | `set_issue_label.sh add pr` (which also removes `done`); result: `pr` only, `done` absent |
...
10. **Shared MR recovery is label-safe.** Late binding first writes the anchor's `branch_migration.status:"pending"` checkpoint, then creates `issue/A+C`, closes A's ordinary MR, creates one intent-owned replacement, deletes `issue/A`, and rewrites A's binding. A multi-head fan-in first freezes every source in `dependency_aggregation`, advances the same replacement to the aggregate SHA, verifies all closing lines, and only then retires auxiliary MRs/refs; replay resumes those identities without rerunning a source or duplicating the replacement. C's later shared push writes `mr_finalization.status:"pending"` with the same 64-hex `intent_id`. Missing C evidence leaves `done` in place and retains the exact scheduler claim; identity-only evidence selects the replacement for fresh exact-IID and source-branch uniqueness reads but never authorizes success by itself. Heartbeat recovery only finalizes the MR for the already-pushed SHA. `pr` is added only after live reads prove exactly one open replacement MR with the role-specific `created|reused` identity, exact branch/target/SHA, current token author, intent marker, and all frozen closing lines. C also waits for every current-campaign source claim to drain; earlier-campaign sources are proved through private completed state and live ref/MR identity. Closed, moved, or foreign replacement identities never authorize another replacement; ambiguous shared MR history becomes terminal `failed-dispatcher` evidence. A transient GitLab read or final `pr` label failure retains the claim and retries only checkpoint/live-read/label work.
...
| ------------------- | ----------------------------------- | ------------------------------------------------- | -------------------------------------- |
| `done` label        | the subagent (Step 5)               | immediately after post-push verification, before MR creation, rotation, or reuse | transient: "agent finished solving; MR finalization in progress" |
| `pr` label          | the subagent (Step 7)               | immediately after `create_mr.sh` creates/rotates an ordinary MR or reuses the exact shared MR; simultaneously removes `done` | stable: "the MR exists for human review; issue is complete from the agent's perspective" |
...
GitLab's native auto-close is triggered by **closing keywords in the MR description**. An ordinary MR contains:

```
Closes #${ISSUE_IID}
```

A shared `issue/A+C` MR instead contains both:
