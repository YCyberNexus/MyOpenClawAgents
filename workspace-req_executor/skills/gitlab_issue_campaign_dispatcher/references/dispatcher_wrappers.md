...
20. **Per-IID prep loop** (sequential). For each IID:
    ...
    8. Render `references/executor_prompt.md` fenced block via
       ...
       sentinels `# REQ_EXECUTOR_EXECUTOR_PROMPT_V1` (opener) and
       `# REQ_EXECUTOR_EXECUTOR_PROMPT_V1_END` (closer), NOT by the
    ...
    9. Sentinel check: rendered first line MUST equal
       `# REQ_EXECUTOR_EXECUTOR_PROMPT_V1`. Mismatch → `prep_blocked`
...
```bash
cd workspace-req_executor/skills/gitlab_issue_campaign_dispatcher
