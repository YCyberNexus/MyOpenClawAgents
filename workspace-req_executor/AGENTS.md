...
- `RUN_CHILD_COMPLETION_CALLBACK` (callback wake-up) — Phase 6 for ONE IID: validate the subagent's compact JSON reply (in `worker_result_json`), sync live labels (completion=`pr` only — `done` is removed when `pr` is added; blocked side-split: `blocked-cc` for CC/subagent failures, `blocked-dispatcher` for dispatcher-synthesized failures; `failed-cc` / `failed-dispatcher` mirror same split; `timeout` is unsplit; entries carry `block_side=cc|dispatcher`), write terminal `${ISSUE_STATE_FILE}` + `${ATTEMPT_STATE_FILE}`, drain `pending_subagents[iid]`, classify into `completed / blocked / failed / timeout_iids`, best-effort clean up the terminal child runtime session when enabled, and — when `result_note_enabled` (default off) — best-effort post a `req_result` note for terminal `done` / `failed` / `timeout` via `post_result_note.sh` (reads the issue's `req_origin` marker; no-op if absent; test-result回报, see `references/glab_commands.md` §G14). The callback path NEVER spawns a replacement subagent.
...
Logical issue subagent name: `issue-<project>-<iid>` (e.g. `issue-px_ifp_hulat-1`). Used for `active_issue_sessions` bookkeeping and human-readable logging only. **The runtime session name is always anonymous** — the orchestrator does not pass any name to `sessions_spawn`. The runtime returns its own auto-generated key (e.g. `agent:req_executor:subagent:<uuid>`) which the orchestrator records into `pending_subagents[iid].child_session_key` for audit and stuck-pending detection. Callbacks are matched back to dispatched IIDs by the `iid` field of the compact JSON, not by the runtime session-key label. Detailed session policy lives in [`SOUL.md`](SOUL.md) §Session Policy.
...
- Each issue's committed spec output is at the worktree-relative path `${RESULT_BASENAME}/issue-<iid>/hulat-spec-issue<iid>/` so MRs into master never collide on file paths. The per-attempt log dir lives alongside it at `${RESULT_BASENAME}/issue-<iid>/log/attempt-NNN/` (still attempt-scoped inside the shared per-issue worktree so successive attempts do not overwrite each other's evidence): `stage_and_guard.sh` force-adds `prompt.txt` and `claude_result.txt` from there into the MR, while the bulkier files (`acpx_raw.log`, `git_status.txt`, `git_diff.patch`, `wiki_*`, `mr_description.md`) stay locally ignored and are removed when housekeeping deletes the worktree.
