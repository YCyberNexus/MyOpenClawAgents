...
Files in this directory are **deployment-time pins** edited once on each runner where the agent is deployed. They are not generated from ordinary issue triggers. Runtime `/slot` and `/timeout-executor` controls persist their values in scheduler state and do not edit these files.
...
`req_executor` stores its own state under the fixed in-repo directory `${REPO_PATH}/.req_executor/`. This directory is not configurable through trigger fields or tracked config. `clone_or_pull.sh` adds `/.req_executor/` and `logs/` to the local `.git/info/exclude`. When business changes exist, `stage_and_guard.sh` force-adds the current issue's output directory and complete staging-time `${LOG_DIR}` into the MR; a log-only run still returns `NO_CHANGES`. After the terminal `worker_result.json` is written, `archive_execution_logs.sh` publishes the complete directory on append-only branch `req-executor-logs/issue-<iid>/execution-<execution_id>` without moving the business branch; later recovery evidence advances that log branch with a child snapshot commit. Unrelated `logs/` paths remain outside the commit index.
