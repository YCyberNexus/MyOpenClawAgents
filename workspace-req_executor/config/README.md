...
`campaign_defaults.env` does not define branch, per-project quota, runtime basename, project data directory, account-pool, or GitLab token fields. It defines scheduler initialization values only; `GITLAB_TOKEN` continues to use the process-environment-then-`gitlab.env` selection order above.
