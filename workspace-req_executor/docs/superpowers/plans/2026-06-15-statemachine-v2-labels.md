...
- **路径前缀**：除非特别说明，脚本在 `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/`（下称 `<SKILL>/`）。
...
- **SKILL_VERSION**：本计划所有改动都在 `workspace-req_executor/` 下，**最后一个文档任务统一 bump 一次** `SKILL.md` 第 3 行的 `[SKILL_VERSION=...]` 到 `2026-06-15.N`（见 Task 14）。中途任务不各自 bump。
...
- Modify: `workspace-req_executor/SOUL.md`
- Modify: `workspace-req_executor/AGENTS.md`
...
- [ ] **Step 5：commit**
```bash
git add <SKILL>/SKILL.md workspace-req_executor/SOUL.md workspace-req_executor/AGENTS.md CLAUDE.md
