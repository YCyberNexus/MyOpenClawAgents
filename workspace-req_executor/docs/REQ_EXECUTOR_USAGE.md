...
| `RUN_SINGLE_ISSUE_TEST` | `req_dispatcher` 主动驱动单个 issue 时发送 | 只测一个 IID，结果经 Phase 6 回调 `req_dispatcher`（不走 cron 的 `req_result` note）。详见 §4.5 |
...
### 4.5 RUN_SINGLE_ISSUE_TEST（req_dispatcher driven 单测入口）

由 `req_dispatcher` 在端到端编排一条需求时发送（`git_issuer` 建好 issue 后），让本执行器**只测一个 IID** 并把终态结果回调 `req_dispatcher`（不走 cron 的 `req_result` note）。唤醒同一 orchestrator session `agent:req_executor:main`，由 `scripts/dispatch_single_issue.sh` 处理。
...
`skills/gitlab_issue_campaign_dispatcher/references/trigger_command.md` §RUN_SINGLE_ISSUE_TEST。
