...
`req_executor`，以及 agent 从收到 trigger 到创建 MR、处理回调的完整流程。

详细实现契约位于
`workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/`。本文只描述人需要
知道和配置的内容。

## 1. Agent 是什么

`req_executor` 是一个非交互式 GitLab issue 自动化 agent，用来周期性扫描一批
...
agent 支持三个 trigger command：

| Command | 谁发送 | 用途 |
| --- | --- | --- |
| `RUN_SCHEDULED_ISSUE_CAMPAIGN` | scheduler 周期性发送 | 扫描 issue、形成 batch、spawn per-issue subagent（cron 批量路径） |
...
| `RUN_SINGLE_ISSUE` | `req_dispatcher` 主动驱动单个 issue 时发送 | 只执行一个 IID，结果经 Phase 6 回调 `req_dispatcher`（不走 cron 的 `req_result` note）。详见 §4.5 |
...
### 4.5 RUN_SINGLE_ISSUE（req_dispatcher driven 单 issue 入口）

由 `req_dispatcher` 在端到端编排一条需求时发送（`git_issuer` 建好 issue 后），让本执行器**只执行一个 IID**（coding/测试/规格等由 issue 决定）并把终态结果回调 `req_dispatcher`（不走 cron 的 `req_result` note）。唤醒同一 orchestrator session `agent:req_executor:main`，由 `scripts/dispatch_single_issue.sh` 处理。

入参（I1，只收下面这几项；其余 campaign 字段全部从 `config/gitlab.env` + `config/campaign_defaults.env` pin 取，**不**从 trigger 收）：

```text
RUN_SINGLE_ISSUE
project=<group/project 全名>            # git_issuer 回调形态，dispatch_single_issue.sh 内部拆成 group+slug
iid=<要执行的单个 IID，正整数>
correlation_id=<req_dispatcher 关联 token，原样回显在结果回调里供其匹配>
dispatcher_callback_target=<结果回调目标，⚠️ 待对齐 form>
group=<可选；缺省取 pin>
```

流程：`dispatch_single_issue.sh` 校验入参 → 拆 `project` 全名为 slug+group → 从 pin 合成等价的 `issue_iids=[iid]`、`hourly_issue_quota=1`、`max_concurrent_subagents=1` 的单 IID `RUN_SCHEDULED_ISSUE_CAMPAIGN`，并把 `{correlation_id, dispatcher_callback_target, project(全名), iid}` 写入 `${ISSUES_ROOT}/issue-<iid>/dispatch_origin.json` → 跑 Phases 1–5（与 scheduled tick 同一套机器）。Phase 6 终态（`done`/`failed`/`timeout`）时，`dispatch_followup.sh` 经 `notify_dispatcher.sh` 发 **I2 结果信封** `{correlation_id,iid,project,status,mr_url,wiki_url,reason}` 回投 `req_dispatcher`，并**跳过** driven 路径不需要的 `req_result` note。

driven 路径与 cron 路径相互独立：`RUN_SCHEDULED_ISSUE_CAMPAIGN` + cron 原样保留供批量/backfill 与非 driven 来源用。完整入参/信封与待对齐项见
`skills/gitlab_issue_campaign_dispatcher/references/trigger_command.md` §RUN_SINGLE_ISSUE。
...
| inner Claude Code prompt | `${LOG_DIR}/prompt.txt` | `acpx claude exec -f` | 指挥 Claude Code 根据 GitLab issue 生成 issue 要求的产物（代码/测试/规格/文档等） |
...
- `workspace-req_executor/USER.md`
- `workspace-req_executor/SOUL.md`
- `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/SKILL.md`
- `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/references/trigger_command.md`
...
- `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/references/paths.md`
