...
- `RUN_SINGLE_ISSUE` — `req_dispatcher`-driven single-issue entry (I1). `dispatch_single_issue.sh` pins the rest of the campaign from `config/`, records the issue's `dispatch_origin.json` (`correlation_id` + `dispatcher_callback_target`), and synthesizes an equivalent single-IID `RUN_SCHEDULED_ISSUE_CAMPAIGN` (quota=1, concurrency=1). Its terminal result is reported back to `req_dispatcher` via the Phase 6 I2 callback (`notify_dispatcher.sh`), not the cron `req_result` note.
