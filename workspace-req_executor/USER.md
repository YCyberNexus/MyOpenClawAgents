...
- `branch` is the **integration / target** branch (typically `master`); `dev_branch` is the **clean baseline** (typically `dev`) used to reset fresh-mode repo checkouts so Claude Code does not see past issues' output accumulation, and it is also the source for refreshing tracked shared config paths (`.claude/`, `hulat/`, and the data directory) before every run. If your project has no separate baseline, set `dev_branch=<same-as-branch>`.
