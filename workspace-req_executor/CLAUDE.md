...
This is **not** an application repo — it is the **deployment artifact for an OpenClaw agent** named `req_executor`. It contains the agent's prompt contracts (`SOUL.md`, `AGENTS.md`, `USER.md`), one SKILL (`gitlab_issue_campaign_dispatcher`), and the bash scripts that SKILL invokes. There is no build, no test runner, no package manifest. Changes here are deployed by syncing this workspace to the runner.
...
**Orchestrator (`workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/`)** stays alive across scheduler ticks (`agent:req_executor:main`). It runs different phases on each of its two trigger commands:

### Path A: scheduled wake-up (`RUN_SCHEDULED_ISSUE_CAMPAIGN`)

| Phase | What |
| ----- | ---- |
| 1 Parse        | bootstrap, flock, load + override `campaign_state.json` from trigger; **stuck-pending eviction** (synthesizes Phase 6 replies for any pending entries past `stuck_after_minutes`: `timeout` — parked, no auto-retry — when the run outlived `acpx_timeout_seconds - 60s` since `spawned_at`, which is always true under the default `stuck_after_minutes`; `blocked-dispatcher` [stuck-non-timeout is a dispatcher-synthesized termination] otherwise) |
...
| 4 Per-IID Prep | for each IID in the batch: `allocate_attempt.sh` → load that IID's UI account slot from `${REPO_PATH}/${UI_ACCOUNTS_RELPATH}` (no default; opt in via trigger `ui_accounts_relpath`, carry-forward persisted; relpath resolved under the project checkout root, NOT under `${REPO_PATH}/${DATA_BASENAME}/`; when unconfigured the entire UI-account flow is skipped and the rendered Claude Code prompt omits its `# UI test accounts` section) → `resolve_model_tier` (determines effective `model:{tier}` label for this attempt: upgrades on CC-side failures `blocked-cc`/`timeout`/`failed-cc` from prior attempt, on `quality:low` signal, or when `continue_count >= continue_upgrade_threshold` [trigger field, default 2]; dispatcher-side failures do not trigger upgrade; tier is capped at highest tier in `model_tiers` [trigger field, ordered list of `{tier,settings}`, carry-forward]; unknown tier falls back to default; `quality:low` is consumed after evaluation) → `prepare_attempt.sh` (creates the shared per-issue linked worktree at `${REPO_PATH}/${RESULT_BASENAME}/.worktrees/issue-<iid>/` via `git worktree add -B` on attempt 1, or in-place force-switches the checked-out branch to BASE_REF on attempt N>1 after preserving same-IID `${RESULT_BASENAME}/issue-<iid>/` files; only `continue` restores those files into the active worktree, while every non-continue entry label resets from the clean baseline; tracked shared config paths `hulat/`, `.claude/`, and `ifp-data/` are refreshed from latest `origin/${DEV_BRANCH}` before every run) → reads issue title/url/labels/body via `glab` → `set_issue_label.sh` transitions entry labels (`todo` / `retry` / `new` / `continue` / `blocked-cc` / `blocked-dispatcher` / `blocked` [compat] plus matched trigger labels) to `doing`; sets `model:{tier}` label per resolved tier → `build_prompt.sh` (writes `${LOG_DIR}/prompt.txt` with UI account and effective model settings injected) → initializes `${ISSUE_STATE_FILE}` and `${ATTEMPT_STATE_FILE}` (status=in_progress) → writes `pending_subagents[iid]` placeholder → renders [`references/executor_prompt.md`](skills/gitlab_issue_campaign_dispatcher/references/executor_prompt.md) with per-IID values |
| 5 Async Spawn  | one **anonymous** `sessions_spawn` call per surviving IID (NO session name passed — runtime returns `runId` + `childSessionKey` like `agent:req_executor:subagent:<uuid>`). Launch failures retry the identical payload up to 3 total attempts with 2-second fixed backoff; only a valid ack is recorded into `pending_subagents[iid]`. Returns `waiting_for_callbacks` and exits. **Phase 6 does NOT run on this path** (except inline-synthesized `blocked-dispatcher` for launch failures, which do not increment `retry_count`). |
...
| 6 Follow-up | parse + validate the callback's compact JSON → match to `pending_subagents[reply.iid]` (Phase 6 validation rule 2; reply.attempt_number must equal pending entry's) → synchronize live labels (`pr` [completion, replaces `done`; `done` is a pre-MR transient removed when `pr` is added], `blocked-cc` [CC/subagent-side failure], `blocked-dispatcher` [dispatcher-synthesized: prep/launch_failed/scope-evict/stuck-non-timeout/reply-downgrade/label-sync failure], `failed-cc` / `failed-dispatcher` [same side-split], or `timeout` [unsplit]; entries carry `block_side=cc|dispatcher` in state.json) → write **terminal** `${ISSUE_STATE_FILE}` + `${ATTEMPT_STATE_FILE}` → drain pending entry → classify into `completed_iids` / `blocked_iids` / `failed_iids` / `timeout_iids` (promote `blocked-cc → failed-cc` or `blocked-dispatcher → failed-dispatcher` if `retry_count > blocked_retry_limit`) → best-effort `subagents kill --target <child_session_key>` for terminal `pr` / `blocked-*` / `failed-*` / `timeout` when `kill_subagent_on_terminal=true` (default; blocked/failed/timeout cleanup first requires local evidence under `${LOG_DIR}` / `${ISSUE_ROOT}`) → when `result_note_enabled` (default false), best-effort `post_result_note.sh` for terminal `done`/`failed`/`timeout`: read the issue's `req_origin` marker note and, if present, post a `req_result` note for the 114-side relay (结果回报; pure glab, non-fatal, no-op when no `req_origin`) → return |
...
1. SETUP: call `bash ${SCRIPTS_DIR}/run_acpx_attempt.sh` with the standard per-issue env. The script `cd`s into `${WORKTREE_DIR}` (the shared per-issue linked worktree for this IID) and owns the fixed `acpx --auth-policy skip claude exec -f ${LOG_DIR}/prompt.txt` invocation. Current acpx releases expose `claude exec` as a one-shot command with no saved-session flag, so attempts of the same IID do NOT share Claude-Code session memory. Cross-attempt continuity for `continue` comes from four layers: (a) the self-contained rendered prompt (incl. prior attempt summaries auto-posted as GitLab notes and reviewer comments); (b) the committed `origin/${WORK_BRANCH}` contents or latest local prior-attempt branch that continue/resume attempts check out; (c) tracked shared config paths refreshed from latest `origin/${DEV_BRANCH}`; and (d) the shared per-issue worktree's `${RESULT_BASENAME}/issue-<iid>/` subtree, which `prepare_attempt.sh` snapshots and restores around in-place branch switches. Non-continue entry labels (`todo`, `retry`, `new`, `blocked-cc`, `blocked-dispatcher`, `blocked` [legacy compat], trigger require_labels) are reset runs: the prior subtree is archived, not restored into the active worktree. See `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/references/executor_prompt.md` and `references/continue_mode.md` for the externalized continuity contract.
...
6. `set_issue_label.sh` `doing → done` (transient; `done` is removed in step 8 when `pr` is added).
...
8. `set_issue_label.sh add pr; remove done` after MR creation succeeds (`pr` is the completion signal; `done` is removed at the same time — live completion state = `pr` only).
9. `summarize_attempt.sh` writes a local per-attempt summary; successful `pr` attempts also post it as a GitLab issue note, while failure summaries stay local.
10. **Emit ONE compact JSON line** on the LAST line of its turn, carrying every fact the orchestrator's Phase 6 needs (`iid`, `attempt_number`, `status`, `mode_actual`, `work_branch`, `local_branch`, `commit_sha`, `merge_request_url`, `mr_action`, `wiki_url`, `labels_added`, `labels_removed`, `summary_posted`, `block_reason`, `block_side` [cc for subagent-originated failures; dispatcher for dispatcher-synthesized; omitted on non-blocked/non-failed outcomes], `log_dir`).

On any subagent FAIL path, remove `doing` and add `blocked-cc` (`block_side=cc` in compact reply) before summarizing and returning the compact JSON. For non-timeout acpx failures, the subagent first tries `stage_and_guard.sh` → `commit_and_push.sh` → `post_push_verify.sh` so committable partial work reaches `${WORK_BRANCH}` even though the issue remains `blocked-cc` and no Wiki/MR/`pr` is produced. Phase 6 re-applies the final label state idempotently when the callback arrives. Dispatcher-synthesized failures (prep, launch_failed, scope-evict, stuck-non-timeout, reply-downgrade, label-sync failures) use `blocked-dispatcher` (`block_side=dispatcher`) instead.
...
Key reconciled signals: `is_closed_on_gitlab` (closed = hard terminal skip, never schedule), `has_pr` (`pr` label present = completed; v2: `done` alone is NOT completion — `done` is a transient pre-MR label removed when `pr` is added; legacy `has_done_pr` check treated as `has_pr` when only `pr` is present), `needs_continue` (opened + has `continue` or legacy `contiune` = reviewer wants resume; wins over cached `pr` / completed state), `user_reopened` (opened + missing `pr` + no `failed-cc`/`failed-dispatcher`/`failed`/`blocked-cc`/`blocked-dispatcher`/`blocked`/`doing`/`continue`/`contiune`; `timeout` 仅在叠加 `retry` 时允许——残留的 `doing` 说明这是 dispatcher 终态但标签同步未落地,磁盘停放必须保留); `model_tier` (current tier label `model:{tier}` present on issue, read by reconcile to seed `state.json.model_tier` if disk state is missing); `has_blocked_cc` / `has_blocked_dispatcher` / `has_failed_cc` / `has_failed_dispatcher` (split-side terminal states; bare `blocked`/`failed` legacy labels recognized as compat equiv of `*-cc` respectively).
    ...
    ifp-result/                                ← agent runtime root
        ...
        .worktrees/                            ← per-issue linked git worktrees (one per IID, shared across attempts)
            issue-<iid>/                       ← ${WORKTREE_DIR}; acpx cwd; created on attempt 1 via `git worktree add -B`,
                ...
                ifp-result/issue-<iid>/output/   ← Claude Code's standalone issue deliverables (force-added; lands in MR); shared across attempts
...
- `branch` (typically `master`) — **integration / target** branch. MRs are opened against it. Each issue's primary committable output lives under `ifp-result/issue-<iid>/output/` so MRs into master never collide.
- `dev_branch` (typically `dev`) — **clean baseline and shared config source**. Fresh-mode attempts reset their per-issue worktree to `origin/${dev_branch}` so Claude Code does not see past issues' output accumulation on tracked files. Every attempt, including continue, refreshes tracked `hulat/`, `.claude/`, and `${DATA_BASENAME}/` from latest `origin/${dev_branch}` before acpx runs. Untracked scratch left in the shared worktree by a prior attempt is NOT touched by the reset (that is intentional — `acpx claude exec` resumes from it). Set `dev_branch=<same-as-branch>` to disable.
...
- The Claude Code invocation contract is `bash ${SCRIPTS_DIR}/run_acpx_attempt.sh` with the standard per-issue env. That script is the only place that constructs the `acpx --auth-policy skip claude exec -f "${LOG_DIR}/prompt.txt"` command, and it runs from `cwd=${WORKTREE_DIR}` (the shared per-issue linked worktree for this IID, NOT the parent checkout). Current acpx releases expose `claude exec` as a one-shot command with no saved-session flag — there is no `-s` / session persistence inside acpx, so attempts of the same IID run independently at the acpx level. Cross-attempt continuity is externalized via (a) the rendered prompt, prior attempt summaries (auto-posted GitLab notes for successful `pr` attempts), and reviewer comments; (b) the `origin/${WORK_BRANCH}` contents that continue-mode resets check out (including the force-added `log/attempt-NNN/prompt.txt` + `claude_result.txt`); AND (c) any untracked files Claude Code left in the shared per-issue worktree that the in-place branch switch in `prepare_attempt.sh` preserves. See `references/executor_prompt.md` and `references/continue_mode.md`. Start the script with PTY on the first attempt, use a timeout that covers the expected run, and if the exec tool yields a pollable process, poll the same process until it exits. Do not re-run the script after a tool timeout — if no pollable process is available (no captured `ACPX_EXIT=` line), enter the TIMEOUT flow: FAIL `status=timeout` with `block_reason="acpx exec exceeded ${ACPX_TIMEOUT_SECONDS}s wall-clock cap"` (timeout 停放为终态、不自动重试,绝不改报 `blocked`). No direct `acpx` call from the subagent, no `--no-wait`, no streaming mode, no calling `claude` directly without acpx, no other LLM as substitute.
- If `git push` is rejected, mark `blocked-cc` — do **not** add extra `git push --force` outside `commit_and_push.sh`, do not rebase and re-push.
...
- If a per-IID dispatcher prep step fails (clone_or_pull, prepare_attempt, build_prompt, label transitions), mark that IID `blocked-dispatcher` (`block_side=dispatcher`) and end the serial batch. Do NOT spawn an IID with partial setup. Do NOT retry the failed prep inline.
- If `sessions_spawn` launch fails, retry the SAME payload up to 3 total attempts with 2-second fixed backoff. If all attempts fail, synthesize `blocked-dispatcher` (`block_side=dispatcher`) for that IID, preserve the last raw error in `block_reason`, do not add it to `pending_subagents`, and do not increment `retry_count`.
...
The runner has this deployment-time pin at `workspace-req_executor/config/`:
...
After any edit to a file under `workspace-req_executor/` — including `SOUL.md`, `AGENTS.md`, `USER.md`, `config/`, and anything under `skills/gitlab_issue_campaign_dispatcher/` (the SKILL itself, its `scripts/`, its `references/`) — bump the `[SKILL_VERSION=...]` token at the start of line 3 of [`workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/SKILL.md`](skills/gitlab_issue_campaign_dispatcher/SKILL.md) (inside the `description:` field).
...
Bump the version in the SAME edit/commit that introduces the workspace change — do not leave it for a follow-up. Edits to files OUTSIDE `workspace-req_executor/` (e.g. this `CLAUDE.md`, repo-root files, `.claude/`) do NOT trigger a bump.
...
This applies to all edits under `workspace-req_executor/` (scripts, references, SKILL.md, SOUL.md, AGENTS.md, USER.md, config/). Trivial changes (typos, version bumps, single-line fixes) can skip the loop at the main agent's discretion.

A project-local Stop hook ([`.claude/hooks/require-workspace-review.sh`](.claude/hooks/require-workspace-review.sh), registered in [`.claude/settings.json`](.claude/settings.json)) enforces this: when the turn tries to end with uncommitted changes under `workspace-req_executor/`, it returns `decision:"block"` and feeds back the review instruction. After the loop completes (or the change is genuinely trivial), clear the block by writing the current diff fingerprint to the hook's sentinel `workspace-req_executor/.claude/.review-done-sha` — the hook's reason text includes the exact `printf %s '<hash>' > <sentinel>` line to run (the sentinel is emitted as an absolute path, valid from any cwd, since Claude Code may be launched from inside `workspace-req_executor/`).

## Where to look for full details

- Workspace contracts: `workspace-req_executor/SOUL.md` (subagent concurrency policy, no-fallback, GitLab access, host pinning, per-exec env contract, working directory, source of truth).
- Dispatcher algorithm + spawn payload: `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/SKILL.md` (§Dispatcher Algorithm, §UI Account Allocation Policy, §Source-of-Truth Policy, §Concurrency Policy).
- The subagent's prompt template: `workspace-req_executor/skills/gitlab_issue_campaign_dispatcher/references/executor_prompt.md` — the dispatcher renders this and ships it as the spawn payload.
